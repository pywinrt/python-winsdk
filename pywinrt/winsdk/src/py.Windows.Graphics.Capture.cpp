...
PyMODINIT_FUNC PyInit__winsdk_Windows_Graphics_Capture(void) noexcept
{
    ...
    state->type_Direct3D11CaptureFrame = py::register_python_type(module.get(), type_name_Direct3D11CaptureFrame, &type_spec_Direct3D11CaptureFrame, bases.get(), nullptr);
    if (!state->type_Direct3D11CaptureFrame)
    {
        return nullptr;
    }

    state->type_Direct3D11CaptureFramePool = py::register_python_type(module.get(), type_name_Direct3D11CaptureFramePool, &type_spec_Direct3D11CaptureFramePool, bases.get(), nullptr);
    if (!state->type_Direct3D11CaptureFramePool)
    {
        return nullptr;
    }

    state->type_GraphicsCaptureAccess = py::register_python_type(module.get(), type_name_GraphicsCaptureAccess, &type_spec_GraphicsCaptureAccess, nullptr, nullptr);
    if (!state->type_GraphicsCaptureAccess)
    {
        return nullptr;
    }

    state->type_GraphicsCaptureItem = py::register_python_type(module.get(), type_name_GraphicsCaptureItem, &type_spec_GraphicsCaptureItem, bases.get(), nullptr);
    if (!state->type_GraphicsCaptureItem)
    {
        return nullptr;
    }

    state->type_GraphicsCapturePicker = py::register_python_type(module.get(), type_name_GraphicsCapturePicker, &type_spec_GraphicsCapturePicker, bases.get(), nullptr);
    if (!state->type_GraphicsCapturePicker)
    {
        return nullptr;
    }

    state->type_GraphicsCaptureSession = py::register_python_type(module.get(), type_name_GraphicsCaptureSession, &type_spec_GraphicsCaptureSession, bases.get(), nullptr);

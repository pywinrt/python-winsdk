...
PyMODINIT_FUNC PyInit__winsdk_Windows_Graphics_Printing_PrintSupport(void) noexcept
{
    ...
    state->type_PrintSupportExtensionSession = py::register_python_type(module.get(), type_name_PrintSupportExtensionSession, &type_spec_PrintSupportExtensionSession, bases.get(), nullptr);
    if (!state->type_PrintSupportExtensionSession)
    {
        return nullptr;
    }

    state->type_PrintSupportExtensionTriggerDetails = py::register_python_type(module.get(), type_name_PrintSupportExtensionTriggerDetails, &type_spec_PrintSupportExtensionTriggerDetails, bases.get(), nullptr);
    if (!state->type_PrintSupportExtensionTriggerDetails)
    {
        return nullptr;
    }

    state->type_PrintSupportPrintDeviceCapabilitiesChangedEventArgs = py::register_python_type(module.get(), type_name_PrintSupportPrintDeviceCapabilitiesChangedEventArgs, &type_spec_PrintSupportPrintDeviceCapabilitiesChangedEventArgs, bases.get(), nullptr);
    if (!state->type_PrintSupportPrintDeviceCapabilitiesChangedEventArgs)
    {
        return nullptr;
    }

    state->type_PrintSupportPrintDeviceCapabilitiesUpdatePolicy = py::register_python_type(module.get(), type_name_PrintSupportPrintDeviceCapabilitiesUpdatePolicy, &type_spec_PrintSupportPrintDeviceCapabilitiesUpdatePolicy, bases.get(), nullptr);
    if (!state->type_PrintSupportPrintDeviceCapabilitiesUpdatePolicy)
    {
        return nullptr;
    }

    state->type_PrintSupportPrintTicketElement = py::register_python_type(module.get(), type_name_PrintSupportPrintTicketElement, &type_spec_PrintSupportPrintTicketElement, bases.get(), nullptr);
    if (!state->type_PrintSupportPrintTicketElement)
    {
        return nullptr;
    }

    state->type_PrintSupportPrintTicketValidationRequestedEventArgs = py::register_python_type(module.get(), type_name_PrintSupportPrintTicketValidationRequestedEventArgs, &type_spec_PrintSupportPrintTicketValidationRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_PrintSupportPrintTicketValidationRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_PrintSupportPrinterSelectedEventArgs = py::register_python_type(module.get(), type_name_PrintSupportPrinterSelectedEventArgs, &type_spec_PrintSupportPrinterSelectedEventArgs, bases.get(), nullptr);
    if (!state->type_PrintSupportPrinterSelectedEventArgs)
    {
        return nullptr;
    }

    state->type_PrintSupportSessionInfo = py::register_python_type(module.get(), type_name_PrintSupportSessionInfo, &type_spec_PrintSupportSessionInfo, bases.get(), nullptr);
    if (!state->type_PrintSupportSessionInfo)
    {
        return nullptr;
    }

    state->type_PrintSupportSettingsActivatedEventArgs = py::register_python_type(module.get(), type_name_PrintSupportSettingsActivatedEventArgs, &type_spec_PrintSupportSettingsActivatedEventArgs, bases.get(), nullptr);
    if (!state->type_PrintSupportSettingsActivatedEventArgs)
    {
        return nullptr;
    }

    state->type_PrintSupportSettingsUISession = py::register_python_type(module.get(), type_name_PrintSupportSettingsUISession, &type_spec_PrintSupportSettingsUISession, bases.get(), nullptr);

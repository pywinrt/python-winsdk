...
PyMODINIT_FUNC PyInit__winsdk_Windows_Devices_WiFiDirect(void) noexcept
{
    ...
    state->type_WiFiDirectAdvertisement = py::register_python_type(module.get(), type_name_WiFiDirectAdvertisement, &type_spec_WiFiDirectAdvertisement, bases.get(), nullptr);
    if (!state->type_WiFiDirectAdvertisement)
    {
        return nullptr;
    }

    state->type_WiFiDirectAdvertisementPublisher = py::register_python_type(module.get(), type_name_WiFiDirectAdvertisementPublisher, &type_spec_WiFiDirectAdvertisementPublisher, bases.get(), nullptr);
    if (!state->type_WiFiDirectAdvertisementPublisher)
    {
        return nullptr;
    }

    state->type_WiFiDirectAdvertisementPublisherStatusChangedEventArgs = py::register_python_type(module.get(), type_name_WiFiDirectAdvertisementPublisherStatusChangedEventArgs, &type_spec_WiFiDirectAdvertisementPublisherStatusChangedEventArgs, bases.get(), nullptr);
    if (!state->type_WiFiDirectAdvertisementPublisherStatusChangedEventArgs)
    {
        return nullptr;
    }

    state->type_WiFiDirectConnectionListener = py::register_python_type(module.get(), type_name_WiFiDirectConnectionListener, &type_spec_WiFiDirectConnectionListener, bases.get(), nullptr);
    if (!state->type_WiFiDirectConnectionListener)
    {
        return nullptr;
    }

    state->type_WiFiDirectConnectionParameters = py::register_python_type(module.get(), type_name_WiFiDirectConnectionParameters, &type_spec_WiFiDirectConnectionParameters, bases.get(), nullptr);
    if (!state->type_WiFiDirectConnectionParameters)
    {
        return nullptr;
    }

    state->type_WiFiDirectConnectionRequest = py::register_python_type(module.get(), type_name_WiFiDirectConnectionRequest, &type_spec_WiFiDirectConnectionRequest, bases.get(), nullptr);
    if (!state->type_WiFiDirectConnectionRequest)
    {
        return nullptr;
    }

    state->type_WiFiDirectConnectionRequestedEventArgs = py::register_python_type(module.get(), type_name_WiFiDirectConnectionRequestedEventArgs, &type_spec_WiFiDirectConnectionRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_WiFiDirectConnectionRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_WiFiDirectDevice = py::register_python_type(module.get(), type_name_WiFiDirectDevice, &type_spec_WiFiDirectDevice, bases.get(), nullptr);
    if (!state->type_WiFiDirectDevice)
    {
        return nullptr;
    }

    state->type_WiFiDirectInformationElement = py::register_python_type(module.get(), type_name_WiFiDirectInformationElement, &type_spec_WiFiDirectInformationElement, bases.get(), nullptr);
    if (!state->type_WiFiDirectInformationElement)
    {
        return nullptr;
    }

    state->type_WiFiDirectLegacySettings = py::register_python_type(module.get(), type_name_WiFiDirectLegacySettings, &type_spec_WiFiDirectLegacySettings, bases.get(), nullptr);

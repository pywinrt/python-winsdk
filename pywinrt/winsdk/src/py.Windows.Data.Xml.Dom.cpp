...
namespace py::cpp::Windows::Data::Xml::Dom
{
    ...
    static PyType_Slot _type_slots_XmlNamedNodeMap[] = 
    {
        ...
        { Py_mp_subscript, _seq_subscript_XmlNamedNodeMap },
        { },
    };
    ...
    static PyType_Slot _type_slots_XmlNodeList[] = 
    {
        ...
        { Py_mp_subscript, _seq_subscript_XmlNodeList },
        { },
    };
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Data_Xml_Dom(void) noexcept
{
    ...
    state->type_DtdEntity = py::register_python_type(module.get(), type_name_DtdEntity, &type_spec_DtdEntity, bases.get(), nullptr);
    if (!state->type_DtdEntity)
    {
        return nullptr;
    }

    state->type_DtdNotation = py::register_python_type(module.get(), type_name_DtdNotation, &type_spec_DtdNotation, bases.get(), nullptr);
    if (!state->type_DtdNotation)
    {
        return nullptr;
    }

    state->type_XmlAttribute = py::register_python_type(module.get(), type_name_XmlAttribute, &type_spec_XmlAttribute, bases.get(), nullptr);
    if (!state->type_XmlAttribute)
    {
        return nullptr;
    }

    state->type_XmlCDataSection = py::register_python_type(module.get(), type_name_XmlCDataSection, &type_spec_XmlCDataSection, bases.get(), nullptr);
    if (!state->type_XmlCDataSection)
    {
        return nullptr;
    }

    state->type_XmlComment = py::register_python_type(module.get(), type_name_XmlComment, &type_spec_XmlComment, bases.get(), nullptr);
    if (!state->type_XmlComment)
    {
        return nullptr;
    }

    state->type_XmlDocument = py::register_python_type(module.get(), type_name_XmlDocument, &type_spec_XmlDocument, bases.get(), nullptr);
    if (!state->type_XmlDocument)
    {
        return nullptr;
    }

    state->type_XmlDocumentFragment = py::register_python_type(module.get(), type_name_XmlDocumentFragment, &type_spec_XmlDocumentFragment, bases.get(), nullptr);
    if (!state->type_XmlDocumentFragment)
    {
        return nullptr;
    }

    state->type_XmlDocumentType = py::register_python_type(module.get(), type_name_XmlDocumentType, &type_spec_XmlDocumentType, bases.get(), nullptr);
    if (!state->type_XmlDocumentType)
    {
        return nullptr;
    }

    state->type_XmlDomImplementation = py::register_python_type(module.get(), type_name_XmlDomImplementation, &type_spec_XmlDomImplementation, bases.get(), nullptr);
    if (!state->type_XmlDomImplementation)
    {
        return nullptr;
    }

    state->type_XmlElement = py::register_python_type(module.get(), type_name_XmlElement, &type_spec_XmlElement, bases.get(), nullptr);
    if (!state->type_XmlElement)
    {
        return nullptr;
    }

    state->type_XmlEntityReference = py::register_python_type(module.get(), type_name_XmlEntityReference, &type_spec_XmlEntityReference, bases.get(), nullptr);
    if (!state->type_XmlEntityReference)
    {
        return nullptr;
    }

    state->type_XmlLoadSettings = py::register_python_type(module.get(), type_name_XmlLoadSettings, &type_spec_XmlLoadSettings, bases.get(), nullptr);
    if (!state->type_XmlLoadSettings)
    {
        return nullptr;
    }

    state->type_XmlNamedNodeMap = py::register_python_type(module.get(), type_name_XmlNamedNodeMap, &type_spec_XmlNamedNodeMap, sequence_bases.get(), nullptr);
    if (!state->type_XmlNamedNodeMap)
    {
        return nullptr;
    }

    state->type_XmlNodeList = py::register_python_type(module.get(), type_name_XmlNodeList, &type_spec_XmlNodeList, sequence_bases.get(), nullptr);
    if (!state->type_XmlNodeList)
    {
        return nullptr;
    }

    state->type_XmlProcessingInstruction = py::register_python_type(module.get(), type_name_XmlProcessingInstruction, &type_spec_XmlProcessingInstruction, bases.get(), nullptr);
    if (!state->type_XmlProcessingInstruction)
    {
        return nullptr;
    }

    state->type_XmlText = py::register_python_type(module.get(), type_name_XmlText, &type_spec_XmlText, bases.get(), nullptr);
    if (!state->type_XmlText)
    {
        return nullptr;
    }

    state->type_IXmlCharacterData = py::register_python_type(module.get(), type_name_IXmlCharacterData, &type_spec_IXmlCharacterData, bases.get(), nullptr);
    if (!state->type_IXmlCharacterData)
    {
        return nullptr;
    }

    state->type_IXmlNode = py::register_python_type(module.get(), type_name_IXmlNode, &type_spec_IXmlNode, bases.get(), nullptr);
    if (!state->type_IXmlNode)
    {
        return nullptr;
    }

    state->type_IXmlNodeSelector = py::register_python_type(module.get(), type_name_IXmlNodeSelector, &type_spec_IXmlNodeSelector, bases.get(), nullptr);
    if (!state->type_IXmlNodeSelector)
    {
        return nullptr;
    }

    state->type_IXmlNodeSerializer = py::register_python_type(module.get(), type_name_IXmlNodeSerializer, &type_spec_IXmlNodeSerializer, bases.get(), nullptr);
    if (!state->type_IXmlNodeSerializer)
    {
        return nullptr;
    }

    state->type_IXmlText = py::register_python_type(module.get(), type_name_IXmlText, &type_spec_IXmlText, bases.get(), nullptr);

...
namespace py::cpp::Windows::Storage::Search
{
    ...
    static PyObject* _seq_subscript_SortEntryVector(py::wrapper::Windows::Storage::Search::SortEntryVector* self, PyObject* slice) noexcept
    {
        try
        {
            ...
            winrt::com_array<winrt::Windows::Storage::Search::SortEntry> items(length, empty_instance<winrt::Windows::Storage::Search::SortEntry>::get());
    ...
    static PyType_Slot _type_slots_SortEntryVector[] = 
    {
        ...
        { Py_mp_subscript, _seq_subscript_SortEntryVector },
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Storage_Search(void) noexcept
{
    ...
    state->type_ContentIndexer = py::register_python_type(module.get(), type_name_ContentIndexer, &type_spec_ContentIndexer, bases.get(), nullptr);
    if (!state->type_ContentIndexer)
    {
        return nullptr;
    }

    state->type_ContentIndexerQuery = py::register_python_type(module.get(), type_name_ContentIndexerQuery, &type_spec_ContentIndexerQuery, bases.get(), nullptr);
    if (!state->type_ContentIndexerQuery)
    {
        return nullptr;
    }

    state->type_IndexableContent = py::register_python_type(module.get(), type_name_IndexableContent, &type_spec_IndexableContent, bases.get(), nullptr);
    if (!state->type_IndexableContent)
    {
        return nullptr;
    }

    state->type_QueryOptions = py::register_python_type(module.get(), type_name_QueryOptions, &type_spec_QueryOptions, bases.get(), nullptr);
    if (!state->type_QueryOptions)
    {
        return nullptr;
    }

    state->type_SortEntryVector = py::register_python_type(module.get(), type_name_SortEntryVector, &type_spec_SortEntryVector, mutable_sequence_bases.get(), nullptr);
    if (!state->type_SortEntryVector)
    {
        return nullptr;
    }

    state->type_StorageFileQueryResult = py::register_python_type(module.get(), type_name_StorageFileQueryResult, &type_spec_StorageFileQueryResult, bases.get(), nullptr);
    if (!state->type_StorageFileQueryResult)
    {
        return nullptr;
    }

    state->type_StorageFolderQueryResult = py::register_python_type(module.get(), type_name_StorageFolderQueryResult, &type_spec_StorageFolderQueryResult, bases.get(), nullptr);
    if (!state->type_StorageFolderQueryResult)
    {
        return nullptr;
    }

    state->type_StorageItemQueryResult = py::register_python_type(module.get(), type_name_StorageItemQueryResult, &type_spec_StorageItemQueryResult, bases.get(), nullptr);
    if (!state->type_StorageItemQueryResult)
    {
        return nullptr;
    }

    state->type_StorageLibraryChangeTrackerTriggerDetails = py::register_python_type(module.get(), type_name_StorageLibraryChangeTrackerTriggerDetails, &type_spec_StorageLibraryChangeTrackerTriggerDetails, bases.get(), nullptr);
    if (!state->type_StorageLibraryChangeTrackerTriggerDetails)
    {
        return nullptr;
    }

    state->type_StorageLibraryContentChangedTriggerDetails = py::register_python_type(module.get(), type_name_StorageLibraryContentChangedTriggerDetails, &type_spec_StorageLibraryContentChangedTriggerDetails, bases.get(), nullptr);
    if (!state->type_StorageLibraryContentChangedTriggerDetails)
    {
        return nullptr;
    }

    state->type_ValueAndLanguage = py::register_python_type(module.get(), type_name_ValueAndLanguage, &type_spec_ValueAndLanguage, bases.get(), nullptr);
    if (!state->type_ValueAndLanguage)
    {
        return nullptr;
    }

    state->type_IIndexableContent = py::register_python_type(module.get(), type_name_IIndexableContent, &type_spec_IIndexableContent, bases.get(), nullptr);
    if (!state->type_IIndexableContent)
    {
        return nullptr;
    }

    state->type_IStorageFolderQueryOperations = py::register_python_type(module.get(), type_name_IStorageFolderQueryOperations, &type_spec_IStorageFolderQueryOperations, bases.get(), nullptr);
    if (!state->type_IStorageFolderQueryOperations)
    {
        return nullptr;
    }

    state->type_IStorageQueryResultBase = py::register_python_type(module.get(), type_name_IStorageQueryResultBase, &type_spec_IStorageQueryResultBase, bases.get(), nullptr);
    if (!state->type_IStorageQueryResultBase)
    {
        return nullptr;
    }

    state->type_SortEntry = py::register_python_type(module.get(), type_name_SortEntry, &type_spec_SortEntry, bases.get(), nullptr);

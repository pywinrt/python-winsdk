...
namespace py::cpp::Windows::System::Threading
{
    ...
    static PyObject* register_WorkItemOptions(PyObject* module, PyObject* type)
    {
    ...
    static PyObject* register_WorkItemPriority(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
...
PyMODINIT_FUNC PyInit__winsdk_Windows_System_Threading(void) noexcept
{
    ...
    state->type_ThreadPool = py::register_python_type(module.get(), type_name_ThreadPool, &type_spec_ThreadPool, nullptr);
    if (!state->type_ThreadPool)
    {
        return nullptr;
    }

    Py_INCREF(state->type_ThreadPool);

    state->type_ThreadPoolTimer = py::register_python_type(module.get(), type_name_ThreadPoolTimer, &type_spec_ThreadPoolTimer, bases.get());
    if (!state->type_ThreadPoolTimer)
    {
        return nullptr;
    }

    Py_INCREF(state->type_ThreadPoolTimer);

...
namespace py::cpp::Windows::Media::Transcoding
{
    ...
    static PyObject* register_MediaVideoProcessingAlgorithm(PyObject* module, PyObject* type)
    {
    ...
    static PyObject* register_TranscodeFailureReason(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    constexpr const char* const type_name_MediaTranscoder = "MediaTranscoder";
    ...
    constexpr const char* const type_name_PrepareTranscodeResult = "PrepareTranscodeResult";
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Media_Transcoding(void) noexcept
{
    ...
    state->type_MediaTranscoder = py::register_python_type(module.get(), type_name_MediaTranscoder, &type_spec_MediaTranscoder, bases.get());
    if (!state->type_MediaTranscoder)
    {
        return nullptr;
    }

    Py_INCREF(state->type_MediaTranscoder);

    state->type_PrepareTranscodeResult = py::register_python_type(module.get(), type_name_PrepareTranscodeResult, &type_spec_PrepareTranscodeResult, bases.get());
    if (!state->type_PrepareTranscodeResult)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PrepareTranscodeResult);

...
PyMODINIT_FUNC PyInit__winsdk_Windows_Storage_FileProperties(void) noexcept
{
    ...
    state->type_BasicProperties = py::register_python_type(module.get(), type_name_BasicProperties, &type_spec_BasicProperties, bases.get(), nullptr);
    if (!state->type_BasicProperties)
    {
        return nullptr;
    }

    state->type_DocumentProperties = py::register_python_type(module.get(), type_name_DocumentProperties, &type_spec_DocumentProperties, bases.get(), nullptr);
    if (!state->type_DocumentProperties)
    {
        return nullptr;
    }

    state->type_GeotagHelper = py::register_python_type(module.get(), type_name_GeotagHelper, &type_spec_GeotagHelper, nullptr, nullptr);
    if (!state->type_GeotagHelper)
    {
        return nullptr;
    }

    state->type_ImageProperties = py::register_python_type(module.get(), type_name_ImageProperties, &type_spec_ImageProperties, bases.get(), nullptr);
    if (!state->type_ImageProperties)
    {
        return nullptr;
    }

    state->type_MusicProperties = py::register_python_type(module.get(), type_name_MusicProperties, &type_spec_MusicProperties, bases.get(), nullptr);
    if (!state->type_MusicProperties)
    {
        return nullptr;
    }

    state->type_StorageItemContentProperties = py::register_python_type(module.get(), type_name_StorageItemContentProperties, &type_spec_StorageItemContentProperties, bases.get(), nullptr);
    if (!state->type_StorageItemContentProperties)
    {
        return nullptr;
    }

    state->type_StorageItemThumbnail = py::register_python_type(module.get(), type_name_StorageItemThumbnail, &type_spec_StorageItemThumbnail, bases.get(), nullptr);
    if (!state->type_StorageItemThumbnail)
    {
        return nullptr;
    }

    state->type_VideoProperties = py::register_python_type(module.get(), type_name_VideoProperties, &type_spec_VideoProperties, bases.get(), nullptr);
    if (!state->type_VideoProperties)
    {
        return nullptr;
    }

    state->type_IStorageItemExtraProperties = py::register_python_type(module.get(), type_name_IStorageItemExtraProperties, &type_spec_IStorageItemExtraProperties, bases.get(), nullptr);

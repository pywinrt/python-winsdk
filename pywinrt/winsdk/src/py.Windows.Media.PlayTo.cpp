...
namespace py::cpp::Windows::Media::PlayTo
{
    ...
    static PyObject* register_PlayToConnectionError(PyObject* module, PyObject* type) noexcept
    {
    ...
    static PyObject* register_PlayToConnectionState(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Media_PlayTo(void) noexcept
{
    ...
    state->type_CurrentTimeChangeRequestedEventArgs = py::register_python_type(module.get(), type_name_CurrentTimeChangeRequestedEventArgs, &type_spec_CurrentTimeChangeRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_CurrentTimeChangeRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_MuteChangeRequestedEventArgs = py::register_python_type(module.get(), type_name_MuteChangeRequestedEventArgs, &type_spec_MuteChangeRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_MuteChangeRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_PlayToConnection = py::register_python_type(module.get(), type_name_PlayToConnection, &type_spec_PlayToConnection, bases.get(), nullptr);
    if (!state->type_PlayToConnection)
    {
        return nullptr;
    }

    state->type_PlayToConnectionErrorEventArgs = py::register_python_type(module.get(), type_name_PlayToConnectionErrorEventArgs, &type_spec_PlayToConnectionErrorEventArgs, bases.get(), nullptr);
    if (!state->type_PlayToConnectionErrorEventArgs)
    {
        return nullptr;
    }

    state->type_PlayToConnectionStateChangedEventArgs = py::register_python_type(module.get(), type_name_PlayToConnectionStateChangedEventArgs, &type_spec_PlayToConnectionStateChangedEventArgs, bases.get(), nullptr);
    if (!state->type_PlayToConnectionStateChangedEventArgs)
    {
        return nullptr;
    }

    state->type_PlayToConnectionTransferredEventArgs = py::register_python_type(module.get(), type_name_PlayToConnectionTransferredEventArgs, &type_spec_PlayToConnectionTransferredEventArgs, bases.get(), nullptr);
    if (!state->type_PlayToConnectionTransferredEventArgs)
    {
        return nullptr;
    }

    state->type_PlayToManager = py::register_python_type(module.get(), type_name_PlayToManager, &type_spec_PlayToManager, bases.get(), nullptr);
    if (!state->type_PlayToManager)
    {
        return nullptr;
    }

    state->type_PlayToReceiver = py::register_python_type(module.get(), type_name_PlayToReceiver, &type_spec_PlayToReceiver, bases.get(), nullptr);
    if (!state->type_PlayToReceiver)
    {
        return nullptr;
    }

    state->type_PlayToSource = py::register_python_type(module.get(), type_name_PlayToSource, &type_spec_PlayToSource, bases.get(), nullptr);
    if (!state->type_PlayToSource)
    {
        return nullptr;
    }

    state->type_PlayToSourceDeferral = py::register_python_type(module.get(), type_name_PlayToSourceDeferral, &type_spec_PlayToSourceDeferral, bases.get(), nullptr);
    if (!state->type_PlayToSourceDeferral)
    {
        return nullptr;
    }

    state->type_PlayToSourceRequest = py::register_python_type(module.get(), type_name_PlayToSourceRequest, &type_spec_PlayToSourceRequest, bases.get(), nullptr);
    if (!state->type_PlayToSourceRequest)
    {
        return nullptr;
    }

    state->type_PlayToSourceRequestedEventArgs = py::register_python_type(module.get(), type_name_PlayToSourceRequestedEventArgs, &type_spec_PlayToSourceRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_PlayToSourceRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_PlayToSourceSelectedEventArgs = py::register_python_type(module.get(), type_name_PlayToSourceSelectedEventArgs, &type_spec_PlayToSourceSelectedEventArgs, bases.get(), nullptr);
    ...
    state->type_SourceChangeRequestedEventArgs = py::register_python_type(module.get(), type_name_SourceChangeRequestedEventArgs, &type_spec_SourceChangeRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_SourceChangeRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_VolumeChangeRequestedEventArgs = py::register_python_type(module.get(), type_name_VolumeChangeRequestedEventArgs, &type_spec_VolumeChangeRequestedEventArgs, bases.get(), nullptr);

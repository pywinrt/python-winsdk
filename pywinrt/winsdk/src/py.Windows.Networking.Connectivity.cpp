...
namespace py::cpp::Windows::Networking::Connectivity
{
    ...
    static PyObject* register_CellularApnAuthenticationType(PyObject* module, PyObject* type) noexcept
    {
    ...
    static PyObject* register_ConnectionProfileDeleteStatus(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_DataUsageGranularity(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_DomainAuthenticationKind(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_DomainConnectivityLevel(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_NetworkAuthenticationType(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_NetworkConnectivityLevel(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_NetworkCostType(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_NetworkEncryptionType(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_NetworkTypes(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_RoamingStates(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_TriStates(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_WwanDataClass(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_WwanNetworkIPKind(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_WwanNetworkRegistrationState(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Networking_Connectivity(void) noexcept
{
    ...
    state->type_AttributedNetworkUsage = py::register_python_type(module.get(), type_name_AttributedNetworkUsage, &type_spec_AttributedNetworkUsage, bases.get(), nullptr);
    if (!state->type_AttributedNetworkUsage)
    {
        return nullptr;
    }

    state->type_CellularApnContext = py::register_python_type(module.get(), type_name_CellularApnContext, &type_spec_CellularApnContext, bases.get(), nullptr);
    if (!state->type_CellularApnContext)
    {
        return nullptr;
    }

    state->type_ConnectionCost = py::register_python_type(module.get(), type_name_ConnectionCost, &type_spec_ConnectionCost, bases.get(), nullptr);
    if (!state->type_ConnectionCost)
    {
        return nullptr;
    }

    state->type_ConnectionProfile = py::register_python_type(module.get(), type_name_ConnectionProfile, &type_spec_ConnectionProfile, bases.get(), nullptr);
    if (!state->type_ConnectionProfile)
    {
        return nullptr;
    }

    state->type_ConnectionProfileFilter = py::register_python_type(module.get(), type_name_ConnectionProfileFilter, &type_spec_ConnectionProfileFilter, bases.get(), nullptr);
    if (!state->type_ConnectionProfileFilter)
    {
        return nullptr;
    }

    state->type_ConnectionSession = py::register_python_type(module.get(), type_name_ConnectionSession, &type_spec_ConnectionSession, bases.get(), nullptr);
    if (!state->type_ConnectionSession)
    {
        return nullptr;
    }

    state->type_ConnectivityInterval = py::register_python_type(module.get(), type_name_ConnectivityInterval, &type_spec_ConnectivityInterval, bases.get(), nullptr);
    if (!state->type_ConnectivityInterval)
    {
        return nullptr;
    }

    state->type_ConnectivityManager = py::register_python_type(module.get(), type_name_ConnectivityManager, &type_spec_ConnectivityManager, nullptr, nullptr);
    if (!state->type_ConnectivityManager)
    {
        return nullptr;
    }

    state->type_DataPlanStatus = py::register_python_type(module.get(), type_name_DataPlanStatus, &type_spec_DataPlanStatus, bases.get(), nullptr);
    if (!state->type_DataPlanStatus)
    {
        return nullptr;
    }

    state->type_DataPlanUsage = py::register_python_type(module.get(), type_name_DataPlanUsage, &type_spec_DataPlanUsage, bases.get(), nullptr);
    if (!state->type_DataPlanUsage)
    {
        return nullptr;
    }

    state->type_DataUsage = py::register_python_type(module.get(), type_name_DataUsage, &type_spec_DataUsage, bases.get(), nullptr);
    if (!state->type_DataUsage)
    {
        return nullptr;
    }

    state->type_IPInformation = py::register_python_type(module.get(), type_name_IPInformation, &type_spec_IPInformation, bases.get(), nullptr);
    if (!state->type_IPInformation)
    {
        return nullptr;
    }

    state->type_LanIdentifier = py::register_python_type(module.get(), type_name_LanIdentifier, &type_spec_LanIdentifier, bases.get(), nullptr);
    if (!state->type_LanIdentifier)
    {
        return nullptr;
    }

    state->type_LanIdentifierData = py::register_python_type(module.get(), type_name_LanIdentifierData, &type_spec_LanIdentifierData, bases.get(), nullptr);
    if (!state->type_LanIdentifierData)
    {
        return nullptr;
    }

    state->type_NetworkAdapter = py::register_python_type(module.get(), type_name_NetworkAdapter, &type_spec_NetworkAdapter, bases.get(), nullptr);
    if (!state->type_NetworkAdapter)
    {
        return nullptr;
    }

    state->type_NetworkInformation = py::register_python_type(module.get(), type_name_NetworkInformation, &type_spec_NetworkInformation, nullptr, nullptr);
    if (!state->type_NetworkInformation)
    {
        return nullptr;
    }

    state->type_NetworkItem = py::register_python_type(module.get(), type_name_NetworkItem, &type_spec_NetworkItem, bases.get(), nullptr);
    if (!state->type_NetworkItem)
    {
        return nullptr;
    }

    state->type_NetworkSecuritySettings = py::register_python_type(module.get(), type_name_NetworkSecuritySettings, &type_spec_NetworkSecuritySettings, bases.get(), nullptr);
    if (!state->type_NetworkSecuritySettings)
    {
        return nullptr;
    }

    state->type_NetworkStateChangeEventDetails = py::register_python_type(module.get(), type_name_NetworkStateChangeEventDetails, &type_spec_NetworkStateChangeEventDetails, bases.get(), nullptr);
    if (!state->type_NetworkStateChangeEventDetails)
    {
        return nullptr;
    }

    state->type_NetworkUsage = py::register_python_type(module.get(), type_name_NetworkUsage, &type_spec_NetworkUsage, bases.get(), nullptr);
    if (!state->type_NetworkUsage)
    {
        return nullptr;
    }

    state->type_ProviderNetworkUsage = py::register_python_type(module.get(), type_name_ProviderNetworkUsage, &type_spec_ProviderNetworkUsage, bases.get(), nullptr);
    if (!state->type_ProviderNetworkUsage)
    {
        return nullptr;
    }

    state->type_ProxyConfiguration = py::register_python_type(module.get(), type_name_ProxyConfiguration, &type_spec_ProxyConfiguration, bases.get(), nullptr);
    if (!state->type_ProxyConfiguration)
    {
        return nullptr;
    }

    state->type_RoutePolicy = py::register_python_type(module.get(), type_name_RoutePolicy, &type_spec_RoutePolicy, bases.get(), nullptr);
    if (!state->type_RoutePolicy)
    {
        return nullptr;
    }

    state->type_WlanConnectionProfileDetails = py::register_python_type(module.get(), type_name_WlanConnectionProfileDetails, &type_spec_WlanConnectionProfileDetails, bases.get(), nullptr);
    if (!state->type_WlanConnectionProfileDetails)
    {
        return nullptr;
    }

    state->type_WwanConnectionProfileDetails = py::register_python_type(module.get(), type_name_WwanConnectionProfileDetails, &type_spec_WwanConnectionProfileDetails, bases.get(), nullptr);
    if (!state->type_WwanConnectionProfileDetails)
    {
        return nullptr;
    }

    state->type_NetworkUsageStates = py::register_python_type(module.get(), type_name_NetworkUsageStates, &type_spec_NetworkUsageStates, bases.get(), nullptr);

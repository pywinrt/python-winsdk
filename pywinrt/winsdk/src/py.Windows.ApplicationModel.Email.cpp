...
PyMODINIT_FUNC PyInit__winsdk_Windows_ApplicationModel_Email(void) noexcept
{
    ...
    state->type_EmailAttachment = py::register_python_type(module.get(), type_name_EmailAttachment, &type_spec_EmailAttachment, bases.get(), nullptr);
    if (!state->type_EmailAttachment)
    {
        return nullptr;
    }

    state->type_EmailConversation = py::register_python_type(module.get(), type_name_EmailConversation, &type_spec_EmailConversation, bases.get(), nullptr);
    if (!state->type_EmailConversation)
    {
        return nullptr;
    }

    state->type_EmailConversationBatch = py::register_python_type(module.get(), type_name_EmailConversationBatch, &type_spec_EmailConversationBatch, bases.get(), nullptr);
    if (!state->type_EmailConversationBatch)
    {
        return nullptr;
    }

    state->type_EmailConversationReader = py::register_python_type(module.get(), type_name_EmailConversationReader, &type_spec_EmailConversationReader, bases.get(), nullptr);
    if (!state->type_EmailConversationReader)
    {
        return nullptr;
    }

    state->type_EmailFolder = py::register_python_type(module.get(), type_name_EmailFolder, &type_spec_EmailFolder, bases.get(), nullptr);
    if (!state->type_EmailFolder)
    {
        return nullptr;
    }

    state->type_EmailIrmInfo = py::register_python_type(module.get(), type_name_EmailIrmInfo, &type_spec_EmailIrmInfo, bases.get(), nullptr);
    if (!state->type_EmailIrmInfo)
    {
        return nullptr;
    }

    state->type_EmailIrmTemplate = py::register_python_type(module.get(), type_name_EmailIrmTemplate, &type_spec_EmailIrmTemplate, bases.get(), nullptr);
    if (!state->type_EmailIrmTemplate)
    {
        return nullptr;
    }

    state->type_EmailItemCounts = py::register_python_type(module.get(), type_name_EmailItemCounts, &type_spec_EmailItemCounts, bases.get(), nullptr);
    if (!state->type_EmailItemCounts)
    {
        return nullptr;
    }

    state->type_EmailMailbox = py::register_python_type(module.get(), type_name_EmailMailbox, &type_spec_EmailMailbox, bases.get(), nullptr);
    if (!state->type_EmailMailbox)
    {
        return nullptr;
    }

    state->type_EmailMailboxAction = py::register_python_type(module.get(), type_name_EmailMailboxAction, &type_spec_EmailMailboxAction, bases.get(), nullptr);
    if (!state->type_EmailMailboxAction)
    {
        return nullptr;
    }

    state->type_EmailMailboxAutoReply = py::register_python_type(module.get(), type_name_EmailMailboxAutoReply, &type_spec_EmailMailboxAutoReply, bases.get(), nullptr);
    if (!state->type_EmailMailboxAutoReply)
    {
        return nullptr;
    }

    state->type_EmailMailboxAutoReplySettings = py::register_python_type(module.get(), type_name_EmailMailboxAutoReplySettings, &type_spec_EmailMailboxAutoReplySettings, bases.get(), nullptr);
    if (!state->type_EmailMailboxAutoReplySettings)
    {
        return nullptr;
    }

    state->type_EmailMailboxCapabilities = py::register_python_type(module.get(), type_name_EmailMailboxCapabilities, &type_spec_EmailMailboxCapabilities, bases.get(), nullptr);
    if (!state->type_EmailMailboxCapabilities)
    {
        return nullptr;
    }

    state->type_EmailMailboxChange = py::register_python_type(module.get(), type_name_EmailMailboxChange, &type_spec_EmailMailboxChange, bases.get(), nullptr);
    if (!state->type_EmailMailboxChange)
    {
        return nullptr;
    }

    state->type_EmailMailboxChangeReader = py::register_python_type(module.get(), type_name_EmailMailboxChangeReader, &type_spec_EmailMailboxChangeReader, bases.get(), nullptr);
    if (!state->type_EmailMailboxChangeReader)
    {
        return nullptr;
    }

    state->type_EmailMailboxChangeTracker = py::register_python_type(module.get(), type_name_EmailMailboxChangeTracker, &type_spec_EmailMailboxChangeTracker, bases.get(), nullptr);
    if (!state->type_EmailMailboxChangeTracker)
    {
        return nullptr;
    }

    state->type_EmailMailboxChangedDeferral = py::register_python_type(module.get(), type_name_EmailMailboxChangedDeferral, &type_spec_EmailMailboxChangedDeferral, bases.get(), nullptr);
    if (!state->type_EmailMailboxChangedDeferral)
    {
        return nullptr;
    }

    state->type_EmailMailboxChangedEventArgs = py::register_python_type(module.get(), type_name_EmailMailboxChangedEventArgs, &type_spec_EmailMailboxChangedEventArgs, bases.get(), nullptr);
    if (!state->type_EmailMailboxChangedEventArgs)
    {
        return nullptr;
    }

    state->type_EmailMailboxCreateFolderResult = py::register_python_type(module.get(), type_name_EmailMailboxCreateFolderResult, &type_spec_EmailMailboxCreateFolderResult, bases.get(), nullptr);
    if (!state->type_EmailMailboxCreateFolderResult)
    {
        return nullptr;
    }

    state->type_EmailMailboxPolicies = py::register_python_type(module.get(), type_name_EmailMailboxPolicies, &type_spec_EmailMailboxPolicies, bases.get(), nullptr);
    if (!state->type_EmailMailboxPolicies)
    {
        return nullptr;
    }

    state->type_EmailMailboxSyncManager = py::register_python_type(module.get(), type_name_EmailMailboxSyncManager, &type_spec_EmailMailboxSyncManager, bases.get(), nullptr);
    if (!state->type_EmailMailboxSyncManager)
    {
        return nullptr;
    }

    state->type_EmailManager = py::register_python_type(module.get(), type_name_EmailManager, &type_spec_EmailManager, nullptr, nullptr);
    if (!state->type_EmailManager)
    {
        return nullptr;
    }

    state->type_EmailManagerForUser = py::register_python_type(module.get(), type_name_EmailManagerForUser, &type_spec_EmailManagerForUser, bases.get(), nullptr);
    if (!state->type_EmailManagerForUser)
    {
        return nullptr;
    }

    state->type_EmailMeetingInfo = py::register_python_type(module.get(), type_name_EmailMeetingInfo, &type_spec_EmailMeetingInfo, bases.get(), nullptr);
    if (!state->type_EmailMeetingInfo)
    {
        return nullptr;
    }

    state->type_EmailMessage = py::register_python_type(module.get(), type_name_EmailMessage, &type_spec_EmailMessage, bases.get(), nullptr);
    if (!state->type_EmailMessage)
    {
        return nullptr;
    }

    state->type_EmailMessageBatch = py::register_python_type(module.get(), type_name_EmailMessageBatch, &type_spec_EmailMessageBatch, bases.get(), nullptr);
    if (!state->type_EmailMessageBatch)
    {
        return nullptr;
    }

    state->type_EmailMessageReader = py::register_python_type(module.get(), type_name_EmailMessageReader, &type_spec_EmailMessageReader, bases.get(), nullptr);
    if (!state->type_EmailMessageReader)
    {
        return nullptr;
    }

    state->type_EmailQueryOptions = py::register_python_type(module.get(), type_name_EmailQueryOptions, &type_spec_EmailQueryOptions, bases.get(), nullptr);
    if (!state->type_EmailQueryOptions)
    {
        return nullptr;
    }

    state->type_EmailQueryTextSearch = py::register_python_type(module.get(), type_name_EmailQueryTextSearch, &type_spec_EmailQueryTextSearch, bases.get(), nullptr);
    if (!state->type_EmailQueryTextSearch)
    {
        return nullptr;
    }

    state->type_EmailRecipient = py::register_python_type(module.get(), type_name_EmailRecipient, &type_spec_EmailRecipient, bases.get(), nullptr);
    if (!state->type_EmailRecipient)
    {
        return nullptr;
    }

    state->type_EmailRecipientResolutionResult = py::register_python_type(module.get(), type_name_EmailRecipientResolutionResult, &type_spec_EmailRecipientResolutionResult, bases.get(), nullptr);
    if (!state->type_EmailRecipientResolutionResult)
    {
        return nullptr;
    }

    state->type_EmailStore = py::register_python_type(module.get(), type_name_EmailStore, &type_spec_EmailStore, bases.get(), nullptr);
    if (!state->type_EmailStore)
    {
        return nullptr;
    }

    state->type_EmailStoreNotificationTriggerDetails = py::register_python_type(module.get(), type_name_EmailStoreNotificationTriggerDetails, &type_spec_EmailStoreNotificationTriggerDetails, bases.get(), nullptr);

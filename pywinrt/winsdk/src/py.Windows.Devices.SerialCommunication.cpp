...
PyMODINIT_FUNC PyInit__winsdk_Windows_Devices_SerialCommunication(void) noexcept
{
    ...
    state->type_ErrorReceivedEventArgs = py::register_python_type(module.get(), type_name_ErrorReceivedEventArgs, &type_spec_ErrorReceivedEventArgs, bases.get(), nullptr);
    if (!state->type_ErrorReceivedEventArgs)
    {
        return nullptr;
    }

    state->type_PinChangedEventArgs = py::register_python_type(module.get(), type_name_PinChangedEventArgs, &type_spec_PinChangedEventArgs, bases.get(), nullptr);
    if (!state->type_PinChangedEventArgs)
    {
        return nullptr;
    }

    state->type_SerialDevice = py::register_python_type(module.get(), type_name_SerialDevice, &type_spec_SerialDevice, bases.get(), nullptr);

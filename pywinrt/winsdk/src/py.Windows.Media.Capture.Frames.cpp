...
namespace py::cpp::Windows::Media::Capture::Frames
{
    ...
    static PyObject* register_MediaFrameReaderAcquisitionMode(PyObject* module, PyObject* type)
    {
    ...
    static PyObject* register_MediaFrameReaderStartStatus(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_MediaFrameSourceGetPropertyStatus(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_MediaFrameSourceKind(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_MediaFrameSourceSetPropertyStatus(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_MultiSourceMediaFrameReaderStartStatus(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    constexpr const char* const type_name_AudioMediaFrame = "AudioMediaFrame";
    ...
    constexpr const char* const type_name_BufferMediaFrame = "BufferMediaFrame";
    ...
    constexpr const char* const type_name_DepthMediaFrame = "DepthMediaFrame";
    ...
    constexpr const char* const type_name_DepthMediaFrameFormat = "DepthMediaFrameFormat";
    ...
    constexpr const char* const type_name_InfraredMediaFrame = "InfraredMediaFrame";
    ...
    constexpr const char* const type_name_MediaFrameArrivedEventArgs = "MediaFrameArrivedEventArgs";
    ...
    constexpr const char* const type_name_MediaFrameFormat = "MediaFrameFormat";
    ...
    constexpr const char* const type_name_MediaFrameReader = "MediaFrameReader";
    ...
    constexpr const char* const type_name_MediaFrameReference = "MediaFrameReference";
    ...
    constexpr const char* const type_name_MediaFrameSource = "MediaFrameSource";
    ...
    constexpr const char* const type_name_MediaFrameSourceController = "MediaFrameSourceController";
    ...
    constexpr const char* const type_name_MediaFrameSourceGetPropertyResult = "MediaFrameSourceGetPropertyResult";
    ...
    constexpr const char* const type_name_MediaFrameSourceGroup = "MediaFrameSourceGroup";
    ...
    constexpr const char* const type_name_MediaFrameSourceInfo = "MediaFrameSourceInfo";
    ...
    constexpr const char* const type_name_MultiSourceMediaFrameArrivedEventArgs = "MultiSourceMediaFrameArrivedEventArgs";
    ...
    constexpr const char* const type_name_MultiSourceMediaFrameReader = "MultiSourceMediaFrameReader";
    ...
    constexpr const char* const type_name_MultiSourceMediaFrameReference = "MultiSourceMediaFrameReference";
    ...
    constexpr const char* const type_name_VideoMediaFrame = "VideoMediaFrame";
    ...
    constexpr const char* const type_name_VideoMediaFrameFormat = "VideoMediaFrameFormat";
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Media_Capture_Frames(void) noexcept
{
    ...
    state->type_AudioMediaFrame = py::register_python_type(module.get(), type_name_AudioMediaFrame, &type_spec_AudioMediaFrame, bases.get());
    if (!state->type_AudioMediaFrame)
    {
        return nullptr;
    }

    Py_INCREF(state->type_AudioMediaFrame);

    state->type_BufferMediaFrame = py::register_python_type(module.get(), type_name_BufferMediaFrame, &type_spec_BufferMediaFrame, bases.get());
    if (!state->type_BufferMediaFrame)
    {
        return nullptr;
    }

    Py_INCREF(state->type_BufferMediaFrame);

    state->type_DepthMediaFrame = py::register_python_type(module.get(), type_name_DepthMediaFrame, &type_spec_DepthMediaFrame, bases.get());
    if (!state->type_DepthMediaFrame)
    {
        return nullptr;
    }

    Py_INCREF(state->type_DepthMediaFrame);

    state->type_DepthMediaFrameFormat = py::register_python_type(module.get(), type_name_DepthMediaFrameFormat, &type_spec_DepthMediaFrameFormat, bases.get());
    if (!state->type_DepthMediaFrameFormat)
    {
        return nullptr;
    }

    Py_INCREF(state->type_DepthMediaFrameFormat);

    state->type_InfraredMediaFrame = py::register_python_type(module.get(), type_name_InfraredMediaFrame, &type_spec_InfraredMediaFrame, bases.get());
    if (!state->type_InfraredMediaFrame)
    {
        return nullptr;
    }

    Py_INCREF(state->type_InfraredMediaFrame);

    state->type_MediaFrameArrivedEventArgs = py::register_python_type(module.get(), type_name_MediaFrameArrivedEventArgs, &type_spec_MediaFrameArrivedEventArgs, bases.get());
    if (!state->type_MediaFrameArrivedEventArgs)
    {
        return nullptr;
    }

    Py_INCREF(state->type_MediaFrameArrivedEventArgs);

    state->type_MediaFrameFormat = py::register_python_type(module.get(), type_name_MediaFrameFormat, &type_spec_MediaFrameFormat, bases.get());
    if (!state->type_MediaFrameFormat)
    {
        return nullptr;
    }

    Py_INCREF(state->type_MediaFrameFormat);

    state->type_MediaFrameReader = py::register_python_type(module.get(), type_name_MediaFrameReader, &type_spec_MediaFrameReader, bases.get());
    if (!state->type_MediaFrameReader)
    {
        return nullptr;
    }

    Py_INCREF(state->type_MediaFrameReader);

    state->type_MediaFrameReference = py::register_python_type(module.get(), type_name_MediaFrameReference, &type_spec_MediaFrameReference, bases.get());
    if (!state->type_MediaFrameReference)
    {
        return nullptr;
    }

    Py_INCREF(state->type_MediaFrameReference);

    state->type_MediaFrameSource = py::register_python_type(module.get(), type_name_MediaFrameSource, &type_spec_MediaFrameSource, bases.get());
    if (!state->type_MediaFrameSource)
    {
        return nullptr;
    }

    Py_INCREF(state->type_MediaFrameSource);

    state->type_MediaFrameSourceController = py::register_python_type(module.get(), type_name_MediaFrameSourceController, &type_spec_MediaFrameSourceController, bases.get());
    if (!state->type_MediaFrameSourceController)
    {
        return nullptr;
    }

    Py_INCREF(state->type_MediaFrameSourceController);

    state->type_MediaFrameSourceGetPropertyResult = py::register_python_type(module.get(), type_name_MediaFrameSourceGetPropertyResult, &type_spec_MediaFrameSourceGetPropertyResult, bases.get());
    if (!state->type_MediaFrameSourceGetPropertyResult)
    {
        return nullptr;
    }

    Py_INCREF(state->type_MediaFrameSourceGetPropertyResult);

    state->type_MediaFrameSourceGroup = py::register_python_type(module.get(), type_name_MediaFrameSourceGroup, &type_spec_MediaFrameSourceGroup, bases.get());
    if (!state->type_MediaFrameSourceGroup)
    {
        return nullptr;
    }

    Py_INCREF(state->type_MediaFrameSourceGroup);

    state->type_MediaFrameSourceInfo = py::register_python_type(module.get(), type_name_MediaFrameSourceInfo, &type_spec_MediaFrameSourceInfo, bases.get());
    if (!state->type_MediaFrameSourceInfo)
    {
        return nullptr;
    }

    Py_INCREF(state->type_MediaFrameSourceInfo);

    state->type_MultiSourceMediaFrameArrivedEventArgs = py::register_python_type(module.get(), type_name_MultiSourceMediaFrameArrivedEventArgs, &type_spec_MultiSourceMediaFrameArrivedEventArgs, bases.get());
    if (!state->type_MultiSourceMediaFrameArrivedEventArgs)
    {
        return nullptr;
    }

    Py_INCREF(state->type_MultiSourceMediaFrameArrivedEventArgs);

    state->type_MultiSourceMediaFrameReader = py::register_python_type(module.get(), type_name_MultiSourceMediaFrameReader, &type_spec_MultiSourceMediaFrameReader, bases.get());
    if (!state->type_MultiSourceMediaFrameReader)
    {
        return nullptr;
    }

    Py_INCREF(state->type_MultiSourceMediaFrameReader);

    state->type_MultiSourceMediaFrameReference = py::register_python_type(module.get(), type_name_MultiSourceMediaFrameReference, &type_spec_MultiSourceMediaFrameReference, bases.get());
    if (!state->type_MultiSourceMediaFrameReference)
    {
        return nullptr;
    }

    Py_INCREF(state->type_MultiSourceMediaFrameReference);

    state->type_VideoMediaFrame = py::register_python_type(module.get(), type_name_VideoMediaFrame, &type_spec_VideoMediaFrame, bases.get());
    if (!state->type_VideoMediaFrame)
    {
        return nullptr;
    }

    Py_INCREF(state->type_VideoMediaFrame);

    state->type_VideoMediaFrameFormat = py::register_python_type(module.get(), type_name_VideoMediaFrameFormat, &type_spec_VideoMediaFrameFormat, bases.get());
    if (!state->type_VideoMediaFrameFormat)
    {
        return nullptr;
    }

    Py_INCREF(state->type_VideoMediaFrameFormat);

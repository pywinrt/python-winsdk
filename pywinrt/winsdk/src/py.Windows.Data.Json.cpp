...
namespace py::cpp::Windows::Data::Json
{
    ...
    static PyType_Slot _type_slots_JsonArray[] = 
    {
        ...
        { Py_mp_subscript, _seq_subscript_JsonArray },
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Data_Json(void) noexcept
{
    ...
    state->type_JsonArray = py::register_python_type(module.get(), type_name_JsonArray, &type_spec_JsonArray, mutable_sequence_bases.get(), nullptr);
    if (!state->type_JsonArray)
    {
        return nullptr;
    }

    state->type_JsonError = py::register_python_type(module.get(), type_name_JsonError, &type_spec_JsonError, nullptr, nullptr);
    if (!state->type_JsonError)
    {
        return nullptr;
    }

    state->type_JsonObject = py::register_python_type(module.get(), type_name_JsonObject, &type_spec_JsonObject, mutable_mapping_bases.get(), nullptr);
    if (!state->type_JsonObject)
    {
        return nullptr;
    }

    state->type_JsonValue = py::register_python_type(module.get(), type_name_JsonValue, &type_spec_JsonValue, bases.get(), nullptr);
    if (!state->type_JsonValue)
    {
        return nullptr;
    }

    state->type_IJsonValue = py::register_python_type(module.get(), type_name_IJsonValue, &type_spec_IJsonValue, bases.get(), nullptr);

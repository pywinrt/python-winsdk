...
namespace py::cpp::Windows::ApplicationModel::DataTransfer
{
    ...
    static PyType_Slot type_slots_StandardDataFormats_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_StandardDataFormats_Meta },
        { }
    };

    static PyType_Spec type_spec_StandardDataFormats_Meta =
    {
        "_winsdk_Windows_ApplicationModel_DataTransfer.StandardDataFormats_Meta",
...
PyMODINIT_FUNC PyInit__winsdk_Windows_ApplicationModel_DataTransfer(void) noexcept
{
    ...
    state->type_Clipboard = py::register_python_type(module.get(), type_name_Clipboard, &type_spec_Clipboard, nullptr, nullptr);
    if (!state->type_Clipboard)
    {
        return nullptr;
    }

    state->type_ClipboardContentOptions = py::register_python_type(module.get(), type_name_ClipboardContentOptions, &type_spec_ClipboardContentOptions, bases.get(), nullptr);
    if (!state->type_ClipboardContentOptions)
    {
        return nullptr;
    }

    state->type_ClipboardHistoryChangedEventArgs = py::register_python_type(module.get(), type_name_ClipboardHistoryChangedEventArgs, &type_spec_ClipboardHistoryChangedEventArgs, bases.get(), nullptr);
    if (!state->type_ClipboardHistoryChangedEventArgs)
    {
        return nullptr;
    }

    state->type_ClipboardHistoryItem = py::register_python_type(module.get(), type_name_ClipboardHistoryItem, &type_spec_ClipboardHistoryItem, bases.get(), nullptr);
    if (!state->type_ClipboardHistoryItem)
    {
        return nullptr;
    }

    state->type_ClipboardHistoryItemsResult = py::register_python_type(module.get(), type_name_ClipboardHistoryItemsResult, &type_spec_ClipboardHistoryItemsResult, bases.get(), nullptr);
    if (!state->type_ClipboardHistoryItemsResult)
    {
        return nullptr;
    }

    state->type_DataPackage = py::register_python_type(module.get(), type_name_DataPackage, &type_spec_DataPackage, bases.get(), nullptr);
    if (!state->type_DataPackage)
    {
        return nullptr;
    }

    state->type_DataPackagePropertySet = py::register_python_type(module.get(), type_name_DataPackagePropertySet, &type_spec_DataPackagePropertySet, mutable_mapping_bases.get(), nullptr);
    if (!state->type_DataPackagePropertySet)
    {
        return nullptr;
    }

    state->type_DataPackagePropertySetView = py::register_python_type(module.get(), type_name_DataPackagePropertySetView, &type_spec_DataPackagePropertySetView, mapping_bases.get(), nullptr);
    if (!state->type_DataPackagePropertySetView)
    {
        return nullptr;
    }

    state->type_DataPackageView = py::register_python_type(module.get(), type_name_DataPackageView, &type_spec_DataPackageView, bases.get(), nullptr);
    if (!state->type_DataPackageView)
    {
        return nullptr;
    }

    state->type_DataProviderDeferral = py::register_python_type(module.get(), type_name_DataProviderDeferral, &type_spec_DataProviderDeferral, bases.get(), nullptr);
    if (!state->type_DataProviderDeferral)
    {
        return nullptr;
    }

    state->type_DataProviderRequest = py::register_python_type(module.get(), type_name_DataProviderRequest, &type_spec_DataProviderRequest, bases.get(), nullptr);
    if (!state->type_DataProviderRequest)
    {
        return nullptr;
    }

    state->type_DataRequest = py::register_python_type(module.get(), type_name_DataRequest, &type_spec_DataRequest, bases.get(), nullptr);
    if (!state->type_DataRequest)
    {
        return nullptr;
    }

    state->type_DataRequestDeferral = py::register_python_type(module.get(), type_name_DataRequestDeferral, &type_spec_DataRequestDeferral, bases.get(), nullptr);
    if (!state->type_DataRequestDeferral)
    {
        return nullptr;
    }

    state->type_DataRequestedEventArgs = py::register_python_type(module.get(), type_name_DataRequestedEventArgs, &type_spec_DataRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_DataRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_DataTransferManager = py::register_python_type(module.get(), type_name_DataTransferManager, &type_spec_DataTransferManager, bases.get(), nullptr);
    if (!state->type_DataTransferManager)
    {
        return nullptr;
    }

    state->type_HtmlFormatHelper = py::register_python_type(module.get(), type_name_HtmlFormatHelper, &type_spec_HtmlFormatHelper, nullptr, nullptr);
    if (!state->type_HtmlFormatHelper)
    {
        return nullptr;
    }

    state->type_OperationCompletedEventArgs = py::register_python_type(module.get(), type_name_OperationCompletedEventArgs, &type_spec_OperationCompletedEventArgs, bases.get(), nullptr);
    if (!state->type_OperationCompletedEventArgs)
    {
        return nullptr;
    }

    state->type_ShareCompletedEventArgs = py::register_python_type(module.get(), type_name_ShareCompletedEventArgs, &type_spec_ShareCompletedEventArgs, bases.get(), nullptr);
    if (!state->type_ShareCompletedEventArgs)
    {
        return nullptr;
    }

    state->type_ShareProvider = py::register_python_type(module.get(), type_name_ShareProvider, &type_spec_ShareProvider, bases.get(), nullptr);
    if (!state->type_ShareProvider)
    {
        return nullptr;
    }

    state->type_ShareProviderOperation = py::register_python_type(module.get(), type_name_ShareProviderOperation, &type_spec_ShareProviderOperation, bases.get(), nullptr);
    if (!state->type_ShareProviderOperation)
    {
        return nullptr;
    }

    state->type_ShareProvidersRequestedEventArgs = py::register_python_type(module.get(), type_name_ShareProvidersRequestedEventArgs, &type_spec_ShareProvidersRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_ShareProvidersRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_ShareTargetInfo = py::register_python_type(module.get(), type_name_ShareTargetInfo, &type_spec_ShareTargetInfo, bases.get(), nullptr);
    if (!state->type_ShareTargetInfo)
    {
        return nullptr;
    }

    state->type_ShareUIOptions = py::register_python_type(module.get(), type_name_ShareUIOptions, &type_spec_ShareUIOptions, bases.get(), nullptr);
    if (!state->type_ShareUIOptions)
    {
        return nullptr;
    }

    state->type_SharedStorageAccessManager = py::register_python_type(module.get(), type_name_SharedStorageAccessManager, &type_spec_SharedStorageAccessManager, nullptr, nullptr);
    ...
    state->type_StandardDataFormats = py::register_python_type(module.get(), type_name_StandardDataFormats, &type_spec_StandardDataFormats, nullptr, reinterpret_cast<PyTypeObject*>(type_StandardDataFormats_Meta.get()));
    if (!state->type_StandardDataFormats)
    {
        return nullptr;
    }

    state->type_TargetApplicationChosenEventArgs = py::register_python_type(module.get(), type_name_TargetApplicationChosenEventArgs, &type_spec_TargetApplicationChosenEventArgs, bases.get(), nullptr);

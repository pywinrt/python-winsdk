...
namespace py::cpp::Windows::ApplicationModel::SocialInfo::Provider
{
    ...
    constexpr const char* const type_name_SocialDashboardItemUpdater = "SocialDashboardItemUpdater";
    ...
    constexpr const char* const type_name_SocialFeedUpdater = "SocialFeedUpdater";
    ...
    constexpr const char* const type_name_SocialInfoProviderManager = "SocialInfoProviderManager";
...
PyMODINIT_FUNC PyInit__winsdk_Windows_ApplicationModel_SocialInfo_Provider(void) noexcept
{
    ...
    state->type_SocialDashboardItemUpdater = py::register_python_type(module.get(), type_name_SocialDashboardItemUpdater, &type_spec_SocialDashboardItemUpdater, bases.get());
    if (!state->type_SocialDashboardItemUpdater)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SocialDashboardItemUpdater);

    state->type_SocialFeedUpdater = py::register_python_type(module.get(), type_name_SocialFeedUpdater, &type_spec_SocialFeedUpdater, bases.get());
    if (!state->type_SocialFeedUpdater)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SocialFeedUpdater);

    state->type_SocialInfoProviderManager = py::register_python_type(module.get(), type_name_SocialInfoProviderManager, &type_spec_SocialInfoProviderManager, nullptr);
    if (!state->type_SocialInfoProviderManager)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SocialInfoProviderManager);

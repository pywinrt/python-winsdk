...
PyMODINIT_FUNC PyInit__winsdk_Windows_ApplicationModel_UserActivities(void) noexcept
{
    ...
    state->type_UserActivity = py::register_python_type(module.get(), type_name_UserActivity, &type_spec_UserActivity, bases.get(), nullptr);
    if (!state->type_UserActivity)
    {
        return nullptr;
    }

    state->type_UserActivityAttribution = py::register_python_type(module.get(), type_name_UserActivityAttribution, &type_spec_UserActivityAttribution, bases.get(), nullptr);
    if (!state->type_UserActivityAttribution)
    {
        return nullptr;
    }

    state->type_UserActivityChannel = py::register_python_type(module.get(), type_name_UserActivityChannel, &type_spec_UserActivityChannel, bases.get(), nullptr);
    if (!state->type_UserActivityChannel)
    {
        return nullptr;
    }

    state->type_UserActivityContentInfo = py::register_python_type(module.get(), type_name_UserActivityContentInfo, &type_spec_UserActivityContentInfo, bases.get(), nullptr);
    if (!state->type_UserActivityContentInfo)
    {
        return nullptr;
    }

    state->type_UserActivityRequest = py::register_python_type(module.get(), type_name_UserActivityRequest, &type_spec_UserActivityRequest, bases.get(), nullptr);
    if (!state->type_UserActivityRequest)
    {
        return nullptr;
    }

    state->type_UserActivityRequestManager = py::register_python_type(module.get(), type_name_UserActivityRequestManager, &type_spec_UserActivityRequestManager, bases.get(), nullptr);
    if (!state->type_UserActivityRequestManager)
    {
        return nullptr;
    }

    state->type_UserActivityRequestedEventArgs = py::register_python_type(module.get(), type_name_UserActivityRequestedEventArgs, &type_spec_UserActivityRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_UserActivityRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_UserActivitySession = py::register_python_type(module.get(), type_name_UserActivitySession, &type_spec_UserActivitySession, bases.get(), nullptr);
    if (!state->type_UserActivitySession)
    {
        return nullptr;
    }

    state->type_UserActivitySessionHistoryItem = py::register_python_type(module.get(), type_name_UserActivitySessionHistoryItem, &type_spec_UserActivitySessionHistoryItem, bases.get(), nullptr);
    if (!state->type_UserActivitySessionHistoryItem)
    {
        return nullptr;
    }

    state->type_UserActivityVisualElements = py::register_python_type(module.get(), type_name_UserActivityVisualElements, &type_spec_UserActivityVisualElements, bases.get(), nullptr);
    if (!state->type_UserActivityVisualElements)
    {
        return nullptr;
    }

    state->type_IUserActivityContentInfo = py::register_python_type(module.get(), type_name_IUserActivityContentInfo, &type_spec_IUserActivityContentInfo, bases.get(), nullptr);

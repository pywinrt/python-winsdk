...
PyMODINIT_FUNC PyInit__winsdk_Windows_Media_Import(void) noexcept
{
    ...
    state->type_PhotoImportDeleteImportedItemsFromSourceResult = py::register_python_type(module.get(), type_name_PhotoImportDeleteImportedItemsFromSourceResult, &type_spec_PhotoImportDeleteImportedItemsFromSourceResult, bases.get(), nullptr);
    if (!state->type_PhotoImportDeleteImportedItemsFromSourceResult)
    {
        return nullptr;
    }

    state->type_PhotoImportFindItemsResult = py::register_python_type(module.get(), type_name_PhotoImportFindItemsResult, &type_spec_PhotoImportFindItemsResult, bases.get(), nullptr);
    if (!state->type_PhotoImportFindItemsResult)
    {
        return nullptr;
    }

    state->type_PhotoImportImportItemsResult = py::register_python_type(module.get(), type_name_PhotoImportImportItemsResult, &type_spec_PhotoImportImportItemsResult, bases.get(), nullptr);
    if (!state->type_PhotoImportImportItemsResult)
    {
        return nullptr;
    }

    state->type_PhotoImportItem = py::register_python_type(module.get(), type_name_PhotoImportItem, &type_spec_PhotoImportItem, bases.get(), nullptr);
    if (!state->type_PhotoImportItem)
    {
        return nullptr;
    }

    state->type_PhotoImportItemImportedEventArgs = py::register_python_type(module.get(), type_name_PhotoImportItemImportedEventArgs, &type_spec_PhotoImportItemImportedEventArgs, bases.get(), nullptr);
    if (!state->type_PhotoImportItemImportedEventArgs)
    {
        return nullptr;
    }

    state->type_PhotoImportManager = py::register_python_type(module.get(), type_name_PhotoImportManager, &type_spec_PhotoImportManager, nullptr, nullptr);
    if (!state->type_PhotoImportManager)
    {
        return nullptr;
    }

    state->type_PhotoImportOperation = py::register_python_type(module.get(), type_name_PhotoImportOperation, &type_spec_PhotoImportOperation, bases.get(), nullptr);
    if (!state->type_PhotoImportOperation)
    {
        return nullptr;
    }

    state->type_PhotoImportSelectionChangedEventArgs = py::register_python_type(module.get(), type_name_PhotoImportSelectionChangedEventArgs, &type_spec_PhotoImportSelectionChangedEventArgs, bases.get(), nullptr);
    if (!state->type_PhotoImportSelectionChangedEventArgs)
    {
        return nullptr;
    }

    state->type_PhotoImportSession = py::register_python_type(module.get(), type_name_PhotoImportSession, &type_spec_PhotoImportSession, bases.get(), nullptr);
    if (!state->type_PhotoImportSession)
    {
        return nullptr;
    }

    state->type_PhotoImportSidecar = py::register_python_type(module.get(), type_name_PhotoImportSidecar, &type_spec_PhotoImportSidecar, bases.get(), nullptr);
    if (!state->type_PhotoImportSidecar)
    {
        return nullptr;
    }

    state->type_PhotoImportSource = py::register_python_type(module.get(), type_name_PhotoImportSource, &type_spec_PhotoImportSource, bases.get(), nullptr);
    if (!state->type_PhotoImportSource)
    {
        return nullptr;
    }

    state->type_PhotoImportStorageMedium = py::register_python_type(module.get(), type_name_PhotoImportStorageMedium, &type_spec_PhotoImportStorageMedium, bases.get(), nullptr);
    if (!state->type_PhotoImportStorageMedium)
    {
        return nullptr;
    }

    state->type_PhotoImportVideoSegment = py::register_python_type(module.get(), type_name_PhotoImportVideoSegment, &type_spec_PhotoImportVideoSegment, bases.get(), nullptr);
    if (!state->type_PhotoImportVideoSegment)
    {
        return nullptr;
    }

    state->type_PhotoImportProgress = py::register_python_type(module.get(), type_name_PhotoImportProgress, &type_spec_PhotoImportProgress, bases.get(), nullptr);

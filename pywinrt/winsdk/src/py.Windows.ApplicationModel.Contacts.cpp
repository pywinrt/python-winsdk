...
namespace py::cpp::Windows::ApplicationModel::Contacts
{
    ...
    static PyType_Slot type_slots_ContactLaunchActionVerbs_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_ContactLaunchActionVerbs_Meta },
        { }
    };

    static PyType_Spec type_spec_ContactLaunchActionVerbs_Meta =
    {
        "_winsdk_Windows_ApplicationModel_Contacts.ContactLaunchActionVerbs_Meta",
    ...
    {
        { Py_tp_base, &PyType_Type },
        { Py_tp_getset, getset_ContactManager_Meta },
        { }
    };

    static PyType_Spec type_spec_ContactManager_Meta =
    {
        "_winsdk_Windows_ApplicationModel_Contacts.ContactManager_Meta",
        static_cast<int>(PyType_Type.tp_basicsize),
        static_cast<int>(PyType_Type.tp_itemsize),
        Py_TPFLAGS_DEFAULT,
    ...
    {
        { Py_tp_base, &PyType_Type },
        { Py_tp_getset, getset_KnownContactField_Meta },
        { }
    };

    static PyType_Spec type_spec_KnownContactField_Meta =
    {
        "_winsdk_Windows_ApplicationModel_Contacts.KnownContactField_Meta",
        static_cast<int>(PyType_Type.tp_basicsize),
        static_cast<int>(PyType_Type.tp_itemsize),
        Py_TPFLAGS_DEFAULT,
...
PyMODINIT_FUNC PyInit__winsdk_Windows_ApplicationModel_Contacts(void) noexcept
{
    ...
    state->type_AggregateContactManager = py::register_python_type(module.get(), type_name_AggregateContactManager, &type_spec_AggregateContactManager, bases.get(), nullptr);
    if (!state->type_AggregateContactManager)
    {
        return nullptr;
    }

    state->type_Contact = py::register_python_type(module.get(), type_name_Contact, &type_spec_Contact, bases.get(), nullptr);
    ...
    state->type_ContactAnnotation = py::register_python_type(module.get(), type_name_ContactAnnotation, &type_spec_ContactAnnotation, bases.get(), nullptr);
    if (!state->type_ContactAnnotation)
    {
        return nullptr;
    }

    state->type_ContactAnnotationList = py::register_python_type(module.get(), type_name_ContactAnnotationList, &type_spec_ContactAnnotationList, bases.get(), nullptr);
    if (!state->type_ContactAnnotationList)
    {
        return nullptr;
    }

    state->type_ContactAnnotationStore = py::register_python_type(module.get(), type_name_ContactAnnotationStore, &type_spec_ContactAnnotationStore, bases.get(), nullptr);
    if (!state->type_ContactAnnotationStore)
    {
        return nullptr;
    }

    state->type_ContactBatch = py::register_python_type(module.get(), type_name_ContactBatch, &type_spec_ContactBatch, bases.get(), nullptr);
    if (!state->type_ContactBatch)
    {
        return nullptr;
    }

    state->type_ContactCardDelayedDataLoader = py::register_python_type(module.get(), type_name_ContactCardDelayedDataLoader, &type_spec_ContactCardDelayedDataLoader, bases.get(), nullptr);
    if (!state->type_ContactCardDelayedDataLoader)
    {
        return nullptr;
    }

    state->type_ContactCardOptions = py::register_python_type(module.get(), type_name_ContactCardOptions, &type_spec_ContactCardOptions, bases.get(), nullptr);
    if (!state->type_ContactCardOptions)
    {
        return nullptr;
    }

    state->type_ContactChange = py::register_python_type(module.get(), type_name_ContactChange, &type_spec_ContactChange, bases.get(), nullptr);
    if (!state->type_ContactChange)
    {
        return nullptr;
    }

    state->type_ContactChangeReader = py::register_python_type(module.get(), type_name_ContactChangeReader, &type_spec_ContactChangeReader, bases.get(), nullptr);
    if (!state->type_ContactChangeReader)
    {
        return nullptr;
    }

    state->type_ContactChangeTracker = py::register_python_type(module.get(), type_name_ContactChangeTracker, &type_spec_ContactChangeTracker, bases.get(), nullptr);
    if (!state->type_ContactChangeTracker)
    {
        return nullptr;
    }

    state->type_ContactChangedDeferral = py::register_python_type(module.get(), type_name_ContactChangedDeferral, &type_spec_ContactChangedDeferral, bases.get(), nullptr);
    if (!state->type_ContactChangedDeferral)
    {
        return nullptr;
    }

    state->type_ContactChangedEventArgs = py::register_python_type(module.get(), type_name_ContactChangedEventArgs, &type_spec_ContactChangedEventArgs, bases.get(), nullptr);
    if (!state->type_ContactChangedEventArgs)
    {
        return nullptr;
    }

    state->type_ContactConnectedServiceAccount = py::register_python_type(module.get(), type_name_ContactConnectedServiceAccount, &type_spec_ContactConnectedServiceAccount, bases.get(), nullptr);
    if (!state->type_ContactConnectedServiceAccount)
    {
        return nullptr;
    }

    state->type_ContactDate = py::register_python_type(module.get(), type_name_ContactDate, &type_spec_ContactDate, bases.get(), nullptr);
    if (!state->type_ContactDate)
    {
        return nullptr;
    }

    state->type_ContactEmail = py::register_python_type(module.get(), type_name_ContactEmail, &type_spec_ContactEmail, bases.get(), nullptr);
    if (!state->type_ContactEmail)
    {
        return nullptr;
    }

    state->type_ContactField = py::register_python_type(module.get(), type_name_ContactField, &type_spec_ContactField, bases.get(), nullptr);
    if (!state->type_ContactField)
    {
        return nullptr;
    }

    state->type_ContactFieldFactory = py::register_python_type(module.get(), type_name_ContactFieldFactory, &type_spec_ContactFieldFactory, bases.get(), nullptr);
    if (!state->type_ContactFieldFactory)
    {
        return nullptr;
    }

    state->type_ContactGroup = py::register_python_type(module.get(), type_name_ContactGroup, &type_spec_ContactGroup, bases.get(), nullptr);
    ...
    state->type_ContactInstantMessageField = py::register_python_type(module.get(), type_name_ContactInstantMessageField, &type_spec_ContactInstantMessageField, bases.get(), nullptr);
    if (!state->type_ContactInstantMessageField)
    {
        return nullptr;
    }

    state->type_ContactJobInfo = py::register_python_type(module.get(), type_name_ContactJobInfo, &type_spec_ContactJobInfo, bases.get(), nullptr);
    ...
    state->type_ContactLaunchActionVerbs = py::register_python_type(module.get(), type_name_ContactLaunchActionVerbs, &type_spec_ContactLaunchActionVerbs, nullptr, reinterpret_cast<PyTypeObject*>(type_ContactLaunchActionVerbs_Meta.get()));
    if (!state->type_ContactLaunchActionVerbs)
    {
        return nullptr;
    }

    state->type_ContactList = py::register_python_type(module.get(), type_name_ContactList, &type_spec_ContactList, bases.get(), nullptr);
    if (!state->type_ContactList)
    {
        return nullptr;
    }

    state->type_ContactListLimitedWriteOperations = py::register_python_type(module.get(), type_name_ContactListLimitedWriteOperations, &type_spec_ContactListLimitedWriteOperations, bases.get(), nullptr);
    if (!state->type_ContactListLimitedWriteOperations)
    {
        return nullptr;
    }

    state->type_ContactListSyncConstraints = py::register_python_type(module.get(), type_name_ContactListSyncConstraints, &type_spec_ContactListSyncConstraints, bases.get(), nullptr);
    if (!state->type_ContactListSyncConstraints)
    {
        return nullptr;
    }

    state->type_ContactListSyncManager = py::register_python_type(module.get(), type_name_ContactListSyncManager, &type_spec_ContactListSyncManager, bases.get(), nullptr);
    if (!state->type_ContactListSyncManager)
    {
        return nullptr;
    }

    state->type_ContactLocationField = py::register_python_type(module.get(), type_name_ContactLocationField, &type_spec_ContactLocationField, bases.get(), nullptr);
    ...
    state->type_ContactManager = py::register_python_type(module.get(), type_name_ContactManager, &type_spec_ContactManager, nullptr, reinterpret_cast<PyTypeObject*>(type_ContactManager_Meta.get()));
    if (!state->type_ContactManager)
    {
        return nullptr;
    }

    state->type_ContactManagerForUser = py::register_python_type(module.get(), type_name_ContactManagerForUser, &type_spec_ContactManagerForUser, bases.get(), nullptr);
    if (!state->type_ContactManagerForUser)
    {
        return nullptr;
    }

    state->type_ContactMatchReason = py::register_python_type(module.get(), type_name_ContactMatchReason, &type_spec_ContactMatchReason, bases.get(), nullptr);
    if (!state->type_ContactMatchReason)
    {
        return nullptr;
    }

    state->type_ContactPanel = py::register_python_type(module.get(), type_name_ContactPanel, &type_spec_ContactPanel, bases.get(), nullptr);
    if (!state->type_ContactPanel)
    {
        return nullptr;
    }

    state->type_ContactPanelClosingEventArgs = py::register_python_type(module.get(), type_name_ContactPanelClosingEventArgs, &type_spec_ContactPanelClosingEventArgs, bases.get(), nullptr);
    if (!state->type_ContactPanelClosingEventArgs)
    {
        return nullptr;
    }

    state->type_ContactPanelLaunchFullAppRequestedEventArgs = py::register_python_type(module.get(), type_name_ContactPanelLaunchFullAppRequestedEventArgs, &type_spec_ContactPanelLaunchFullAppRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_ContactPanelLaunchFullAppRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_ContactPhone = py::register_python_type(module.get(), type_name_ContactPhone, &type_spec_ContactPhone, bases.get(), nullptr);
    if (!state->type_ContactPhone)
    {
        return nullptr;
    }

    state->type_ContactPicker = py::register_python_type(module.get(), type_name_ContactPicker, &type_spec_ContactPicker, bases.get(), nullptr);
    ...
    state->type_ContactQueryTextSearch = py::register_python_type(module.get(), type_name_ContactQueryTextSearch, &type_spec_ContactQueryTextSearch, bases.get(), nullptr);
    if (!state->type_ContactQueryTextSearch)
    {
        return nullptr;
    }

    state->type_ContactReader = py::register_python_type(module.get(), type_name_ContactReader, &type_spec_ContactReader, bases.get(), nullptr);
    if (!state->type_ContactReader)
    {
        return nullptr;
    }

    state->type_ContactSignificantOther = py::register_python_type(module.get(), type_name_ContactSignificantOther, &type_spec_ContactSignificantOther, bases.get(), nullptr);
    ...
    state->type_ContactStoreNotificationTriggerDetails = py::register_python_type(module.get(), type_name_ContactStoreNotificationTriggerDetails, &type_spec_ContactStoreNotificationTriggerDetails, bases.get(), nullptr);
    if (!state->type_ContactStoreNotificationTriggerDetails)
    {
        return nullptr;
    }

    state->type_ContactWebsite = py::register_python_type(module.get(), type_name_ContactWebsite, &type_spec_ContactWebsite, bases.get(), nullptr);
    if (!state->type_ContactWebsite)
    {
        return nullptr;
    }

    state->type_FullContactCardOptions = py::register_python_type(module.get(), type_name_FullContactCardOptions, &type_spec_FullContactCardOptions, bases.get(), nullptr);
    ...
    state->type_KnownContactField = py::register_python_type(module.get(), type_name_KnownContactField, &type_spec_KnownContactField, nullptr, reinterpret_cast<PyTypeObject*>(type_KnownContactField_Meta.get()));
    if (!state->type_KnownContactField)
    {
        return nullptr;
    }

    state->type_PinnedContactIdsQueryResult = py::register_python_type(module.get(), type_name_PinnedContactIdsQueryResult, &type_spec_PinnedContactIdsQueryResult, bases.get(), nullptr);
    if (!state->type_PinnedContactIdsQueryResult)
    {
        return nullptr;
    }

    state->type_PinnedContactManager = py::register_python_type(module.get(), type_name_PinnedContactManager, &type_spec_PinnedContactManager, bases.get(), nullptr);
    if (!state->type_PinnedContactManager)
    {
        return nullptr;
    }

    state->type_IContactField = py::register_python_type(module.get(), type_name_IContactField, &type_spec_IContactField, bases.get(), nullptr);
    if (!state->type_IContactField)
    {
        return nullptr;
    }

    state->type_IContactFieldFactory = py::register_python_type(module.get(), type_name_IContactFieldFactory, &type_spec_IContactFieldFactory, bases.get(), nullptr);
    if (!state->type_IContactFieldFactory)
    {
        return nullptr;
    }

    state->type_IContactInstantMessageFieldFactory = py::register_python_type(module.get(), type_name_IContactInstantMessageFieldFactory, &type_spec_IContactInstantMessageFieldFactory, bases.get(), nullptr);
    if (!state->type_IContactInstantMessageFieldFactory)
    {
        return nullptr;
    }

    state->type_IContactLocationFieldFactory = py::register_python_type(module.get(), type_name_IContactLocationFieldFactory, &type_spec_IContactLocationFieldFactory, bases.get(), nullptr);

...
PyMODINIT_FUNC PyInit__winsdk_Windows_Web_UI(void) noexcept
{
    ...
    state->type_WebViewControlContentLoadingEventArgs = py::register_python_type(module.get(), type_name_WebViewControlContentLoadingEventArgs, &type_spec_WebViewControlContentLoadingEventArgs, bases.get(), nullptr);
    if (!state->type_WebViewControlContentLoadingEventArgs)
    {
        return nullptr;
    }

    state->type_WebViewControlDOMContentLoadedEventArgs = py::register_python_type(module.get(), type_name_WebViewControlDOMContentLoadedEventArgs, &type_spec_WebViewControlDOMContentLoadedEventArgs, bases.get(), nullptr);
    if (!state->type_WebViewControlDOMContentLoadedEventArgs)
    {
        return nullptr;
    }

    state->type_WebViewControlDeferredPermissionRequest = py::register_python_type(module.get(), type_name_WebViewControlDeferredPermissionRequest, &type_spec_WebViewControlDeferredPermissionRequest, bases.get(), nullptr);
    if (!state->type_WebViewControlDeferredPermissionRequest)
    {
        return nullptr;
    }

    state->type_WebViewControlLongRunningScriptDetectedEventArgs = py::register_python_type(module.get(), type_name_WebViewControlLongRunningScriptDetectedEventArgs, &type_spec_WebViewControlLongRunningScriptDetectedEventArgs, bases.get(), nullptr);
    if (!state->type_WebViewControlLongRunningScriptDetectedEventArgs)
    {
        return nullptr;
    }

    state->type_WebViewControlNavigationCompletedEventArgs = py::register_python_type(module.get(), type_name_WebViewControlNavigationCompletedEventArgs, &type_spec_WebViewControlNavigationCompletedEventArgs, bases.get(), nullptr);
    if (!state->type_WebViewControlNavigationCompletedEventArgs)
    {
        return nullptr;
    }

    state->type_WebViewControlNavigationStartingEventArgs = py::register_python_type(module.get(), type_name_WebViewControlNavigationStartingEventArgs, &type_spec_WebViewControlNavigationStartingEventArgs, bases.get(), nullptr);
    if (!state->type_WebViewControlNavigationStartingEventArgs)
    {
        return nullptr;
    }

    state->type_WebViewControlNewWindowRequestedEventArgs = py::register_python_type(module.get(), type_name_WebViewControlNewWindowRequestedEventArgs, &type_spec_WebViewControlNewWindowRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_WebViewControlNewWindowRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_WebViewControlPermissionRequest = py::register_python_type(module.get(), type_name_WebViewControlPermissionRequest, &type_spec_WebViewControlPermissionRequest, bases.get(), nullptr);
    if (!state->type_WebViewControlPermissionRequest)
    {
        return nullptr;
    }

    state->type_WebViewControlPermissionRequestedEventArgs = py::register_python_type(module.get(), type_name_WebViewControlPermissionRequestedEventArgs, &type_spec_WebViewControlPermissionRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_WebViewControlPermissionRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_WebViewControlScriptNotifyEventArgs = py::register_python_type(module.get(), type_name_WebViewControlScriptNotifyEventArgs, &type_spec_WebViewControlScriptNotifyEventArgs, bases.get(), nullptr);
    if (!state->type_WebViewControlScriptNotifyEventArgs)
    {
        return nullptr;
    }

    state->type_WebViewControlSettings = py::register_python_type(module.get(), type_name_WebViewControlSettings, &type_spec_WebViewControlSettings, bases.get(), nullptr);
    if (!state->type_WebViewControlSettings)
    {
        return nullptr;
    }

    state->type_WebViewControlUnsupportedUriSchemeIdentifiedEventArgs = py::register_python_type(module.get(), type_name_WebViewControlUnsupportedUriSchemeIdentifiedEventArgs, &type_spec_WebViewControlUnsupportedUriSchemeIdentifiedEventArgs, bases.get(), nullptr);
    if (!state->type_WebViewControlUnsupportedUriSchemeIdentifiedEventArgs)
    {
        return nullptr;
    }

    state->type_WebViewControlUnviewableContentIdentifiedEventArgs = py::register_python_type(module.get(), type_name_WebViewControlUnviewableContentIdentifiedEventArgs, &type_spec_WebViewControlUnviewableContentIdentifiedEventArgs, bases.get(), nullptr);
    if (!state->type_WebViewControlUnviewableContentIdentifiedEventArgs)
    {
        return nullptr;
    }

    state->type_WebViewControlWebResourceRequestedEventArgs = py::register_python_type(module.get(), type_name_WebViewControlWebResourceRequestedEventArgs, &type_spec_WebViewControlWebResourceRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_WebViewControlWebResourceRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_IWebViewControl = py::register_python_type(module.get(), type_name_IWebViewControl, &type_spec_IWebViewControl, bases.get(), nullptr);
    if (!state->type_IWebViewControl)
    {
        return nullptr;
    }

    state->type_IWebViewControl2 = py::register_python_type(module.get(), type_name_IWebViewControl2, &type_spec_IWebViewControl2, bases.get(), nullptr);

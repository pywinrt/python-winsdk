...
PyMODINIT_FUNC PyInit__winsdk_Windows_Devices_Pwm_Provider(void) noexcept
{
    ...
    state->type_IPwmControllerProvider = py::register_python_type(module.get(), type_name_IPwmControllerProvider, &type_spec_IPwmControllerProvider, bases.get(), nullptr);
    if (!state->type_IPwmControllerProvider)
    {
        return nullptr;
    }

    state->type_IPwmProvider = py::register_python_type(module.get(), type_name_IPwmProvider, &type_spec_IPwmProvider, bases.get(), nullptr);

...
PyMODINIT_FUNC PyInit__winsdk_Windows_Web(void) noexcept
{
    ...
    state->type_WebError = py::register_python_type(module.get(), type_name_WebError, &type_spec_WebError, nullptr, nullptr);
    if (!state->type_WebError)
    {
        return nullptr;
    }

    state->type_IUriToStreamResolver = py::register_python_type(module.get(), type_name_IUriToStreamResolver, &type_spec_IUriToStreamResolver, bases.get(), nullptr);

...
PyMODINIT_FUNC PyInit__winsdk_Windows_UI_Input_Spatial(void) noexcept
{
    ...
    state->type_SpatialGestureRecognizer = py::register_python_type(module.get(), type_name_SpatialGestureRecognizer, &type_spec_SpatialGestureRecognizer, bases.get(), nullptr);
    if (!state->type_SpatialGestureRecognizer)
    {
        return nullptr;
    }

    state->type_SpatialHoldCanceledEventArgs = py::register_python_type(module.get(), type_name_SpatialHoldCanceledEventArgs, &type_spec_SpatialHoldCanceledEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialHoldCanceledEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialHoldCompletedEventArgs = py::register_python_type(module.get(), type_name_SpatialHoldCompletedEventArgs, &type_spec_SpatialHoldCompletedEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialHoldCompletedEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialHoldStartedEventArgs = py::register_python_type(module.get(), type_name_SpatialHoldStartedEventArgs, &type_spec_SpatialHoldStartedEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialHoldStartedEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialInteraction = py::register_python_type(module.get(), type_name_SpatialInteraction, &type_spec_SpatialInteraction, bases.get(), nullptr);
    if (!state->type_SpatialInteraction)
    {
        return nullptr;
    }

    state->type_SpatialInteractionController = py::register_python_type(module.get(), type_name_SpatialInteractionController, &type_spec_SpatialInteractionController, bases.get(), nullptr);
    if (!state->type_SpatialInteractionController)
    {
        return nullptr;
    }

    state->type_SpatialInteractionControllerProperties = py::register_python_type(module.get(), type_name_SpatialInteractionControllerProperties, &type_spec_SpatialInteractionControllerProperties, bases.get(), nullptr);
    if (!state->type_SpatialInteractionControllerProperties)
    {
        return nullptr;
    }

    state->type_SpatialInteractionDetectedEventArgs = py::register_python_type(module.get(), type_name_SpatialInteractionDetectedEventArgs, &type_spec_SpatialInteractionDetectedEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialInteractionDetectedEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialInteractionManager = py::register_python_type(module.get(), type_name_SpatialInteractionManager, &type_spec_SpatialInteractionManager, bases.get(), nullptr);
    if (!state->type_SpatialInteractionManager)
    {
        return nullptr;
    }

    state->type_SpatialInteractionSource = py::register_python_type(module.get(), type_name_SpatialInteractionSource, &type_spec_SpatialInteractionSource, bases.get(), nullptr);
    if (!state->type_SpatialInteractionSource)
    {
        return nullptr;
    }

    state->type_SpatialInteractionSourceEventArgs = py::register_python_type(module.get(), type_name_SpatialInteractionSourceEventArgs, &type_spec_SpatialInteractionSourceEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialInteractionSourceEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialInteractionSourceLocation = py::register_python_type(module.get(), type_name_SpatialInteractionSourceLocation, &type_spec_SpatialInteractionSourceLocation, bases.get(), nullptr);
    if (!state->type_SpatialInteractionSourceLocation)
    {
        return nullptr;
    }

    state->type_SpatialInteractionSourceProperties = py::register_python_type(module.get(), type_name_SpatialInteractionSourceProperties, &type_spec_SpatialInteractionSourceProperties, bases.get(), nullptr);
    if (!state->type_SpatialInteractionSourceProperties)
    {
        return nullptr;
    }

    state->type_SpatialInteractionSourceState = py::register_python_type(module.get(), type_name_SpatialInteractionSourceState, &type_spec_SpatialInteractionSourceState, bases.get(), nullptr);
    if (!state->type_SpatialInteractionSourceState)
    {
        return nullptr;
    }

    state->type_SpatialManipulationCanceledEventArgs = py::register_python_type(module.get(), type_name_SpatialManipulationCanceledEventArgs, &type_spec_SpatialManipulationCanceledEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialManipulationCanceledEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialManipulationCompletedEventArgs = py::register_python_type(module.get(), type_name_SpatialManipulationCompletedEventArgs, &type_spec_SpatialManipulationCompletedEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialManipulationCompletedEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialManipulationDelta = py::register_python_type(module.get(), type_name_SpatialManipulationDelta, &type_spec_SpatialManipulationDelta, bases.get(), nullptr);
    if (!state->type_SpatialManipulationDelta)
    {
        return nullptr;
    }

    state->type_SpatialManipulationStartedEventArgs = py::register_python_type(module.get(), type_name_SpatialManipulationStartedEventArgs, &type_spec_SpatialManipulationStartedEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialManipulationStartedEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialManipulationUpdatedEventArgs = py::register_python_type(module.get(), type_name_SpatialManipulationUpdatedEventArgs, &type_spec_SpatialManipulationUpdatedEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialManipulationUpdatedEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialNavigationCanceledEventArgs = py::register_python_type(module.get(), type_name_SpatialNavigationCanceledEventArgs, &type_spec_SpatialNavigationCanceledEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialNavigationCanceledEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialNavigationCompletedEventArgs = py::register_python_type(module.get(), type_name_SpatialNavigationCompletedEventArgs, &type_spec_SpatialNavigationCompletedEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialNavigationCompletedEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialNavigationStartedEventArgs = py::register_python_type(module.get(), type_name_SpatialNavigationStartedEventArgs, &type_spec_SpatialNavigationStartedEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialNavigationStartedEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialNavigationUpdatedEventArgs = py::register_python_type(module.get(), type_name_SpatialNavigationUpdatedEventArgs, &type_spec_SpatialNavigationUpdatedEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialNavigationUpdatedEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialPointerInteractionSourcePose = py::register_python_type(module.get(), type_name_SpatialPointerInteractionSourcePose, &type_spec_SpatialPointerInteractionSourcePose, bases.get(), nullptr);
    if (!state->type_SpatialPointerInteractionSourcePose)
    {
        return nullptr;
    }

    state->type_SpatialPointerPose = py::register_python_type(module.get(), type_name_SpatialPointerPose, &type_spec_SpatialPointerPose, bases.get(), nullptr);
    if (!state->type_SpatialPointerPose)
    {
        return nullptr;
    }

    state->type_SpatialRecognitionEndedEventArgs = py::register_python_type(module.get(), type_name_SpatialRecognitionEndedEventArgs, &type_spec_SpatialRecognitionEndedEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialRecognitionEndedEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialRecognitionStartedEventArgs = py::register_python_type(module.get(), type_name_SpatialRecognitionStartedEventArgs, &type_spec_SpatialRecognitionStartedEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialRecognitionStartedEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialTappedEventArgs = py::register_python_type(module.get(), type_name_SpatialTappedEventArgs, &type_spec_SpatialTappedEventArgs, bases.get(), nullptr);

...
PyMODINIT_FUNC PyInit__winsdk_Windows_UI_Input_Inking_Core(void) noexcept
{
    ...
    state->type_CoreIncrementalInkStroke = py::register_python_type(module.get(), type_name_CoreIncrementalInkStroke, &type_spec_CoreIncrementalInkStroke, bases.get(), nullptr);
    if (!state->type_CoreIncrementalInkStroke)
    {
        return nullptr;
    }

    state->type_CoreInkIndependentInputSource = py::register_python_type(module.get(), type_name_CoreInkIndependentInputSource, &type_spec_CoreInkIndependentInputSource, bases.get(), nullptr);
    if (!state->type_CoreInkIndependentInputSource)
    {
        return nullptr;
    }

    state->type_CoreInkPresenterHost = py::register_python_type(module.get(), type_name_CoreInkPresenterHost, &type_spec_CoreInkPresenterHost, bases.get(), nullptr);
    if (!state->type_CoreInkPresenterHost)
    {
        return nullptr;
    }

    state->type_CoreWetStrokeUpdateEventArgs = py::register_python_type(module.get(), type_name_CoreWetStrokeUpdateEventArgs, &type_spec_CoreWetStrokeUpdateEventArgs, bases.get(), nullptr);
    if (!state->type_CoreWetStrokeUpdateEventArgs)
    {
        return nullptr;
    }

    state->type_CoreWetStrokeUpdateSource = py::register_python_type(module.get(), type_name_CoreWetStrokeUpdateSource, &type_spec_CoreWetStrokeUpdateSource, bases.get(), nullptr);

...
PyMODINIT_FUNC PyInit__winsdk_Windows_ApplicationModel_DataTransfer_ShareTarget(void) noexcept
{
    ...
    state->type_QuickLink = py::register_python_type(module.get(), type_name_QuickLink, &type_spec_QuickLink, bases.get(), nullptr);
    if (!state->type_QuickLink)
    {
        return nullptr;
    }

    state->type_ShareOperation = py::register_python_type(module.get(), type_name_ShareOperation, &type_spec_ShareOperation, bases.get(), nullptr);

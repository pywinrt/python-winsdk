...
PyMODINIT_FUNC PyInit__winsdk_Windows_Media_Editing(void) noexcept
{
    ...
    state->type_BackgroundAudioTrack = py::register_python_type(module.get(), type_name_BackgroundAudioTrack, &type_spec_BackgroundAudioTrack, bases.get(), nullptr);
    if (!state->type_BackgroundAudioTrack)
    {
        return nullptr;
    }

    state->type_EmbeddedAudioTrack = py::register_python_type(module.get(), type_name_EmbeddedAudioTrack, &type_spec_EmbeddedAudioTrack, bases.get(), nullptr);
    if (!state->type_EmbeddedAudioTrack)
    {
        return nullptr;
    }

    state->type_MediaClip = py::register_python_type(module.get(), type_name_MediaClip, &type_spec_MediaClip, bases.get(), nullptr);
    if (!state->type_MediaClip)
    {
        return nullptr;
    }

    state->type_MediaComposition = py::register_python_type(module.get(), type_name_MediaComposition, &type_spec_MediaComposition, bases.get(), nullptr);
    if (!state->type_MediaComposition)
    {
        return nullptr;
    }

    state->type_MediaOverlay = py::register_python_type(module.get(), type_name_MediaOverlay, &type_spec_MediaOverlay, bases.get(), nullptr);
    if (!state->type_MediaOverlay)
    {
        return nullptr;
    }

    state->type_MediaOverlayLayer = py::register_python_type(module.get(), type_name_MediaOverlayLayer, &type_spec_MediaOverlayLayer, bases.get(), nullptr);

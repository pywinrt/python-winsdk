...
namespace py::cpp::Windows::Devices::Perception::Provider
{
    ...
    static PyType_Slot type_slots_KnownPerceptionFrameKind_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_KnownPerceptionFrameKind_Meta },
        { }
    };

    static PyType_Spec type_spec_KnownPerceptionFrameKind_Meta =
    {
        "_winsdk_Windows_Devices_Perception_Provider.KnownPerceptionFrameKind_Meta",
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Devices_Perception_Provider(void) noexcept
{
    ...
    state->type_KnownPerceptionFrameKind = py::register_python_type(module.get(), type_name_KnownPerceptionFrameKind, &type_spec_KnownPerceptionFrameKind, nullptr, reinterpret_cast<PyTypeObject*>(type_KnownPerceptionFrameKind_Meta.get()));
    if (!state->type_KnownPerceptionFrameKind)
    {
        return nullptr;
    }

    state->type_PerceptionControlGroup = py::register_python_type(module.get(), type_name_PerceptionControlGroup, &type_spec_PerceptionControlGroup, bases.get(), nullptr);
    if (!state->type_PerceptionControlGroup)
    {
        return nullptr;
    }

    state->type_PerceptionCorrelation = py::register_python_type(module.get(), type_name_PerceptionCorrelation, &type_spec_PerceptionCorrelation, bases.get(), nullptr);
    if (!state->type_PerceptionCorrelation)
    {
        return nullptr;
    }

    state->type_PerceptionCorrelationGroup = py::register_python_type(module.get(), type_name_PerceptionCorrelationGroup, &type_spec_PerceptionCorrelationGroup, bases.get(), nullptr);
    if (!state->type_PerceptionCorrelationGroup)
    {
        return nullptr;
    }

    state->type_PerceptionFaceAuthenticationGroup = py::register_python_type(module.get(), type_name_PerceptionFaceAuthenticationGroup, &type_spec_PerceptionFaceAuthenticationGroup, bases.get(), nullptr);
    if (!state->type_PerceptionFaceAuthenticationGroup)
    {
        return nullptr;
    }

    state->type_PerceptionFrame = py::register_python_type(module.get(), type_name_PerceptionFrame, &type_spec_PerceptionFrame, bases.get(), nullptr);
    if (!state->type_PerceptionFrame)
    {
        return nullptr;
    }

    state->type_PerceptionFrameProviderInfo = py::register_python_type(module.get(), type_name_PerceptionFrameProviderInfo, &type_spec_PerceptionFrameProviderInfo, bases.get(), nullptr);
    if (!state->type_PerceptionFrameProviderInfo)
    {
        return nullptr;
    }

    state->type_PerceptionFrameProviderManagerService = py::register_python_type(module.get(), type_name_PerceptionFrameProviderManagerService, &type_spec_PerceptionFrameProviderManagerService, nullptr, nullptr);
    if (!state->type_PerceptionFrameProviderManagerService)
    {
        return nullptr;
    }

    state->type_PerceptionPropertyChangeRequest = py::register_python_type(module.get(), type_name_PerceptionPropertyChangeRequest, &type_spec_PerceptionPropertyChangeRequest, bases.get(), nullptr);
    if (!state->type_PerceptionPropertyChangeRequest)
    {
        return nullptr;
    }

    state->type_PerceptionVideoFrameAllocator = py::register_python_type(module.get(), type_name_PerceptionVideoFrameAllocator, &type_spec_PerceptionVideoFrameAllocator, bases.get(), nullptr);
    if (!state->type_PerceptionVideoFrameAllocator)
    {
        return nullptr;
    }

    state->type_IPerceptionFrameProvider = py::register_python_type(module.get(), type_name_IPerceptionFrameProvider, &type_spec_IPerceptionFrameProvider, bases.get(), nullptr);
    if (!state->type_IPerceptionFrameProvider)
    {
        return nullptr;
    }

    state->type_IPerceptionFrameProviderManager = py::register_python_type(module.get(), type_name_IPerceptionFrameProviderManager, &type_spec_IPerceptionFrameProviderManager, bases.get(), nullptr);

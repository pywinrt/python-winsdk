...
PyMODINIT_FUNC PyInit__winsdk_Windows_UI_WindowManagement(void) noexcept
{
    ...
    state->type_AppWindow = py::register_python_type(module.get(), type_name_AppWindow, &type_spec_AppWindow, bases.get(), nullptr);
    if (!state->type_AppWindow)
    {
        return nullptr;
    }

    state->type_AppWindowChangedEventArgs = py::register_python_type(module.get(), type_name_AppWindowChangedEventArgs, &type_spec_AppWindowChangedEventArgs, bases.get(), nullptr);
    if (!state->type_AppWindowChangedEventArgs)
    {
        return nullptr;
    }

    state->type_AppWindowCloseRequestedEventArgs = py::register_python_type(module.get(), type_name_AppWindowCloseRequestedEventArgs, &type_spec_AppWindowCloseRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_AppWindowCloseRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_AppWindowClosedEventArgs = py::register_python_type(module.get(), type_name_AppWindowClosedEventArgs, &type_spec_AppWindowClosedEventArgs, bases.get(), nullptr);
    if (!state->type_AppWindowClosedEventArgs)
    {
        return nullptr;
    }

    state->type_AppWindowFrame = py::register_python_type(module.get(), type_name_AppWindowFrame, &type_spec_AppWindowFrame, bases.get(), nullptr);
    if (!state->type_AppWindowFrame)
    {
        return nullptr;
    }

    state->type_AppWindowPlacement = py::register_python_type(module.get(), type_name_AppWindowPlacement, &type_spec_AppWindowPlacement, bases.get(), nullptr);
    if (!state->type_AppWindowPlacement)
    {
        return nullptr;
    }

    state->type_AppWindowPresentationConfiguration = py::register_python_type(module.get(), type_name_AppWindowPresentationConfiguration, &type_spec_AppWindowPresentationConfiguration, bases.get(), nullptr);
    if (!state->type_AppWindowPresentationConfiguration)
    {
        return nullptr;
    }

    state->type_AppWindowPresenter = py::register_python_type(module.get(), type_name_AppWindowPresenter, &type_spec_AppWindowPresenter, bases.get(), nullptr);
    if (!state->type_AppWindowPresenter)
    {
        return nullptr;
    }

    state->type_AppWindowTitleBar = py::register_python_type(module.get(), type_name_AppWindowTitleBar, &type_spec_AppWindowTitleBar, bases.get(), nullptr);
    if (!state->type_AppWindowTitleBar)
    {
        return nullptr;
    }

    state->type_AppWindowTitleBarOcclusion = py::register_python_type(module.get(), type_name_AppWindowTitleBarOcclusion, &type_spec_AppWindowTitleBarOcclusion, bases.get(), nullptr);
    if (!state->type_AppWindowTitleBarOcclusion)
    {
        return nullptr;
    }

    state->type_CompactOverlayPresentationConfiguration = py::register_python_type(module.get(), type_name_CompactOverlayPresentationConfiguration, &type_spec_CompactOverlayPresentationConfiguration, bases.get(), nullptr);
    if (!state->type_CompactOverlayPresentationConfiguration)
    {
        return nullptr;
    }

    state->type_DefaultPresentationConfiguration = py::register_python_type(module.get(), type_name_DefaultPresentationConfiguration, &type_spec_DefaultPresentationConfiguration, bases.get(), nullptr);
    if (!state->type_DefaultPresentationConfiguration)
    {
        return nullptr;
    }

    state->type_DisplayRegion = py::register_python_type(module.get(), type_name_DisplayRegion, &type_spec_DisplayRegion, bases.get(), nullptr);
    if (!state->type_DisplayRegion)
    {
        return nullptr;
    }

    state->type_FullScreenPresentationConfiguration = py::register_python_type(module.get(), type_name_FullScreenPresentationConfiguration, &type_spec_FullScreenPresentationConfiguration, bases.get(), nullptr);
    if (!state->type_FullScreenPresentationConfiguration)
    {
        return nullptr;
    }

    state->type_WindowServices = py::register_python_type(module.get(), type_name_WindowServices, &type_spec_WindowServices, nullptr, nullptr);
    if (!state->type_WindowServices)
    {
        return nullptr;
    }

    state->type_WindowingEnvironment = py::register_python_type(module.get(), type_name_WindowingEnvironment, &type_spec_WindowingEnvironment, bases.get(), nullptr);
    if (!state->type_WindowingEnvironment)
    {
        return nullptr;
    }

    state->type_WindowingEnvironmentAddedEventArgs = py::register_python_type(module.get(), type_name_WindowingEnvironmentAddedEventArgs, &type_spec_WindowingEnvironmentAddedEventArgs, bases.get(), nullptr);
    if (!state->type_WindowingEnvironmentAddedEventArgs)
    {
        return nullptr;
    }

    state->type_WindowingEnvironmentChangedEventArgs = py::register_python_type(module.get(), type_name_WindowingEnvironmentChangedEventArgs, &type_spec_WindowingEnvironmentChangedEventArgs, bases.get(), nullptr);
    if (!state->type_WindowingEnvironmentChangedEventArgs)
    {
        return nullptr;
    }

    state->type_WindowingEnvironmentRemovedEventArgs = py::register_python_type(module.get(), type_name_WindowingEnvironmentRemovedEventArgs, &type_spec_WindowingEnvironmentRemovedEventArgs, bases.get(), nullptr);

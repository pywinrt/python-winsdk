...
PyMODINIT_FUNC PyInit__winsdk_Windows_Globalization_NumberFormatting(void) noexcept
{
    ...
    state->type_CurrencyFormatter = py::register_python_type(module.get(), type_name_CurrencyFormatter, &type_spec_CurrencyFormatter, bases.get(), nullptr);
    if (!state->type_CurrencyFormatter)
    {
        return nullptr;
    }

    state->type_DecimalFormatter = py::register_python_type(module.get(), type_name_DecimalFormatter, &type_spec_DecimalFormatter, bases.get(), nullptr);
    if (!state->type_DecimalFormatter)
    {
        return nullptr;
    }

    state->type_IncrementNumberRounder = py::register_python_type(module.get(), type_name_IncrementNumberRounder, &type_spec_IncrementNumberRounder, bases.get(), nullptr);
    if (!state->type_IncrementNumberRounder)
    {
        return nullptr;
    }

    state->type_NumeralSystemTranslator = py::register_python_type(module.get(), type_name_NumeralSystemTranslator, &type_spec_NumeralSystemTranslator, bases.get(), nullptr);
    if (!state->type_NumeralSystemTranslator)
    {
        return nullptr;
    }

    state->type_PercentFormatter = py::register_python_type(module.get(), type_name_PercentFormatter, &type_spec_PercentFormatter, bases.get(), nullptr);
    if (!state->type_PercentFormatter)
    {
        return nullptr;
    }

    state->type_PermilleFormatter = py::register_python_type(module.get(), type_name_PermilleFormatter, &type_spec_PermilleFormatter, bases.get(), nullptr);
    if (!state->type_PermilleFormatter)
    {
        return nullptr;
    }

    state->type_SignificantDigitsNumberRounder = py::register_python_type(module.get(), type_name_SignificantDigitsNumberRounder, &type_spec_SignificantDigitsNumberRounder, bases.get(), nullptr);
    if (!state->type_SignificantDigitsNumberRounder)
    {
        return nullptr;
    }

    state->type_INumberFormatter = py::register_python_type(module.get(), type_name_INumberFormatter, &type_spec_INumberFormatter, bases.get(), nullptr);
    if (!state->type_INumberFormatter)
    {
        return nullptr;
    }

    state->type_INumberFormatter2 = py::register_python_type(module.get(), type_name_INumberFormatter2, &type_spec_INumberFormatter2, bases.get(), nullptr);
    if (!state->type_INumberFormatter2)
    {
        return nullptr;
    }

    state->type_INumberFormatterOptions = py::register_python_type(module.get(), type_name_INumberFormatterOptions, &type_spec_INumberFormatterOptions, bases.get(), nullptr);
    if (!state->type_INumberFormatterOptions)
    {
        return nullptr;
    }

    state->type_INumberParser = py::register_python_type(module.get(), type_name_INumberParser, &type_spec_INumberParser, bases.get(), nullptr);
    if (!state->type_INumberParser)
    {
        return nullptr;
    }

    state->type_INumberRounder = py::register_python_type(module.get(), type_name_INumberRounder, &type_spec_INumberRounder, bases.get(), nullptr);
    if (!state->type_INumberRounder)
    {
        return nullptr;
    }

    state->type_INumberRounderOption = py::register_python_type(module.get(), type_name_INumberRounderOption, &type_spec_INumberRounderOption, bases.get(), nullptr);
    if (!state->type_INumberRounderOption)
    {
        return nullptr;
    }

    state->type_ISignedZeroOption = py::register_python_type(module.get(), type_name_ISignedZeroOption, &type_spec_ISignedZeroOption, bases.get(), nullptr);
    if (!state->type_ISignedZeroOption)
    {
        return nullptr;
    }

    state->type_ISignificantDigitsOption = py::register_python_type(module.get(), type_name_ISignificantDigitsOption, &type_spec_ISignificantDigitsOption, bases.get(), nullptr);

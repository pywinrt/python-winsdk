...
PyMODINIT_FUNC PyInit__winsdk_Windows_UI_ViewManagement_Core(void) noexcept
{
    ...
    state->type_CoreFrameworkInputView = py::register_python_type(module.get(), type_name_CoreFrameworkInputView, &type_spec_CoreFrameworkInputView, bases.get(), nullptr);
    if (!state->type_CoreFrameworkInputView)
    {
        return nullptr;
    }

    state->type_CoreFrameworkInputViewAnimationStartingEventArgs = py::register_python_type(module.get(), type_name_CoreFrameworkInputViewAnimationStartingEventArgs, &type_spec_CoreFrameworkInputViewAnimationStartingEventArgs, bases.get(), nullptr);
    if (!state->type_CoreFrameworkInputViewAnimationStartingEventArgs)
    {
        return nullptr;
    }

    state->type_CoreFrameworkInputViewOcclusionsChangedEventArgs = py::register_python_type(module.get(), type_name_CoreFrameworkInputViewOcclusionsChangedEventArgs, &type_spec_CoreFrameworkInputViewOcclusionsChangedEventArgs, bases.get(), nullptr);
    if (!state->type_CoreFrameworkInputViewOcclusionsChangedEventArgs)
    {
        return nullptr;
    }

    state->type_CoreInputView = py::register_python_type(module.get(), type_name_CoreInputView, &type_spec_CoreInputView, bases.get(), nullptr);
    if (!state->type_CoreInputView)
    {
        return nullptr;
    }

    state->type_CoreInputViewAnimationStartingEventArgs = py::register_python_type(module.get(), type_name_CoreInputViewAnimationStartingEventArgs, &type_spec_CoreInputViewAnimationStartingEventArgs, bases.get(), nullptr);
    if (!state->type_CoreInputViewAnimationStartingEventArgs)
    {
        return nullptr;
    }

    state->type_CoreInputViewHidingEventArgs = py::register_python_type(module.get(), type_name_CoreInputViewHidingEventArgs, &type_spec_CoreInputViewHidingEventArgs, bases.get(), nullptr);
    if (!state->type_CoreInputViewHidingEventArgs)
    {
        return nullptr;
    }

    state->type_CoreInputViewOcclusion = py::register_python_type(module.get(), type_name_CoreInputViewOcclusion, &type_spec_CoreInputViewOcclusion, bases.get(), nullptr);
    if (!state->type_CoreInputViewOcclusion)
    {
        return nullptr;
    }

    state->type_CoreInputViewOcclusionsChangedEventArgs = py::register_python_type(module.get(), type_name_CoreInputViewOcclusionsChangedEventArgs, &type_spec_CoreInputViewOcclusionsChangedEventArgs, bases.get(), nullptr);
    if (!state->type_CoreInputViewOcclusionsChangedEventArgs)
    {
        return nullptr;
    }

    state->type_CoreInputViewShowingEventArgs = py::register_python_type(module.get(), type_name_CoreInputViewShowingEventArgs, &type_spec_CoreInputViewShowingEventArgs, bases.get(), nullptr);
    if (!state->type_CoreInputViewShowingEventArgs)
    {
        return nullptr;
    }

    state->type_CoreInputViewTransferringXYFocusEventArgs = py::register_python_type(module.get(), type_name_CoreInputViewTransferringXYFocusEventArgs, &type_spec_CoreInputViewTransferringXYFocusEventArgs, bases.get(), nullptr);
    if (!state->type_CoreInputViewTransferringXYFocusEventArgs)
    {
        return nullptr;
    }

    state->type_UISettingsController = py::register_python_type(module.get(), type_name_UISettingsController, &type_spec_UISettingsController, bases.get(), nullptr);

...
namespace py::cpp::Windows::Foundation::Collections
{
    ...
    static PyType_Slot _type_slots_IObservableVector[] = 
    {
        ...
        { Py_mp_subscript, _seq_subscript_IObservableVector },
    ...
    static PyType_Slot _type_slots_IVectorView[] = 
    {
        ...
        { Py_mp_subscript, _seq_subscript_IVectorView },
        { },
    };
    ...
    static PyType_Slot _type_slots_IVector[] = 
    {
        ...
        { Py_mp_subscript, _seq_subscript_IVector },
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Foundation_Collections(void) noexcept
{
    ...
    state->type_PropertySet = py::register_python_type(module.get(), type_name_PropertySet, &type_spec_PropertySet, mutable_mapping_bases.get(), nullptr);
    if (!state->type_PropertySet)
    {
        return nullptr;
    }

    state->type_StringMap = py::register_python_type(module.get(), type_name_StringMap, &type_spec_StringMap, mutable_mapping_bases.get(), nullptr);
    if (!state->type_StringMap)
    {
        return nullptr;
    }

    state->type_ValueSet = py::register_python_type(module.get(), type_name_ValueSet, &type_spec_ValueSet, mutable_mapping_bases.get(), nullptr);
    if (!state->type_ValueSet)
    {
        return nullptr;
    }

    state->type_IIterable = py::register_python_type(module.get(), type_name_IIterable, &type_spec_IIterable, bases.get(), nullptr);
    if (!state->type_IIterable)
    {
        return nullptr;
    }

    state->type_IIterator = py::register_python_type(module.get(), type_name_IIterator, &type_spec_IIterator, bases.get(), nullptr);
    if (!state->type_IIterator)
    {
        return nullptr;
    }

    state->type_IKeyValuePair = py::register_python_type(module.get(), type_name_IKeyValuePair, &type_spec_IKeyValuePair, bases.get(), nullptr);
    if (!state->type_IKeyValuePair)
    {
        return nullptr;
    }

    state->type_IMapChangedEventArgs = py::register_python_type(module.get(), type_name_IMapChangedEventArgs, &type_spec_IMapChangedEventArgs, bases.get(), nullptr);
    if (!state->type_IMapChangedEventArgs)
    {
        return nullptr;
    }

    state->type_IMapView = py::register_python_type(module.get(), type_name_IMapView, &type_spec_IMapView, mapping_bases.get(), nullptr);
    if (!state->type_IMapView)
    {
        return nullptr;
    }

    state->type_IMap = py::register_python_type(module.get(), type_name_IMap, &type_spec_IMap, mutable_mapping_bases.get(), nullptr);
    if (!state->type_IMap)
    {
        return nullptr;
    }

    state->type_IObservableMap = py::register_python_type(module.get(), type_name_IObservableMap, &type_spec_IObservableMap, mutable_mapping_bases.get(), nullptr);
    if (!state->type_IObservableMap)
    {
        return nullptr;
    }

    state->type_IObservableVector = py::register_python_type(module.get(), type_name_IObservableVector, &type_spec_IObservableVector, mutable_sequence_bases.get(), nullptr);
    if (!state->type_IObservableVector)
    {
        return nullptr;
    }

    state->type_IPropertySet = py::register_python_type(module.get(), type_name_IPropertySet, &type_spec_IPropertySet, mutable_mapping_bases.get(), nullptr);
    if (!state->type_IPropertySet)
    {
        return nullptr;
    }

    state->type_IVectorChangedEventArgs = py::register_python_type(module.get(), type_name_IVectorChangedEventArgs, &type_spec_IVectorChangedEventArgs, bases.get(), nullptr);
    if (!state->type_IVectorChangedEventArgs)
    {
        return nullptr;
    }

    state->type_IVectorView = py::register_python_type(module.get(), type_name_IVectorView, &type_spec_IVectorView, sequence_bases.get(), nullptr);
    if (!state->type_IVectorView)
    {
        return nullptr;
    }

    state->type_IVector = py::register_python_type(module.get(), type_name_IVector, &type_spec_IVector, mutable_sequence_bases.get(), nullptr);

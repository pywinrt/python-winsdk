...
PyMODINIT_FUNC PyInit__winsdk_Windows_Media_Streaming_Adaptive(void) noexcept
{
    ...
    state->type_AdaptiveMediaSource = py::register_python_type(module.get(), type_name_AdaptiveMediaSource, &type_spec_AdaptiveMediaSource, bases.get(), nullptr);
    if (!state->type_AdaptiveMediaSource)
    {
        return nullptr;
    }

    state->type_AdaptiveMediaSourceAdvancedSettings = py::register_python_type(module.get(), type_name_AdaptiveMediaSourceAdvancedSettings, &type_spec_AdaptiveMediaSourceAdvancedSettings, bases.get(), nullptr);
    if (!state->type_AdaptiveMediaSourceAdvancedSettings)
    {
        return nullptr;
    }

    state->type_AdaptiveMediaSourceCorrelatedTimes = py::register_python_type(module.get(), type_name_AdaptiveMediaSourceCorrelatedTimes, &type_spec_AdaptiveMediaSourceCorrelatedTimes, bases.get(), nullptr);
    if (!state->type_AdaptiveMediaSourceCorrelatedTimes)
    {
        return nullptr;
    }

    state->type_AdaptiveMediaSourceCreationResult = py::register_python_type(module.get(), type_name_AdaptiveMediaSourceCreationResult, &type_spec_AdaptiveMediaSourceCreationResult, bases.get(), nullptr);
    if (!state->type_AdaptiveMediaSourceCreationResult)
    {
        return nullptr;
    }

    state->type_AdaptiveMediaSourceDiagnosticAvailableEventArgs = py::register_python_type(module.get(), type_name_AdaptiveMediaSourceDiagnosticAvailableEventArgs, &type_spec_AdaptiveMediaSourceDiagnosticAvailableEventArgs, bases.get(), nullptr);
    if (!state->type_AdaptiveMediaSourceDiagnosticAvailableEventArgs)
    {
        return nullptr;
    }

    state->type_AdaptiveMediaSourceDiagnostics = py::register_python_type(module.get(), type_name_AdaptiveMediaSourceDiagnostics, &type_spec_AdaptiveMediaSourceDiagnostics, bases.get(), nullptr);
    if (!state->type_AdaptiveMediaSourceDiagnostics)
    {
        return nullptr;
    }

    state->type_AdaptiveMediaSourceDownloadBitrateChangedEventArgs = py::register_python_type(module.get(), type_name_AdaptiveMediaSourceDownloadBitrateChangedEventArgs, &type_spec_AdaptiveMediaSourceDownloadBitrateChangedEventArgs, bases.get(), nullptr);
    if (!state->type_AdaptiveMediaSourceDownloadBitrateChangedEventArgs)
    {
        return nullptr;
    }

    state->type_AdaptiveMediaSourceDownloadCompletedEventArgs = py::register_python_type(module.get(), type_name_AdaptiveMediaSourceDownloadCompletedEventArgs, &type_spec_AdaptiveMediaSourceDownloadCompletedEventArgs, bases.get(), nullptr);
    if (!state->type_AdaptiveMediaSourceDownloadCompletedEventArgs)
    {
        return nullptr;
    }

    state->type_AdaptiveMediaSourceDownloadFailedEventArgs = py::register_python_type(module.get(), type_name_AdaptiveMediaSourceDownloadFailedEventArgs, &type_spec_AdaptiveMediaSourceDownloadFailedEventArgs, bases.get(), nullptr);
    if (!state->type_AdaptiveMediaSourceDownloadFailedEventArgs)
    {
        return nullptr;
    }

    state->type_AdaptiveMediaSourceDownloadRequestedDeferral = py::register_python_type(module.get(), type_name_AdaptiveMediaSourceDownloadRequestedDeferral, &type_spec_AdaptiveMediaSourceDownloadRequestedDeferral, bases.get(), nullptr);
    if (!state->type_AdaptiveMediaSourceDownloadRequestedDeferral)
    {
        return nullptr;
    }

    state->type_AdaptiveMediaSourceDownloadRequestedEventArgs = py::register_python_type(module.get(), type_name_AdaptiveMediaSourceDownloadRequestedEventArgs, &type_spec_AdaptiveMediaSourceDownloadRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_AdaptiveMediaSourceDownloadRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_AdaptiveMediaSourceDownloadResult = py::register_python_type(module.get(), type_name_AdaptiveMediaSourceDownloadResult, &type_spec_AdaptiveMediaSourceDownloadResult, bases.get(), nullptr);
    if (!state->type_AdaptiveMediaSourceDownloadResult)
    {
        return nullptr;
    }

    state->type_AdaptiveMediaSourceDownloadStatistics = py::register_python_type(module.get(), type_name_AdaptiveMediaSourceDownloadStatistics, &type_spec_AdaptiveMediaSourceDownloadStatistics, bases.get(), nullptr);
    if (!state->type_AdaptiveMediaSourceDownloadStatistics)
    {
        return nullptr;
    }

    state->type_AdaptiveMediaSourcePlaybackBitrateChangedEventArgs = py::register_python_type(module.get(), type_name_AdaptiveMediaSourcePlaybackBitrateChangedEventArgs, &type_spec_AdaptiveMediaSourcePlaybackBitrateChangedEventArgs, bases.get(), nullptr);

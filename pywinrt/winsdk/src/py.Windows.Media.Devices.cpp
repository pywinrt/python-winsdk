...
PyMODINIT_FUNC PyInit__winsdk_Windows_Media_Devices(void) noexcept
{
    ...
    state->type_AdvancedPhotoCaptureSettings = py::register_python_type(module.get(), type_name_AdvancedPhotoCaptureSettings, &type_spec_AdvancedPhotoCaptureSettings, bases.get(), nullptr);
    if (!state->type_AdvancedPhotoCaptureSettings)
    {
        return nullptr;
    }

    state->type_AdvancedPhotoControl = py::register_python_type(module.get(), type_name_AdvancedPhotoControl, &type_spec_AdvancedPhotoControl, bases.get(), nullptr);
    if (!state->type_AdvancedPhotoControl)
    {
        return nullptr;
    }

    state->type_AudioDeviceController = py::register_python_type(module.get(), type_name_AudioDeviceController, &type_spec_AudioDeviceController, bases.get(), nullptr);
    if (!state->type_AudioDeviceController)
    {
        return nullptr;
    }

    state->type_AudioDeviceModule = py::register_python_type(module.get(), type_name_AudioDeviceModule, &type_spec_AudioDeviceModule, bases.get(), nullptr);
    if (!state->type_AudioDeviceModule)
    {
        return nullptr;
    }

    state->type_AudioDeviceModuleNotificationEventArgs = py::register_python_type(module.get(), type_name_AudioDeviceModuleNotificationEventArgs, &type_spec_AudioDeviceModuleNotificationEventArgs, bases.get(), nullptr);
    if (!state->type_AudioDeviceModuleNotificationEventArgs)
    {
        return nullptr;
    }

    state->type_AudioDeviceModulesManager = py::register_python_type(module.get(), type_name_AudioDeviceModulesManager, &type_spec_AudioDeviceModulesManager, bases.get(), nullptr);
    if (!state->type_AudioDeviceModulesManager)
    {
        return nullptr;
    }

    state->type_CallControl = py::register_python_type(module.get(), type_name_CallControl, &type_spec_CallControl, bases.get(), nullptr);
    if (!state->type_CallControl)
    {
        return nullptr;
    }

    state->type_CameraOcclusionInfo = py::register_python_type(module.get(), type_name_CameraOcclusionInfo, &type_spec_CameraOcclusionInfo, bases.get(), nullptr);
    if (!state->type_CameraOcclusionInfo)
    {
        return nullptr;
    }

    state->type_CameraOcclusionState = py::register_python_type(module.get(), type_name_CameraOcclusionState, &type_spec_CameraOcclusionState, bases.get(), nullptr);
    if (!state->type_CameraOcclusionState)
    {
        return nullptr;
    }

    state->type_CameraOcclusionStateChangedEventArgs = py::register_python_type(module.get(), type_name_CameraOcclusionStateChangedEventArgs, &type_spec_CameraOcclusionStateChangedEventArgs, bases.get(), nullptr);
    if (!state->type_CameraOcclusionStateChangedEventArgs)
    {
        return nullptr;
    }

    state->type_DefaultAudioCaptureDeviceChangedEventArgs = py::register_python_type(module.get(), type_name_DefaultAudioCaptureDeviceChangedEventArgs, &type_spec_DefaultAudioCaptureDeviceChangedEventArgs, bases.get(), nullptr);
    if (!state->type_DefaultAudioCaptureDeviceChangedEventArgs)
    {
        return nullptr;
    }

    state->type_DefaultAudioRenderDeviceChangedEventArgs = py::register_python_type(module.get(), type_name_DefaultAudioRenderDeviceChangedEventArgs, &type_spec_DefaultAudioRenderDeviceChangedEventArgs, bases.get(), nullptr);
    if (!state->type_DefaultAudioRenderDeviceChangedEventArgs)
    {
        return nullptr;
    }

    state->type_DialRequestedEventArgs = py::register_python_type(module.get(), type_name_DialRequestedEventArgs, &type_spec_DialRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_DialRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_DigitalWindowBounds = py::register_python_type(module.get(), type_name_DigitalWindowBounds, &type_spec_DigitalWindowBounds, bases.get(), nullptr);
    if (!state->type_DigitalWindowBounds)
    {
        return nullptr;
    }

    state->type_DigitalWindowCapability = py::register_python_type(module.get(), type_name_DigitalWindowCapability, &type_spec_DigitalWindowCapability, bases.get(), nullptr);
    if (!state->type_DigitalWindowCapability)
    {
        return nullptr;
    }

    state->type_DigitalWindowControl = py::register_python_type(module.get(), type_name_DigitalWindowControl, &type_spec_DigitalWindowControl, bases.get(), nullptr);
    if (!state->type_DigitalWindowControl)
    {
        return nullptr;
    }

    state->type_ExposureCompensationControl = py::register_python_type(module.get(), type_name_ExposureCompensationControl, &type_spec_ExposureCompensationControl, bases.get(), nullptr);
    if (!state->type_ExposureCompensationControl)
    {
        return nullptr;
    }

    state->type_ExposureControl = py::register_python_type(module.get(), type_name_ExposureControl, &type_spec_ExposureControl, bases.get(), nullptr);
    if (!state->type_ExposureControl)
    {
        return nullptr;
    }

    state->type_ExposurePriorityVideoControl = py::register_python_type(module.get(), type_name_ExposurePriorityVideoControl, &type_spec_ExposurePriorityVideoControl, bases.get(), nullptr);
    if (!state->type_ExposurePriorityVideoControl)
    {
        return nullptr;
    }

    state->type_FlashControl = py::register_python_type(module.get(), type_name_FlashControl, &type_spec_FlashControl, bases.get(), nullptr);
    if (!state->type_FlashControl)
    {
        return nullptr;
    }

    state->type_FocusControl = py::register_python_type(module.get(), type_name_FocusControl, &type_spec_FocusControl, bases.get(), nullptr);
    if (!state->type_FocusControl)
    {
        return nullptr;
    }

    state->type_FocusSettings = py::register_python_type(module.get(), type_name_FocusSettings, &type_spec_FocusSettings, bases.get(), nullptr);
    if (!state->type_FocusSettings)
    {
        return nullptr;
    }

    state->type_HdrVideoControl = py::register_python_type(module.get(), type_name_HdrVideoControl, &type_spec_HdrVideoControl, bases.get(), nullptr);
    if (!state->type_HdrVideoControl)
    {
        return nullptr;
    }

    state->type_InfraredTorchControl = py::register_python_type(module.get(), type_name_InfraredTorchControl, &type_spec_InfraredTorchControl, bases.get(), nullptr);
    if (!state->type_InfraredTorchControl)
    {
        return nullptr;
    }

    state->type_IsoSpeedControl = py::register_python_type(module.get(), type_name_IsoSpeedControl, &type_spec_IsoSpeedControl, bases.get(), nullptr);
    if (!state->type_IsoSpeedControl)
    {
        return nullptr;
    }

    state->type_KeypadPressedEventArgs = py::register_python_type(module.get(), type_name_KeypadPressedEventArgs, &type_spec_KeypadPressedEventArgs, bases.get(), nullptr);
    if (!state->type_KeypadPressedEventArgs)
    {
        return nullptr;
    }

    state->type_LowLagPhotoControl = py::register_python_type(module.get(), type_name_LowLagPhotoControl, &type_spec_LowLagPhotoControl, bases.get(), nullptr);
    if (!state->type_LowLagPhotoControl)
    {
        return nullptr;
    }

    state->type_LowLagPhotoSequenceControl = py::register_python_type(module.get(), type_name_LowLagPhotoSequenceControl, &type_spec_LowLagPhotoSequenceControl, bases.get(), nullptr);
    if (!state->type_LowLagPhotoSequenceControl)
    {
        return nullptr;
    }

    state->type_MediaDevice = py::register_python_type(module.get(), type_name_MediaDevice, &type_spec_MediaDevice, nullptr, nullptr);
    if (!state->type_MediaDevice)
    {
        return nullptr;
    }

    state->type_MediaDeviceControl = py::register_python_type(module.get(), type_name_MediaDeviceControl, &type_spec_MediaDeviceControl, bases.get(), nullptr);
    if (!state->type_MediaDeviceControl)
    {
        return nullptr;
    }

    state->type_MediaDeviceControlCapabilities = py::register_python_type(module.get(), type_name_MediaDeviceControlCapabilities, &type_spec_MediaDeviceControlCapabilities, bases.get(), nullptr);
    if (!state->type_MediaDeviceControlCapabilities)
    {
        return nullptr;
    }

    state->type_ModuleCommandResult = py::register_python_type(module.get(), type_name_ModuleCommandResult, &type_spec_ModuleCommandResult, bases.get(), nullptr);
    if (!state->type_ModuleCommandResult)
    {
        return nullptr;
    }

    state->type_OpticalImageStabilizationControl = py::register_python_type(module.get(), type_name_OpticalImageStabilizationControl, &type_spec_OpticalImageStabilizationControl, bases.get(), nullptr);
    if (!state->type_OpticalImageStabilizationControl)
    {
        return nullptr;
    }

    state->type_PanelBasedOptimizationControl = py::register_python_type(module.get(), type_name_PanelBasedOptimizationControl, &type_spec_PanelBasedOptimizationControl, bases.get(), nullptr);
    if (!state->type_PanelBasedOptimizationControl)
    {
        return nullptr;
    }

    state->type_PhotoConfirmationControl = py::register_python_type(module.get(), type_name_PhotoConfirmationControl, &type_spec_PhotoConfirmationControl, bases.get(), nullptr);
    if (!state->type_PhotoConfirmationControl)
    {
        return nullptr;
    }

    state->type_RedialRequestedEventArgs = py::register_python_type(module.get(), type_name_RedialRequestedEventArgs, &type_spec_RedialRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_RedialRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_RegionOfInterest = py::register_python_type(module.get(), type_name_RegionOfInterest, &type_spec_RegionOfInterest, bases.get(), nullptr);
    if (!state->type_RegionOfInterest)
    {
        return nullptr;
    }

    state->type_RegionsOfInterestControl = py::register_python_type(module.get(), type_name_RegionsOfInterestControl, &type_spec_RegionsOfInterestControl, bases.get(), nullptr);
    if (!state->type_RegionsOfInterestControl)
    {
        return nullptr;
    }

    state->type_SceneModeControl = py::register_python_type(module.get(), type_name_SceneModeControl, &type_spec_SceneModeControl, bases.get(), nullptr);
    if (!state->type_SceneModeControl)
    {
        return nullptr;
    }

    state->type_TorchControl = py::register_python_type(module.get(), type_name_TorchControl, &type_spec_TorchControl, bases.get(), nullptr);
    if (!state->type_TorchControl)
    {
        return nullptr;
    }

    state->type_VideoDeviceController = py::register_python_type(module.get(), type_name_VideoDeviceController, &type_spec_VideoDeviceController, bases.get(), nullptr);
    if (!state->type_VideoDeviceController)
    {
        return nullptr;
    }

    state->type_VideoDeviceControllerGetDevicePropertyResult = py::register_python_type(module.get(), type_name_VideoDeviceControllerGetDevicePropertyResult, &type_spec_VideoDeviceControllerGetDevicePropertyResult, bases.get(), nullptr);
    if (!state->type_VideoDeviceControllerGetDevicePropertyResult)
    {
        return nullptr;
    }

    state->type_VideoTemporalDenoisingControl = py::register_python_type(module.get(), type_name_VideoTemporalDenoisingControl, &type_spec_VideoTemporalDenoisingControl, bases.get(), nullptr);
    if (!state->type_VideoTemporalDenoisingControl)
    {
        return nullptr;
    }

    state->type_WhiteBalanceControl = py::register_python_type(module.get(), type_name_WhiteBalanceControl, &type_spec_WhiteBalanceControl, bases.get(), nullptr);
    if (!state->type_WhiteBalanceControl)
    {
        return nullptr;
    }

    state->type_ZoomControl = py::register_python_type(module.get(), type_name_ZoomControl, &type_spec_ZoomControl, bases.get(), nullptr);
    if (!state->type_ZoomControl)
    {
        return nullptr;
    }

    state->type_ZoomSettings = py::register_python_type(module.get(), type_name_ZoomSettings, &type_spec_ZoomSettings, bases.get(), nullptr);
    if (!state->type_ZoomSettings)
    {
        return nullptr;
    }

    state->type_IDefaultAudioDeviceChangedEventArgs = py::register_python_type(module.get(), type_name_IDefaultAudioDeviceChangedEventArgs, &type_spec_IDefaultAudioDeviceChangedEventArgs, bases.get(), nullptr);
    if (!state->type_IDefaultAudioDeviceChangedEventArgs)
    {
        return nullptr;
    }

    state->type_IMediaDeviceController = py::register_python_type(module.get(), type_name_IMediaDeviceController, &type_spec_IMediaDeviceController, bases.get(), nullptr);

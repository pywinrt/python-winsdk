...
namespace py::cpp::Windows::Security::EnterpriseData
{
    ...
    static PyType_Slot type_slots_ProtectionPolicyManager_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_ProtectionPolicyManager_Meta },
        { }
    };

    static PyType_Spec type_spec_ProtectionPolicyManager_Meta =
    {
        "_winsdk_Windows_Security_EnterpriseData.ProtectionPolicyManager_Meta",
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Security_EnterpriseData(void) noexcept
{
    ...
    state->type_BufferProtectUnprotectResult = py::register_python_type(module.get(), type_name_BufferProtectUnprotectResult, &type_spec_BufferProtectUnprotectResult, bases.get(), nullptr);
    if (!state->type_BufferProtectUnprotectResult)
    {
        return nullptr;
    }

    state->type_DataProtectionInfo = py::register_python_type(module.get(), type_name_DataProtectionInfo, &type_spec_DataProtectionInfo, bases.get(), nullptr);
    if (!state->type_DataProtectionInfo)
    {
        return nullptr;
    }

    state->type_DataProtectionManager = py::register_python_type(module.get(), type_name_DataProtectionManager, &type_spec_DataProtectionManager, nullptr, nullptr);
    if (!state->type_DataProtectionManager)
    {
        return nullptr;
    }

    state->type_FileProtectionInfo = py::register_python_type(module.get(), type_name_FileProtectionInfo, &type_spec_FileProtectionInfo, bases.get(), nullptr);
    if (!state->type_FileProtectionInfo)
    {
        return nullptr;
    }

    state->type_FileProtectionManager = py::register_python_type(module.get(), type_name_FileProtectionManager, &type_spec_FileProtectionManager, nullptr, nullptr);
    if (!state->type_FileProtectionManager)
    {
        return nullptr;
    }

    state->type_FileRevocationManager = py::register_python_type(module.get(), type_name_FileRevocationManager, &type_spec_FileRevocationManager, nullptr, nullptr);
    if (!state->type_FileRevocationManager)
    {
        return nullptr;
    }

    state->type_FileUnprotectOptions = py::register_python_type(module.get(), type_name_FileUnprotectOptions, &type_spec_FileUnprotectOptions, bases.get(), nullptr);
    if (!state->type_FileUnprotectOptions)
    {
        return nullptr;
    }

    state->type_ProtectedAccessResumedEventArgs = py::register_python_type(module.get(), type_name_ProtectedAccessResumedEventArgs, &type_spec_ProtectedAccessResumedEventArgs, bases.get(), nullptr);
    if (!state->type_ProtectedAccessResumedEventArgs)
    {
        return nullptr;
    }

    state->type_ProtectedAccessSuspendingEventArgs = py::register_python_type(module.get(), type_name_ProtectedAccessSuspendingEventArgs, &type_spec_ProtectedAccessSuspendingEventArgs, bases.get(), nullptr);
    if (!state->type_ProtectedAccessSuspendingEventArgs)
    {
        return nullptr;
    }

    state->type_ProtectedContainerExportResult = py::register_python_type(module.get(), type_name_ProtectedContainerExportResult, &type_spec_ProtectedContainerExportResult, bases.get(), nullptr);
    if (!state->type_ProtectedContainerExportResult)
    {
        return nullptr;
    }

    state->type_ProtectedContainerImportResult = py::register_python_type(module.get(), type_name_ProtectedContainerImportResult, &type_spec_ProtectedContainerImportResult, bases.get(), nullptr);
    if (!state->type_ProtectedContainerImportResult)
    {
        return nullptr;
    }

    state->type_ProtectedContentRevokedEventArgs = py::register_python_type(module.get(), type_name_ProtectedContentRevokedEventArgs, &type_spec_ProtectedContentRevokedEventArgs, bases.get(), nullptr);
    if (!state->type_ProtectedContentRevokedEventArgs)
    {
        return nullptr;
    }

    state->type_ProtectedFileCreateResult = py::register_python_type(module.get(), type_name_ProtectedFileCreateResult, &type_spec_ProtectedFileCreateResult, bases.get(), nullptr);
    if (!state->type_ProtectedFileCreateResult)
    {
        return nullptr;
    }

    state->type_ProtectionPolicyAuditInfo = py::register_python_type(module.get(), type_name_ProtectionPolicyAuditInfo, &type_spec_ProtectionPolicyAuditInfo, bases.get(), nullptr);
    ...
    state->type_ProtectionPolicyManager = py::register_python_type(module.get(), type_name_ProtectionPolicyManager, &type_spec_ProtectionPolicyManager, bases.get(), reinterpret_cast<PyTypeObject*>(type_ProtectionPolicyManager_Meta.get()));
    if (!state->type_ProtectionPolicyManager)
    {
        return nullptr;
    }

    state->type_ThreadNetworkContext = py::register_python_type(module.get(), type_name_ThreadNetworkContext, &type_spec_ThreadNetworkContext, bases.get(), nullptr);

...
namespace py::cpp::Windows::ApplicationModel::Resources::Core
{
    ...
    static PyType_Slot _type_slots_ResourceCandidateVectorView[] = 
    {
        ...
        { Py_mp_subscript, _seq_subscript_ResourceCandidateVectorView },
        { },
    };
    ...
    static PyType_Slot _type_slots_ResourceContextLanguagesVectorView[] = 
    {
        ...
        { Py_mp_subscript, _seq_subscript_ResourceContextLanguagesVectorView },
        { },
    };
    ...
    static PyType_Slot type_slots_ResourceManager_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_ResourceManager_Meta },
        { }
    };

    static PyType_Spec type_spec_ResourceManager_Meta =
    {
        "_winsdk_Windows_ApplicationModel_Resources_Core.ResourceManager_Meta",
    ...
    static PyType_Slot _type_slots_ResourceQualifierVectorView[] = 
    {
        ...
        { Py_mp_subscript, _seq_subscript_ResourceQualifierVectorView },
        { },
    };
...
PyMODINIT_FUNC PyInit__winsdk_Windows_ApplicationModel_Resources_Core(void) noexcept
{
    ...
    state->type_NamedResource = py::register_python_type(module.get(), type_name_NamedResource, &type_spec_NamedResource, bases.get(), nullptr);
    if (!state->type_NamedResource)
    {
        return nullptr;
    }

    state->type_ResourceCandidate = py::register_python_type(module.get(), type_name_ResourceCandidate, &type_spec_ResourceCandidate, bases.get(), nullptr);
    if (!state->type_ResourceCandidate)
    {
        return nullptr;
    }

    state->type_ResourceCandidateVectorView = py::register_python_type(module.get(), type_name_ResourceCandidateVectorView, &type_spec_ResourceCandidateVectorView, sequence_bases.get(), nullptr);
    if (!state->type_ResourceCandidateVectorView)
    {
        return nullptr;
    }

    state->type_ResourceContext = py::register_python_type(module.get(), type_name_ResourceContext, &type_spec_ResourceContext, bases.get(), nullptr);
    if (!state->type_ResourceContext)
    {
        return nullptr;
    }

    state->type_ResourceContextLanguagesVectorView = py::register_python_type(module.get(), type_name_ResourceContextLanguagesVectorView, &type_spec_ResourceContextLanguagesVectorView, sequence_bases.get(), nullptr);
    ...
    state->type_ResourceManager = py::register_python_type(module.get(), type_name_ResourceManager, &type_spec_ResourceManager, bases.get(), reinterpret_cast<PyTypeObject*>(type_ResourceManager_Meta.get()));
    if (!state->type_ResourceManager)
    {
        return nullptr;
    }

    state->type_ResourceMap = py::register_python_type(module.get(), type_name_ResourceMap, &type_spec_ResourceMap, mapping_bases.get(), nullptr);
    if (!state->type_ResourceMap)
    {
        return nullptr;
    }

    state->type_ResourceMapIterator = py::register_python_type(module.get(), type_name_ResourceMapIterator, &type_spec_ResourceMapIterator, bases.get(), nullptr);
    if (!state->type_ResourceMapIterator)
    {
        return nullptr;
    }

    state->type_ResourceMapMapView = py::register_python_type(module.get(), type_name_ResourceMapMapView, &type_spec_ResourceMapMapView, mapping_bases.get(), nullptr);
    if (!state->type_ResourceMapMapView)
    {
        return nullptr;
    }

    state->type_ResourceMapMapViewIterator = py::register_python_type(module.get(), type_name_ResourceMapMapViewIterator, &type_spec_ResourceMapMapViewIterator, bases.get(), nullptr);
    if (!state->type_ResourceMapMapViewIterator)
    {
        return nullptr;
    }

    state->type_ResourceQualifier = py::register_python_type(module.get(), type_name_ResourceQualifier, &type_spec_ResourceQualifier, bases.get(), nullptr);
    if (!state->type_ResourceQualifier)
    {
        return nullptr;
    }

    state->type_ResourceQualifierMapView = py::register_python_type(module.get(), type_name_ResourceQualifierMapView, &type_spec_ResourceQualifierMapView, mapping_bases.get(), nullptr);
    if (!state->type_ResourceQualifierMapView)
    {
        return nullptr;
    }

    state->type_ResourceQualifierObservableMap = py::register_python_type(module.get(), type_name_ResourceQualifierObservableMap, &type_spec_ResourceQualifierObservableMap, mutable_mapping_bases.get(), nullptr);
    if (!state->type_ResourceQualifierObservableMap)
    {
        return nullptr;
    }

    state->type_ResourceQualifierVectorView = py::register_python_type(module.get(), type_name_ResourceQualifierVectorView, &type_spec_ResourceQualifierVectorView, sequence_bases.get(), nullptr);
    if (!state->type_ResourceQualifierVectorView)
    {
        return nullptr;
    }

    state->type_ResourceLayoutInfo = py::register_python_type(module.get(), type_name_ResourceLayoutInfo, &type_spec_ResourceLayoutInfo, bases.get(), nullptr);

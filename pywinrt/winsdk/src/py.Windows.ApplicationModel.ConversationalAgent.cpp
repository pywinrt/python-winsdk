...
namespace py::cpp::Windows::ApplicationModel::ConversationalAgent
{
    ...
    static PyType_Slot type_slots_ConversationalAgentDetectorManager_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_ConversationalAgentDetectorManager_Meta },
        { }
    };

    static PyType_Spec type_spec_ConversationalAgentDetectorManager_Meta =
    {
        "_winsdk_Windows_ApplicationModel_ConversationalAgent.ConversationalAgentDetectorManager_Meta",
...
PyMODINIT_FUNC PyInit__winsdk_Windows_ApplicationModel_ConversationalAgent(void) noexcept
{
    ...
    state->type_ActivationSignalDetectionConfiguration = py::register_python_type(module.get(), type_name_ActivationSignalDetectionConfiguration, &type_spec_ActivationSignalDetectionConfiguration, bases.get(), nullptr);
    if (!state->type_ActivationSignalDetectionConfiguration)
    {
        return nullptr;
    }

    state->type_ActivationSignalDetectionConfigurationCreationResult = py::register_python_type(module.get(), type_name_ActivationSignalDetectionConfigurationCreationResult, &type_spec_ActivationSignalDetectionConfigurationCreationResult, bases.get(), nullptr);
    if (!state->type_ActivationSignalDetectionConfigurationCreationResult)
    {
        return nullptr;
    }

    state->type_ActivationSignalDetector = py::register_python_type(module.get(), type_name_ActivationSignalDetector, &type_spec_ActivationSignalDetector, bases.get(), nullptr);
    ...
    state->type_ConversationalAgentDetectorManager = py::register_python_type(module.get(), type_name_ConversationalAgentDetectorManager, &type_spec_ConversationalAgentDetectorManager, bases.get(), reinterpret_cast<PyTypeObject*>(type_ConversationalAgentDetectorManager_Meta.get()));
    if (!state->type_ConversationalAgentDetectorManager)
    {
        return nullptr;
    }

    state->type_ConversationalAgentSession = py::register_python_type(module.get(), type_name_ConversationalAgentSession, &type_spec_ConversationalAgentSession, bases.get(), nullptr);
    if (!state->type_ConversationalAgentSession)
    {
        return nullptr;
    }

    state->type_ConversationalAgentSessionInterruptedEventArgs = py::register_python_type(module.get(), type_name_ConversationalAgentSessionInterruptedEventArgs, &type_spec_ConversationalAgentSessionInterruptedEventArgs, bases.get(), nullptr);
    if (!state->type_ConversationalAgentSessionInterruptedEventArgs)
    {
        return nullptr;
    }

    state->type_ConversationalAgentSignal = py::register_python_type(module.get(), type_name_ConversationalAgentSignal, &type_spec_ConversationalAgentSignal, bases.get(), nullptr);
    if (!state->type_ConversationalAgentSignal)
    {
        return nullptr;
    }

    state->type_ConversationalAgentSignalDetectedEventArgs = py::register_python_type(module.get(), type_name_ConversationalAgentSignalDetectedEventArgs, &type_spec_ConversationalAgentSignalDetectedEventArgs, bases.get(), nullptr);
    if (!state->type_ConversationalAgentSignalDetectedEventArgs)
    {
        return nullptr;
    }

    state->type_ConversationalAgentSystemStateChangedEventArgs = py::register_python_type(module.get(), type_name_ConversationalAgentSystemStateChangedEventArgs, &type_spec_ConversationalAgentSystemStateChangedEventArgs, bases.get(), nullptr);
    if (!state->type_ConversationalAgentSystemStateChangedEventArgs)
    {
        return nullptr;
    }

    state->type_DetectionConfigurationAvailabilityChangedEventArgs = py::register_python_type(module.get(), type_name_DetectionConfigurationAvailabilityChangedEventArgs, &type_spec_DetectionConfigurationAvailabilityChangedEventArgs, bases.get(), nullptr);
    if (!state->type_DetectionConfigurationAvailabilityChangedEventArgs)
    {
        return nullptr;
    }

    state->type_DetectionConfigurationAvailabilityInfo = py::register_python_type(module.get(), type_name_DetectionConfigurationAvailabilityInfo, &type_spec_DetectionConfigurationAvailabilityInfo, bases.get(), nullptr);

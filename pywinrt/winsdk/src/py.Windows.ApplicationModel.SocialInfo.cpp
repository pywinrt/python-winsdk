...
PyMODINIT_FUNC PyInit__winsdk_Windows_ApplicationModel_SocialInfo(void) noexcept
{
    ...
    state->type_SocialFeedChildItem = py::register_python_type(module.get(), type_name_SocialFeedChildItem, &type_spec_SocialFeedChildItem, bases.get(), nullptr);
    if (!state->type_SocialFeedChildItem)
    {
        return nullptr;
    }

    state->type_SocialFeedContent = py::register_python_type(module.get(), type_name_SocialFeedContent, &type_spec_SocialFeedContent, bases.get(), nullptr);
    if (!state->type_SocialFeedContent)
    {
        return nullptr;
    }

    state->type_SocialFeedItem = py::register_python_type(module.get(), type_name_SocialFeedItem, &type_spec_SocialFeedItem, bases.get(), nullptr);
    if (!state->type_SocialFeedItem)
    {
        return nullptr;
    }

    state->type_SocialFeedSharedItem = py::register_python_type(module.get(), type_name_SocialFeedSharedItem, &type_spec_SocialFeedSharedItem, bases.get(), nullptr);
    if (!state->type_SocialFeedSharedItem)
    {
        return nullptr;
    }

    state->type_SocialItemThumbnail = py::register_python_type(module.get(), type_name_SocialItemThumbnail, &type_spec_SocialItemThumbnail, bases.get(), nullptr);
    if (!state->type_SocialItemThumbnail)
    {
        return nullptr;
    }

    state->type_SocialUserInfo = py::register_python_type(module.get(), type_name_SocialUserInfo, &type_spec_SocialUserInfo, bases.get(), nullptr);

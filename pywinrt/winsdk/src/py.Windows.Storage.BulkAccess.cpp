...
PyMODINIT_FUNC PyInit__winsdk_Windows_Storage_BulkAccess(void) noexcept
{
    ...
    state->type_FileInformation = py::register_python_type(module.get(), type_name_FileInformation, &type_spec_FileInformation, bases.get(), nullptr);
    if (!state->type_FileInformation)
    {
        return nullptr;
    }

    state->type_FileInformationFactory = py::register_python_type(module.get(), type_name_FileInformationFactory, &type_spec_FileInformationFactory, bases.get(), nullptr);
    if (!state->type_FileInformationFactory)
    {
        return nullptr;
    }

    state->type_FolderInformation = py::register_python_type(module.get(), type_name_FolderInformation, &type_spec_FolderInformation, bases.get(), nullptr);
    if (!state->type_FolderInformation)
    {
        return nullptr;
    }

    state->type_IStorageItemInformation = py::register_python_type(module.get(), type_name_IStorageItemInformation, &type_spec_IStorageItemInformation, bases.get(), nullptr);

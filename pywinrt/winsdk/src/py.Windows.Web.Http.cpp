...
namespace py::cpp::Windows::Web::Http
{
    ...
    static PyObject* register_HttpCompletionOption(PyObject* module, PyObject* type)
    {
    ...
    static PyObject* register_HttpProgressStage(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_HttpResponseMessageSource(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_HttpStatusCode(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_HttpVersion(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    constexpr const char* const type_name_HttpBufferContent = "HttpBufferContent";
    ...
    // ----- HttpClient class --------------------
    constexpr const char* const type_name_HttpClient = "HttpClient";
    ...
    // ----- HttpCookie class --------------------
    constexpr const char* const type_name_HttpCookie = "HttpCookie";
    ...
    constexpr const char* const type_name_HttpCookieCollection = "HttpCookieCollection";
    ...
    static PyObject* HttpCookieCollection_GetMany(py::wrapper::Windows::Web::Http::HttpCookieCollection* self, PyObject* args) noexcept
    {
        ...
        if (arg_count == 2)
        {
            if (!winrt::Windows::Foundation::Metadata::ApiInformation::IsMethodPresent(L"Windows.Web.Http.HttpCookieCollection", L"GetMany", 2))
            {
                PyErr_SetString(PyExc_AttributeError, "method 2 args is not available in this version of Windows");
                return nullptr;
            }
    ...
    static PyObject* HttpCookieCollection_get_insert(PyObject* self) noexcept
    {
    ...
    constexpr const char* const type_name_HttpCookieManager = "HttpCookieManager";
    ...
    constexpr const char* const type_name_HttpFormUrlEncodedContent = "HttpFormUrlEncodedContent";
    ...
    constexpr const char* const type_name_HttpGetBufferResult = "HttpGetBufferResult";
    ...
    constexpr const char* const type_name_HttpGetInputStreamResult = "HttpGetInputStreamResult";
    ...
    constexpr const char* const type_name_HttpGetStringResult = "HttpGetStringResult";
    ...
    // ----- HttpMethod class --------------------
    constexpr const char* const type_name_HttpMethod = "HttpMethod";
    ...
    constexpr const char* const type_name_HttpMultipartContent = "HttpMultipartContent";
    ...
    constexpr const char* const type_name_HttpMultipartFormDataContent = "HttpMultipartFormDataContent";
    ...
    constexpr const char* const type_name_HttpRequestMessage = "HttpRequestMessage";
    ...
    constexpr const char* const type_name_HttpRequestResult = "HttpRequestResult";
    ...
    constexpr const char* const type_name_HttpResponseMessage = "HttpResponseMessage";
    ...
    constexpr const char* const type_name_HttpStreamContent = "HttpStreamContent";
    ...
    constexpr const char* const type_name_HttpStringContent = "HttpStringContent";
    ...
    constexpr const char* const type_name_HttpTransportInformation = "HttpTransportInformation";
    ...
    constexpr const char* const type_name_IHttpContent = "IHttpContent";
    ...
    // ----- HttpProgress struct --------------------
    constexpr const char* const type_name_HttpProgress = "HttpProgress";
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Web_Http(void) noexcept
{
    ...
    state->type_HttpBufferContent = py::register_python_type(module.get(), type_name_HttpBufferContent, &type_spec_HttpBufferContent, bases.get());
    if (!state->type_HttpBufferContent)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpBufferContent);

    state->type_HttpClient = py::register_python_type(module.get(), type_name_HttpClient, &type_spec_HttpClient, bases.get());
    if (!state->type_HttpClient)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpClient);

    state->type_HttpCookie = py::register_python_type(module.get(), type_name_HttpCookie, &type_spec_HttpCookie, bases.get());
    if (!state->type_HttpCookie)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpCookie);

    state->type_HttpCookieCollection = py::register_python_type(module.get(), type_name_HttpCookieCollection, &type_spec_HttpCookieCollection, sequence_bases.get());
    if (!state->type_HttpCookieCollection)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpCookieCollection);

    state->type_HttpCookieManager = py::register_python_type(module.get(), type_name_HttpCookieManager, &type_spec_HttpCookieManager, bases.get());
    if (!state->type_HttpCookieManager)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpCookieManager);

    state->type_HttpFormUrlEncodedContent = py::register_python_type(module.get(), type_name_HttpFormUrlEncodedContent, &type_spec_HttpFormUrlEncodedContent, bases.get());
    if (!state->type_HttpFormUrlEncodedContent)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpFormUrlEncodedContent);

    state->type_HttpGetBufferResult = py::register_python_type(module.get(), type_name_HttpGetBufferResult, &type_spec_HttpGetBufferResult, bases.get());
    if (!state->type_HttpGetBufferResult)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpGetBufferResult);

    state->type_HttpGetInputStreamResult = py::register_python_type(module.get(), type_name_HttpGetInputStreamResult, &type_spec_HttpGetInputStreamResult, bases.get());
    if (!state->type_HttpGetInputStreamResult)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpGetInputStreamResult);

    state->type_HttpGetStringResult = py::register_python_type(module.get(), type_name_HttpGetStringResult, &type_spec_HttpGetStringResult, bases.get());
    if (!state->type_HttpGetStringResult)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpGetStringResult);

    state->type_HttpMethod = py::register_python_type(module.get(), type_name_HttpMethod, &type_spec_HttpMethod, bases.get());
    if (!state->type_HttpMethod)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpMethod);

    state->type_HttpMultipartContent = py::register_python_type(module.get(), type_name_HttpMultipartContent, &type_spec_HttpMultipartContent, bases.get());
    if (!state->type_HttpMultipartContent)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpMultipartContent);

    state->type_HttpMultipartFormDataContent = py::register_python_type(module.get(), type_name_HttpMultipartFormDataContent, &type_spec_HttpMultipartFormDataContent, bases.get());
    if (!state->type_HttpMultipartFormDataContent)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpMultipartFormDataContent);

    state->type_HttpRequestMessage = py::register_python_type(module.get(), type_name_HttpRequestMessage, &type_spec_HttpRequestMessage, bases.get());
    if (!state->type_HttpRequestMessage)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpRequestMessage);

    state->type_HttpRequestResult = py::register_python_type(module.get(), type_name_HttpRequestResult, &type_spec_HttpRequestResult, bases.get());
    if (!state->type_HttpRequestResult)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpRequestResult);

    state->type_HttpResponseMessage = py::register_python_type(module.get(), type_name_HttpResponseMessage, &type_spec_HttpResponseMessage, bases.get());
    if (!state->type_HttpResponseMessage)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpResponseMessage);

    state->type_HttpStreamContent = py::register_python_type(module.get(), type_name_HttpStreamContent, &type_spec_HttpStreamContent, bases.get());
    if (!state->type_HttpStreamContent)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpStreamContent);

    state->type_HttpStringContent = py::register_python_type(module.get(), type_name_HttpStringContent, &type_spec_HttpStringContent, bases.get());
    if (!state->type_HttpStringContent)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpStringContent);

    state->type_HttpTransportInformation = py::register_python_type(module.get(), type_name_HttpTransportInformation, &type_spec_HttpTransportInformation, bases.get());
    if (!state->type_HttpTransportInformation)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpTransportInformation);

    state->type_IHttpContent = py::register_python_type(module.get(), type_name_IHttpContent, &type_spec_IHttpContent, bases.get());
    if (!state->type_IHttpContent)
    {
        return nullptr;
    }

    Py_INCREF(state->type_IHttpContent);

    state->type_HttpProgress = py::register_python_type(module.get(), type_name_HttpProgress, &type_spec_HttpProgress, bases.get());
    if (!state->type_HttpProgress)
    {
        return nullptr;
    }

    Py_INCREF(state->type_HttpProgress);

...
namespace py::cpp::Windows::Web::Http
{
    ...
    static PyType_Slot _type_slots_HttpCookieCollection[] = 
    {
        ...
        { Py_mp_subscript, _seq_subscript_HttpCookieCollection },
        { },
    };
    ...
    static PyType_Slot type_slots_HttpMethod_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_HttpMethod_Meta },
        { }
    };

    static PyType_Spec type_spec_HttpMethod_Meta =
    {
        "_winsdk_Windows_Web_Http.HttpMethod_Meta",
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Web_Http(void) noexcept
{
    ...
    state->type_HttpBufferContent = py::register_python_type(module.get(), type_name_HttpBufferContent, &type_spec_HttpBufferContent, bases.get(), nullptr);
    if (!state->type_HttpBufferContent)
    {
        return nullptr;
    }

    state->type_HttpClient = py::register_python_type(module.get(), type_name_HttpClient, &type_spec_HttpClient, bases.get(), nullptr);
    if (!state->type_HttpClient)
    {
        return nullptr;
    }

    state->type_HttpCookie = py::register_python_type(module.get(), type_name_HttpCookie, &type_spec_HttpCookie, bases.get(), nullptr);
    if (!state->type_HttpCookie)
    {
        return nullptr;
    }

    state->type_HttpCookieCollection = py::register_python_type(module.get(), type_name_HttpCookieCollection, &type_spec_HttpCookieCollection, sequence_bases.get(), nullptr);
    if (!state->type_HttpCookieCollection)
    {
        return nullptr;
    }

    state->type_HttpCookieManager = py::register_python_type(module.get(), type_name_HttpCookieManager, &type_spec_HttpCookieManager, bases.get(), nullptr);
    if (!state->type_HttpCookieManager)
    {
        return nullptr;
    }

    state->type_HttpFormUrlEncodedContent = py::register_python_type(module.get(), type_name_HttpFormUrlEncodedContent, &type_spec_HttpFormUrlEncodedContent, bases.get(), nullptr);
    if (!state->type_HttpFormUrlEncodedContent)
    {
        return nullptr;
    }

    state->type_HttpGetBufferResult = py::register_python_type(module.get(), type_name_HttpGetBufferResult, &type_spec_HttpGetBufferResult, bases.get(), nullptr);
    if (!state->type_HttpGetBufferResult)
    {
        return nullptr;
    }

    state->type_HttpGetInputStreamResult = py::register_python_type(module.get(), type_name_HttpGetInputStreamResult, &type_spec_HttpGetInputStreamResult, bases.get(), nullptr);
    if (!state->type_HttpGetInputStreamResult)
    {
        return nullptr;
    }

    state->type_HttpGetStringResult = py::register_python_type(module.get(), type_name_HttpGetStringResult, &type_spec_HttpGetStringResult, bases.get(), nullptr);
    ...
    state->type_HttpMethod = py::register_python_type(module.get(), type_name_HttpMethod, &type_spec_HttpMethod, bases.get(), reinterpret_cast<PyTypeObject*>(type_HttpMethod_Meta.get()));
    if (!state->type_HttpMethod)
    {
        return nullptr;
    }

    state->type_HttpMultipartContent = py::register_python_type(module.get(), type_name_HttpMultipartContent, &type_spec_HttpMultipartContent, bases.get(), nullptr);
    if (!state->type_HttpMultipartContent)
    {
        return nullptr;
    }

    state->type_HttpMultipartFormDataContent = py::register_python_type(module.get(), type_name_HttpMultipartFormDataContent, &type_spec_HttpMultipartFormDataContent, bases.get(), nullptr);
    if (!state->type_HttpMultipartFormDataContent)
    {
        return nullptr;
    }

    state->type_HttpRequestMessage = py::register_python_type(module.get(), type_name_HttpRequestMessage, &type_spec_HttpRequestMessage, bases.get(), nullptr);
    if (!state->type_HttpRequestMessage)
    {
        return nullptr;
    }

    state->type_HttpRequestResult = py::register_python_type(module.get(), type_name_HttpRequestResult, &type_spec_HttpRequestResult, bases.get(), nullptr);
    if (!state->type_HttpRequestResult)
    {
        return nullptr;
    }

    state->type_HttpResponseMessage = py::register_python_type(module.get(), type_name_HttpResponseMessage, &type_spec_HttpResponseMessage, bases.get(), nullptr);
    if (!state->type_HttpResponseMessage)
    {
        return nullptr;
    }

    state->type_HttpStreamContent = py::register_python_type(module.get(), type_name_HttpStreamContent, &type_spec_HttpStreamContent, bases.get(), nullptr);
    if (!state->type_HttpStreamContent)
    {
        return nullptr;
    }

    state->type_HttpStringContent = py::register_python_type(module.get(), type_name_HttpStringContent, &type_spec_HttpStringContent, bases.get(), nullptr);
    if (!state->type_HttpStringContent)
    {
        return nullptr;
    }

    state->type_HttpTransportInformation = py::register_python_type(module.get(), type_name_HttpTransportInformation, &type_spec_HttpTransportInformation, bases.get(), nullptr);
    if (!state->type_HttpTransportInformation)
    {
        return nullptr;
    }

    state->type_IHttpContent = py::register_python_type(module.get(), type_name_IHttpContent, &type_spec_IHttpContent, bases.get(), nullptr);
    if (!state->type_IHttpContent)
    {
        return nullptr;
    }

    state->type_HttpProgress = py::register_python_type(module.get(), type_name_HttpProgress, &type_spec_HttpProgress, bases.get(), nullptr);

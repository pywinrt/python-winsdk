...
PyMODINIT_FUNC PyInit__winsdk_Windows_UI_Input_Inking(void) noexcept
{
    ...
    state->type_InkDrawingAttributes = py::register_python_type(module.get(), type_name_InkDrawingAttributes, &type_spec_InkDrawingAttributes, bases.get(), nullptr);
    if (!state->type_InkDrawingAttributes)
    {
        return nullptr;
    }

    state->type_InkDrawingAttributesPencilProperties = py::register_python_type(module.get(), type_name_InkDrawingAttributesPencilProperties, &type_spec_InkDrawingAttributesPencilProperties, bases.get(), nullptr);
    if (!state->type_InkDrawingAttributesPencilProperties)
    {
        return nullptr;
    }

    state->type_InkInputConfiguration = py::register_python_type(module.get(), type_name_InkInputConfiguration, &type_spec_InkInputConfiguration, bases.get(), nullptr);
    if (!state->type_InkInputConfiguration)
    {
        return nullptr;
    }

    state->type_InkInputProcessingConfiguration = py::register_python_type(module.get(), type_name_InkInputProcessingConfiguration, &type_spec_InkInputProcessingConfiguration, bases.get(), nullptr);
    if (!state->type_InkInputProcessingConfiguration)
    {
        return nullptr;
    }

    state->type_InkManager = py::register_python_type(module.get(), type_name_InkManager, &type_spec_InkManager, bases.get(), nullptr);
    if (!state->type_InkManager)
    {
        return nullptr;
    }

    state->type_InkModelerAttributes = py::register_python_type(module.get(), type_name_InkModelerAttributes, &type_spec_InkModelerAttributes, bases.get(), nullptr);
    if (!state->type_InkModelerAttributes)
    {
        return nullptr;
    }

    state->type_InkPoint = py::register_python_type(module.get(), type_name_InkPoint, &type_spec_InkPoint, bases.get(), nullptr);
    if (!state->type_InkPoint)
    {
        return nullptr;
    }

    state->type_InkPresenter = py::register_python_type(module.get(), type_name_InkPresenter, &type_spec_InkPresenter, bases.get(), nullptr);
    if (!state->type_InkPresenter)
    {
        return nullptr;
    }

    state->type_InkPresenterProtractor = py::register_python_type(module.get(), type_name_InkPresenterProtractor, &type_spec_InkPresenterProtractor, bases.get(), nullptr);
    if (!state->type_InkPresenterProtractor)
    {
        return nullptr;
    }

    state->type_InkPresenterRuler = py::register_python_type(module.get(), type_name_InkPresenterRuler, &type_spec_InkPresenterRuler, bases.get(), nullptr);
    if (!state->type_InkPresenterRuler)
    {
        return nullptr;
    }

    state->type_InkRecognitionResult = py::register_python_type(module.get(), type_name_InkRecognitionResult, &type_spec_InkRecognitionResult, bases.get(), nullptr);
    if (!state->type_InkRecognitionResult)
    {
        return nullptr;
    }

    state->type_InkRecognizer = py::register_python_type(module.get(), type_name_InkRecognizer, &type_spec_InkRecognizer, bases.get(), nullptr);
    if (!state->type_InkRecognizer)
    {
        return nullptr;
    }

    state->type_InkRecognizerContainer = py::register_python_type(module.get(), type_name_InkRecognizerContainer, &type_spec_InkRecognizerContainer, bases.get(), nullptr);
    if (!state->type_InkRecognizerContainer)
    {
        return nullptr;
    }

    state->type_InkStroke = py::register_python_type(module.get(), type_name_InkStroke, &type_spec_InkStroke, bases.get(), nullptr);
    if (!state->type_InkStroke)
    {
        return nullptr;
    }

    state->type_InkStrokeBuilder = py::register_python_type(module.get(), type_name_InkStrokeBuilder, &type_spec_InkStrokeBuilder, bases.get(), nullptr);
    if (!state->type_InkStrokeBuilder)
    {
        return nullptr;
    }

    state->type_InkStrokeContainer = py::register_python_type(module.get(), type_name_InkStrokeContainer, &type_spec_InkStrokeContainer, bases.get(), nullptr);
    if (!state->type_InkStrokeContainer)
    {
        return nullptr;
    }

    state->type_InkStrokeInput = py::register_python_type(module.get(), type_name_InkStrokeInput, &type_spec_InkStrokeInput, bases.get(), nullptr);
    if (!state->type_InkStrokeInput)
    {
        return nullptr;
    }

    state->type_InkStrokeRenderingSegment = py::register_python_type(module.get(), type_name_InkStrokeRenderingSegment, &type_spec_InkStrokeRenderingSegment, bases.get(), nullptr);
    if (!state->type_InkStrokeRenderingSegment)
    {
        return nullptr;
    }

    state->type_InkStrokesCollectedEventArgs = py::register_python_type(module.get(), type_name_InkStrokesCollectedEventArgs, &type_spec_InkStrokesCollectedEventArgs, bases.get(), nullptr);
    if (!state->type_InkStrokesCollectedEventArgs)
    {
        return nullptr;
    }

    state->type_InkStrokesErasedEventArgs = py::register_python_type(module.get(), type_name_InkStrokesErasedEventArgs, &type_spec_InkStrokesErasedEventArgs, bases.get(), nullptr);
    if (!state->type_InkStrokesErasedEventArgs)
    {
        return nullptr;
    }

    state->type_InkSynchronizer = py::register_python_type(module.get(), type_name_InkSynchronizer, &type_spec_InkSynchronizer, bases.get(), nullptr);
    if (!state->type_InkSynchronizer)
    {
        return nullptr;
    }

    state->type_InkUnprocessedInput = py::register_python_type(module.get(), type_name_InkUnprocessedInput, &type_spec_InkUnprocessedInput, bases.get(), nullptr);
    if (!state->type_InkUnprocessedInput)
    {
        return nullptr;
    }

    state->type_PenAndInkSettings = py::register_python_type(module.get(), type_name_PenAndInkSettings, &type_spec_PenAndInkSettings, bases.get(), nullptr);
    if (!state->type_PenAndInkSettings)
    {
        return nullptr;
    }

    state->type_IInkPointFactory = py::register_python_type(module.get(), type_name_IInkPointFactory, &type_spec_IInkPointFactory, bases.get(), nullptr);
    if (!state->type_IInkPointFactory)
    {
        return nullptr;
    }

    state->type_IInkPresenterRulerFactory = py::register_python_type(module.get(), type_name_IInkPresenterRulerFactory, &type_spec_IInkPresenterRulerFactory, bases.get(), nullptr);
    if (!state->type_IInkPresenterRulerFactory)
    {
        return nullptr;
    }

    state->type_IInkPresenterStencil = py::register_python_type(module.get(), type_name_IInkPresenterStencil, &type_spec_IInkPresenterStencil, bases.get(), nullptr);
    if (!state->type_IInkPresenterStencil)
    {
        return nullptr;
    }

    state->type_IInkRecognizerContainer = py::register_python_type(module.get(), type_name_IInkRecognizerContainer, &type_spec_IInkRecognizerContainer, bases.get(), nullptr);
    if (!state->type_IInkRecognizerContainer)
    {
        return nullptr;
    }

    state->type_IInkStrokeContainer = py::register_python_type(module.get(), type_name_IInkStrokeContainer, &type_spec_IInkStrokeContainer, bases.get(), nullptr);

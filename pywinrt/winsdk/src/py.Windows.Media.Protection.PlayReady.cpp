...
namespace py::cpp::Windows::Media::Protection::PlayReady
{
    ...
    static PyType_Slot type_slots_PlayReadyStatics_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_PlayReadyStatics_Meta },
        { }
    };

    static PyType_Spec type_spec_PlayReadyStatics_Meta =
    {
        "_winsdk_Windows_Media_Protection_PlayReady.PlayReadyStatics_Meta",
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Media_Protection_PlayReady(void) noexcept
{
    ...
    state->type_NDClient = py::register_python_type(module.get(), type_name_NDClient, &type_spec_NDClient, bases.get(), nullptr);
    if (!state->type_NDClient)
    {
        return nullptr;
    }

    state->type_NDCustomData = py::register_python_type(module.get(), type_name_NDCustomData, &type_spec_NDCustomData, bases.get(), nullptr);
    if (!state->type_NDCustomData)
    {
        return nullptr;
    }

    state->type_NDDownloadEngineNotifier = py::register_python_type(module.get(), type_name_NDDownloadEngineNotifier, &type_spec_NDDownloadEngineNotifier, bases.get(), nullptr);
    if (!state->type_NDDownloadEngineNotifier)
    {
        return nullptr;
    }

    state->type_NDLicenseFetchDescriptor = py::register_python_type(module.get(), type_name_NDLicenseFetchDescriptor, &type_spec_NDLicenseFetchDescriptor, bases.get(), nullptr);
    if (!state->type_NDLicenseFetchDescriptor)
    {
        return nullptr;
    }

    state->type_NDStorageFileHelper = py::register_python_type(module.get(), type_name_NDStorageFileHelper, &type_spec_NDStorageFileHelper, bases.get(), nullptr);
    if (!state->type_NDStorageFileHelper)
    {
        return nullptr;
    }

    state->type_NDStreamParserNotifier = py::register_python_type(module.get(), type_name_NDStreamParserNotifier, &type_spec_NDStreamParserNotifier, bases.get(), nullptr);
    if (!state->type_NDStreamParserNotifier)
    {
        return nullptr;
    }

    state->type_NDTCPMessenger = py::register_python_type(module.get(), type_name_NDTCPMessenger, &type_spec_NDTCPMessenger, bases.get(), nullptr);
    if (!state->type_NDTCPMessenger)
    {
        return nullptr;
    }

    state->type_PlayReadyContentHeader = py::register_python_type(module.get(), type_name_PlayReadyContentHeader, &type_spec_PlayReadyContentHeader, bases.get(), nullptr);
    if (!state->type_PlayReadyContentHeader)
    {
        return nullptr;
    }

    state->type_PlayReadyContentResolver = py::register_python_type(module.get(), type_name_PlayReadyContentResolver, &type_spec_PlayReadyContentResolver, nullptr, nullptr);
    if (!state->type_PlayReadyContentResolver)
    {
        return nullptr;
    }

    state->type_PlayReadyDomain = py::register_python_type(module.get(), type_name_PlayReadyDomain, &type_spec_PlayReadyDomain, bases.get(), nullptr);
    if (!state->type_PlayReadyDomain)
    {
        return nullptr;
    }

    state->type_PlayReadyDomainIterable = py::register_python_type(module.get(), type_name_PlayReadyDomainIterable, &type_spec_PlayReadyDomainIterable, bases.get(), nullptr);
    if (!state->type_PlayReadyDomainIterable)
    {
        return nullptr;
    }

    state->type_PlayReadyDomainIterator = py::register_python_type(module.get(), type_name_PlayReadyDomainIterator, &type_spec_PlayReadyDomainIterator, bases.get(), nullptr);
    if (!state->type_PlayReadyDomainIterator)
    {
        return nullptr;
    }

    state->type_PlayReadyDomainJoinServiceRequest = py::register_python_type(module.get(), type_name_PlayReadyDomainJoinServiceRequest, &type_spec_PlayReadyDomainJoinServiceRequest, bases.get(), nullptr);
    if (!state->type_PlayReadyDomainJoinServiceRequest)
    {
        return nullptr;
    }

    state->type_PlayReadyDomainLeaveServiceRequest = py::register_python_type(module.get(), type_name_PlayReadyDomainLeaveServiceRequest, &type_spec_PlayReadyDomainLeaveServiceRequest, bases.get(), nullptr);
    if (!state->type_PlayReadyDomainLeaveServiceRequest)
    {
        return nullptr;
    }

    state->type_PlayReadyITADataGenerator = py::register_python_type(module.get(), type_name_PlayReadyITADataGenerator, &type_spec_PlayReadyITADataGenerator, bases.get(), nullptr);
    if (!state->type_PlayReadyITADataGenerator)
    {
        return nullptr;
    }

    state->type_PlayReadyIndividualizationServiceRequest = py::register_python_type(module.get(), type_name_PlayReadyIndividualizationServiceRequest, &type_spec_PlayReadyIndividualizationServiceRequest, bases.get(), nullptr);
    if (!state->type_PlayReadyIndividualizationServiceRequest)
    {
        return nullptr;
    }

    state->type_PlayReadyLicense = py::register_python_type(module.get(), type_name_PlayReadyLicense, &type_spec_PlayReadyLicense, bases.get(), nullptr);
    if (!state->type_PlayReadyLicense)
    {
        return nullptr;
    }

    state->type_PlayReadyLicenseAcquisitionServiceRequest = py::register_python_type(module.get(), type_name_PlayReadyLicenseAcquisitionServiceRequest, &type_spec_PlayReadyLicenseAcquisitionServiceRequest, bases.get(), nullptr);
    if (!state->type_PlayReadyLicenseAcquisitionServiceRequest)
    {
        return nullptr;
    }

    state->type_PlayReadyLicenseIterable = py::register_python_type(module.get(), type_name_PlayReadyLicenseIterable, &type_spec_PlayReadyLicenseIterable, bases.get(), nullptr);
    if (!state->type_PlayReadyLicenseIterable)
    {
        return nullptr;
    }

    state->type_PlayReadyLicenseIterator = py::register_python_type(module.get(), type_name_PlayReadyLicenseIterator, &type_spec_PlayReadyLicenseIterator, bases.get(), nullptr);
    if (!state->type_PlayReadyLicenseIterator)
    {
        return nullptr;
    }

    state->type_PlayReadyLicenseManagement = py::register_python_type(module.get(), type_name_PlayReadyLicenseManagement, &type_spec_PlayReadyLicenseManagement, nullptr, nullptr);
    if (!state->type_PlayReadyLicenseManagement)
    {
        return nullptr;
    }

    state->type_PlayReadyLicenseSession = py::register_python_type(module.get(), type_name_PlayReadyLicenseSession, &type_spec_PlayReadyLicenseSession, bases.get(), nullptr);
    if (!state->type_PlayReadyLicenseSession)
    {
        return nullptr;
    }

    state->type_PlayReadyMeteringReportServiceRequest = py::register_python_type(module.get(), type_name_PlayReadyMeteringReportServiceRequest, &type_spec_PlayReadyMeteringReportServiceRequest, bases.get(), nullptr);
    if (!state->type_PlayReadyMeteringReportServiceRequest)
    {
        return nullptr;
    }

    state->type_PlayReadyRevocationServiceRequest = py::register_python_type(module.get(), type_name_PlayReadyRevocationServiceRequest, &type_spec_PlayReadyRevocationServiceRequest, bases.get(), nullptr);
    if (!state->type_PlayReadyRevocationServiceRequest)
    {
        return nullptr;
    }

    state->type_PlayReadySecureStopIterable = py::register_python_type(module.get(), type_name_PlayReadySecureStopIterable, &type_spec_PlayReadySecureStopIterable, bases.get(), nullptr);
    if (!state->type_PlayReadySecureStopIterable)
    {
        return nullptr;
    }

    state->type_PlayReadySecureStopIterator = py::register_python_type(module.get(), type_name_PlayReadySecureStopIterator, &type_spec_PlayReadySecureStopIterator, bases.get(), nullptr);
    if (!state->type_PlayReadySecureStopIterator)
    {
        return nullptr;
    }

    state->type_PlayReadySecureStopServiceRequest = py::register_python_type(module.get(), type_name_PlayReadySecureStopServiceRequest, &type_spec_PlayReadySecureStopServiceRequest, bases.get(), nullptr);
    if (!state->type_PlayReadySecureStopServiceRequest)
    {
        return nullptr;
    }

    state->type_PlayReadySoapMessage = py::register_python_type(module.get(), type_name_PlayReadySoapMessage, &type_spec_PlayReadySoapMessage, bases.get(), nullptr);
    ...
    state->type_PlayReadyStatics = py::register_python_type(module.get(), type_name_PlayReadyStatics, &type_spec_PlayReadyStatics, nullptr, reinterpret_cast<PyTypeObject*>(type_PlayReadyStatics_Meta.get()));
    if (!state->type_PlayReadyStatics)
    {
        return nullptr;
    }

    state->type_INDClosedCaptionDataReceivedEventArgs = py::register_python_type(module.get(), type_name_INDClosedCaptionDataReceivedEventArgs, &type_spec_INDClosedCaptionDataReceivedEventArgs, bases.get(), nullptr);
    if (!state->type_INDClosedCaptionDataReceivedEventArgs)
    {
        return nullptr;
    }

    state->type_INDCustomData = py::register_python_type(module.get(), type_name_INDCustomData, &type_spec_INDCustomData, bases.get(), nullptr);
    if (!state->type_INDCustomData)
    {
        return nullptr;
    }

    state->type_INDDownloadEngine = py::register_python_type(module.get(), type_name_INDDownloadEngine, &type_spec_INDDownloadEngine, bases.get(), nullptr);
    if (!state->type_INDDownloadEngine)
    {
        return nullptr;
    }

    state->type_INDDownloadEngineNotifier = py::register_python_type(module.get(), type_name_INDDownloadEngineNotifier, &type_spec_INDDownloadEngineNotifier, bases.get(), nullptr);
    if (!state->type_INDDownloadEngineNotifier)
    {
        return nullptr;
    }

    state->type_INDLicenseFetchCompletedEventArgs = py::register_python_type(module.get(), type_name_INDLicenseFetchCompletedEventArgs, &type_spec_INDLicenseFetchCompletedEventArgs, bases.get(), nullptr);
    if (!state->type_INDLicenseFetchCompletedEventArgs)
    {
        return nullptr;
    }

    state->type_INDLicenseFetchDescriptor = py::register_python_type(module.get(), type_name_INDLicenseFetchDescriptor, &type_spec_INDLicenseFetchDescriptor, bases.get(), nullptr);
    if (!state->type_INDLicenseFetchDescriptor)
    {
        return nullptr;
    }

    state->type_INDLicenseFetchResult = py::register_python_type(module.get(), type_name_INDLicenseFetchResult, &type_spec_INDLicenseFetchResult, bases.get(), nullptr);
    if (!state->type_INDLicenseFetchResult)
    {
        return nullptr;
    }

    state->type_INDMessenger = py::register_python_type(module.get(), type_name_INDMessenger, &type_spec_INDMessenger, bases.get(), nullptr);
    if (!state->type_INDMessenger)
    {
        return nullptr;
    }

    state->type_INDProximityDetectionCompletedEventArgs = py::register_python_type(module.get(), type_name_INDProximityDetectionCompletedEventArgs, &type_spec_INDProximityDetectionCompletedEventArgs, bases.get(), nullptr);
    if (!state->type_INDProximityDetectionCompletedEventArgs)
    {
        return nullptr;
    }

    state->type_INDRegistrationCompletedEventArgs = py::register_python_type(module.get(), type_name_INDRegistrationCompletedEventArgs, &type_spec_INDRegistrationCompletedEventArgs, bases.get(), nullptr);
    if (!state->type_INDRegistrationCompletedEventArgs)
    {
        return nullptr;
    }

    state->type_INDSendResult = py::register_python_type(module.get(), type_name_INDSendResult, &type_spec_INDSendResult, bases.get(), nullptr);
    if (!state->type_INDSendResult)
    {
        return nullptr;
    }

    state->type_INDStartResult = py::register_python_type(module.get(), type_name_INDStartResult, &type_spec_INDStartResult, bases.get(), nullptr);
    if (!state->type_INDStartResult)
    {
        return nullptr;
    }

    state->type_INDStorageFileHelper = py::register_python_type(module.get(), type_name_INDStorageFileHelper, &type_spec_INDStorageFileHelper, bases.get(), nullptr);
    if (!state->type_INDStorageFileHelper)
    {
        return nullptr;
    }

    state->type_INDStreamParser = py::register_python_type(module.get(), type_name_INDStreamParser, &type_spec_INDStreamParser, bases.get(), nullptr);
    if (!state->type_INDStreamParser)
    {
        return nullptr;
    }

    state->type_INDStreamParserNotifier = py::register_python_type(module.get(), type_name_INDStreamParserNotifier, &type_spec_INDStreamParserNotifier, bases.get(), nullptr);
    if (!state->type_INDStreamParserNotifier)
    {
        return nullptr;
    }

    state->type_INDTransmitterProperties = py::register_python_type(module.get(), type_name_INDTransmitterProperties, &type_spec_INDTransmitterProperties, bases.get(), nullptr);
    if (!state->type_INDTransmitterProperties)
    {
        return nullptr;
    }

    state->type_IPlayReadyDomain = py::register_python_type(module.get(), type_name_IPlayReadyDomain, &type_spec_IPlayReadyDomain, bases.get(), nullptr);
    if (!state->type_IPlayReadyDomain)
    {
        return nullptr;
    }

    state->type_IPlayReadyLicense = py::register_python_type(module.get(), type_name_IPlayReadyLicense, &type_spec_IPlayReadyLicense, bases.get(), nullptr);
    if (!state->type_IPlayReadyLicense)
    {
        return nullptr;
    }

    state->type_IPlayReadyLicenseAcquisitionServiceRequest = py::register_python_type(module.get(), type_name_IPlayReadyLicenseAcquisitionServiceRequest, &type_spec_IPlayReadyLicenseAcquisitionServiceRequest, bases.get(), nullptr);
    if (!state->type_IPlayReadyLicenseAcquisitionServiceRequest)
    {
        return nullptr;
    }

    state->type_IPlayReadyLicenseSession = py::register_python_type(module.get(), type_name_IPlayReadyLicenseSession, &type_spec_IPlayReadyLicenseSession, bases.get(), nullptr);
    if (!state->type_IPlayReadyLicenseSession)
    {
        return nullptr;
    }

    state->type_IPlayReadyLicenseSession2 = py::register_python_type(module.get(), type_name_IPlayReadyLicenseSession2, &type_spec_IPlayReadyLicenseSession2, bases.get(), nullptr);
    if (!state->type_IPlayReadyLicenseSession2)
    {
        return nullptr;
    }

    state->type_IPlayReadySecureStopServiceRequest = py::register_python_type(module.get(), type_name_IPlayReadySecureStopServiceRequest, &type_spec_IPlayReadySecureStopServiceRequest, bases.get(), nullptr);
    if (!state->type_IPlayReadySecureStopServiceRequest)
    {
        return nullptr;
    }

    state->type_IPlayReadyServiceRequest = py::register_python_type(module.get(), type_name_IPlayReadyServiceRequest, &type_spec_IPlayReadyServiceRequest, bases.get(), nullptr);

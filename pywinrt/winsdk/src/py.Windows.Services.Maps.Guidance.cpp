...
namespace py::cpp::Windows::Services::Maps::Guidance
{
    ...
    static PyType_Slot type_slots_GuidanceNavigator_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_GuidanceNavigator_Meta },
        { }
    };

    static PyType_Spec type_spec_GuidanceNavigator_Meta =
    {
        "_winsdk_Windows_Services_Maps_Guidance.GuidanceNavigator_Meta",
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Services_Maps_Guidance(void) noexcept
{
    ...
    state->type_GuidanceAudioNotificationRequestedEventArgs = py::register_python_type(module.get(), type_name_GuidanceAudioNotificationRequestedEventArgs, &type_spec_GuidanceAudioNotificationRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_GuidanceAudioNotificationRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_GuidanceLaneInfo = py::register_python_type(module.get(), type_name_GuidanceLaneInfo, &type_spec_GuidanceLaneInfo, bases.get(), nullptr);
    if (!state->type_GuidanceLaneInfo)
    {
        return nullptr;
    }

    state->type_GuidanceManeuver = py::register_python_type(module.get(), type_name_GuidanceManeuver, &type_spec_GuidanceManeuver, bases.get(), nullptr);
    if (!state->type_GuidanceManeuver)
    {
        return nullptr;
    }

    state->type_GuidanceMapMatchedCoordinate = py::register_python_type(module.get(), type_name_GuidanceMapMatchedCoordinate, &type_spec_GuidanceMapMatchedCoordinate, bases.get(), nullptr);
    ...
    state->type_GuidanceNavigator = py::register_python_type(module.get(), type_name_GuidanceNavigator, &type_spec_GuidanceNavigator, bases.get(), reinterpret_cast<PyTypeObject*>(type_GuidanceNavigator_Meta.get()));
    if (!state->type_GuidanceNavigator)
    {
        return nullptr;
    }

    state->type_GuidanceReroutedEventArgs = py::register_python_type(module.get(), type_name_GuidanceReroutedEventArgs, &type_spec_GuidanceReroutedEventArgs, bases.get(), nullptr);
    if (!state->type_GuidanceReroutedEventArgs)
    {
        return nullptr;
    }

    state->type_GuidanceRoadSegment = py::register_python_type(module.get(), type_name_GuidanceRoadSegment, &type_spec_GuidanceRoadSegment, bases.get(), nullptr);
    if (!state->type_GuidanceRoadSegment)
    {
        return nullptr;
    }

    state->type_GuidanceRoadSignpost = py::register_python_type(module.get(), type_name_GuidanceRoadSignpost, &type_spec_GuidanceRoadSignpost, bases.get(), nullptr);
    if (!state->type_GuidanceRoadSignpost)
    {
        return nullptr;
    }

    state->type_GuidanceRoute = py::register_python_type(module.get(), type_name_GuidanceRoute, &type_spec_GuidanceRoute, bases.get(), nullptr);
    if (!state->type_GuidanceRoute)
    {
        return nullptr;
    }

    state->type_GuidanceTelemetryCollector = py::register_python_type(module.get(), type_name_GuidanceTelemetryCollector, &type_spec_GuidanceTelemetryCollector, bases.get(), nullptr);
    if (!state->type_GuidanceTelemetryCollector)
    {
        return nullptr;
    }

    state->type_GuidanceUpdatedEventArgs = py::register_python_type(module.get(), type_name_GuidanceUpdatedEventArgs, &type_spec_GuidanceUpdatedEventArgs, bases.get(), nullptr);

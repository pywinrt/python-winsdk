...
namespace py::cpp::Windows::Perception::Spatial
{
    ...
    static PyType_Slot type_slots_SpatialEntityStore_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_SpatialEntityStore_Meta },
        { }
    };

    static PyType_Spec type_spec_SpatialEntityStore_Meta =
    {
        "_winsdk_Windows_Perception_Spatial.SpatialEntityStore_Meta",
    ...
    {
        { Py_tp_base, &PyType_Type },
        { Py_tp_getset, getset_SpatialStageFrameOfReference_Meta },
        { }
    };

    static PyType_Spec type_spec_SpatialStageFrameOfReference_Meta =
    {
        "_winsdk_Windows_Perception_Spatial.SpatialStageFrameOfReference_Meta",
        static_cast<int>(PyType_Type.tp_basicsize),
        static_cast<int>(PyType_Type.tp_itemsize),
        Py_TPFLAGS_DEFAULT,
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Perception_Spatial(void) noexcept
{
    ...
    state->type_SpatialAnchor = py::register_python_type(module.get(), type_name_SpatialAnchor, &type_spec_SpatialAnchor, bases.get(), nullptr);
    if (!state->type_SpatialAnchor)
    {
        return nullptr;
    }

    state->type_SpatialAnchorExportSufficiency = py::register_python_type(module.get(), type_name_SpatialAnchorExportSufficiency, &type_spec_SpatialAnchorExportSufficiency, bases.get(), nullptr);
    if (!state->type_SpatialAnchorExportSufficiency)
    {
        return nullptr;
    }

    state->type_SpatialAnchorExporter = py::register_python_type(module.get(), type_name_SpatialAnchorExporter, &type_spec_SpatialAnchorExporter, bases.get(), nullptr);
    if (!state->type_SpatialAnchorExporter)
    {
        return nullptr;
    }

    state->type_SpatialAnchorManager = py::register_python_type(module.get(), type_name_SpatialAnchorManager, &type_spec_SpatialAnchorManager, nullptr, nullptr);
    if (!state->type_SpatialAnchorManager)
    {
        return nullptr;
    }

    state->type_SpatialAnchorRawCoordinateSystemAdjustedEventArgs = py::register_python_type(module.get(), type_name_SpatialAnchorRawCoordinateSystemAdjustedEventArgs, &type_spec_SpatialAnchorRawCoordinateSystemAdjustedEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialAnchorRawCoordinateSystemAdjustedEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialAnchorStore = py::register_python_type(module.get(), type_name_SpatialAnchorStore, &type_spec_SpatialAnchorStore, bases.get(), nullptr);
    if (!state->type_SpatialAnchorStore)
    {
        return nullptr;
    }

    state->type_SpatialAnchorTransferManager = py::register_python_type(module.get(), type_name_SpatialAnchorTransferManager, &type_spec_SpatialAnchorTransferManager, nullptr, nullptr);
    if (!state->type_SpatialAnchorTransferManager)
    {
        return nullptr;
    }

    state->type_SpatialBoundingVolume = py::register_python_type(module.get(), type_name_SpatialBoundingVolume, &type_spec_SpatialBoundingVolume, bases.get(), nullptr);
    if (!state->type_SpatialBoundingVolume)
    {
        return nullptr;
    }

    state->type_SpatialCoordinateSystem = py::register_python_type(module.get(), type_name_SpatialCoordinateSystem, &type_spec_SpatialCoordinateSystem, bases.get(), nullptr);
    if (!state->type_SpatialCoordinateSystem)
    {
        return nullptr;
    }

    state->type_SpatialEntity = py::register_python_type(module.get(), type_name_SpatialEntity, &type_spec_SpatialEntity, bases.get(), nullptr);
    if (!state->type_SpatialEntity)
    {
        return nullptr;
    }

    state->type_SpatialEntityAddedEventArgs = py::register_python_type(module.get(), type_name_SpatialEntityAddedEventArgs, &type_spec_SpatialEntityAddedEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialEntityAddedEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialEntityRemovedEventArgs = py::register_python_type(module.get(), type_name_SpatialEntityRemovedEventArgs, &type_spec_SpatialEntityRemovedEventArgs, bases.get(), nullptr);
    ...
    state->type_SpatialEntityStore = py::register_python_type(module.get(), type_name_SpatialEntityStore, &type_spec_SpatialEntityStore, bases.get(), reinterpret_cast<PyTypeObject*>(type_SpatialEntityStore_Meta.get()));
    if (!state->type_SpatialEntityStore)
    {
        return nullptr;
    }

    state->type_SpatialEntityUpdatedEventArgs = py::register_python_type(module.get(), type_name_SpatialEntityUpdatedEventArgs, &type_spec_SpatialEntityUpdatedEventArgs, bases.get(), nullptr);
    if (!state->type_SpatialEntityUpdatedEventArgs)
    {
        return nullptr;
    }

    state->type_SpatialEntityWatcher = py::register_python_type(module.get(), type_name_SpatialEntityWatcher, &type_spec_SpatialEntityWatcher, bases.get(), nullptr);
    if (!state->type_SpatialEntityWatcher)
    {
        return nullptr;
    }

    state->type_SpatialLocation = py::register_python_type(module.get(), type_name_SpatialLocation, &type_spec_SpatialLocation, bases.get(), nullptr);
    if (!state->type_SpatialLocation)
    {
        return nullptr;
    }

    state->type_SpatialLocator = py::register_python_type(module.get(), type_name_SpatialLocator, &type_spec_SpatialLocator, bases.get(), nullptr);
    if (!state->type_SpatialLocator)
    {
        return nullptr;
    }

    state->type_SpatialLocatorAttachedFrameOfReference = py::register_python_type(module.get(), type_name_SpatialLocatorAttachedFrameOfReference, &type_spec_SpatialLocatorAttachedFrameOfReference, bases.get(), nullptr);
    if (!state->type_SpatialLocatorAttachedFrameOfReference)
    {
        return nullptr;
    }

    state->type_SpatialLocatorPositionalTrackingDeactivatingEventArgs = py::register_python_type(module.get(), type_name_SpatialLocatorPositionalTrackingDeactivatingEventArgs, &type_spec_SpatialLocatorPositionalTrackingDeactivatingEventArgs, bases.get(), nullptr);
    ...
    state->type_SpatialStageFrameOfReference = py::register_python_type(module.get(), type_name_SpatialStageFrameOfReference, &type_spec_SpatialStageFrameOfReference, bases.get(), reinterpret_cast<PyTypeObject*>(type_SpatialStageFrameOfReference_Meta.get()));
    if (!state->type_SpatialStageFrameOfReference)
    {
        return nullptr;
    }

    state->type_SpatialStationaryFrameOfReference = py::register_python_type(module.get(), type_name_SpatialStationaryFrameOfReference, &type_spec_SpatialStationaryFrameOfReference, bases.get(), nullptr);
    if (!state->type_SpatialStationaryFrameOfReference)
    {
        return nullptr;
    }

    state->type_SpatialBoundingBox = py::register_python_type(module.get(), type_name_SpatialBoundingBox, &type_spec_SpatialBoundingBox, bases.get(), nullptr);
    if (!state->type_SpatialBoundingBox)
    {
        return nullptr;
    }

    state->type_SpatialBoundingFrustum = py::register_python_type(module.get(), type_name_SpatialBoundingFrustum, &type_spec_SpatialBoundingFrustum, bases.get(), nullptr);
    if (!state->type_SpatialBoundingFrustum)
    {
        return nullptr;
    }

    state->type_SpatialBoundingOrientedBox = py::register_python_type(module.get(), type_name_SpatialBoundingOrientedBox, &type_spec_SpatialBoundingOrientedBox, bases.get(), nullptr);
    if (!state->type_SpatialBoundingOrientedBox)
    {
        return nullptr;
    }

    state->type_SpatialBoundingSphere = py::register_python_type(module.get(), type_name_SpatialBoundingSphere, &type_spec_SpatialBoundingSphere, bases.get(), nullptr);
    if (!state->type_SpatialBoundingSphere)
    {
        return nullptr;
    }

    state->type_SpatialRay = py::register_python_type(module.get(), type_name_SpatialRay, &type_spec_SpatialRay, bases.get(), nullptr);

...
PyMODINIT_FUNC PyInit__winsdk_Windows_Media_Casting(void) noexcept
{
    ...
    state->type_CastingConnection = py::register_python_type(module.get(), type_name_CastingConnection, &type_spec_CastingConnection, bases.get(), nullptr);
    if (!state->type_CastingConnection)
    {
        return nullptr;
    }

    state->type_CastingConnectionErrorOccurredEventArgs = py::register_python_type(module.get(), type_name_CastingConnectionErrorOccurredEventArgs, &type_spec_CastingConnectionErrorOccurredEventArgs, bases.get(), nullptr);
    if (!state->type_CastingConnectionErrorOccurredEventArgs)
    {
        return nullptr;
    }

    state->type_CastingDevice = py::register_python_type(module.get(), type_name_CastingDevice, &type_spec_CastingDevice, bases.get(), nullptr);
    if (!state->type_CastingDevice)
    {
        return nullptr;
    }

    state->type_CastingDevicePicker = py::register_python_type(module.get(), type_name_CastingDevicePicker, &type_spec_CastingDevicePicker, bases.get(), nullptr);
    if (!state->type_CastingDevicePicker)
    {
        return nullptr;
    }

    state->type_CastingDevicePickerFilter = py::register_python_type(module.get(), type_name_CastingDevicePickerFilter, &type_spec_CastingDevicePickerFilter, bases.get(), nullptr);
    if (!state->type_CastingDevicePickerFilter)
    {
        return nullptr;
    }

    state->type_CastingDeviceSelectedEventArgs = py::register_python_type(module.get(), type_name_CastingDeviceSelectedEventArgs, &type_spec_CastingDeviceSelectedEventArgs, bases.get(), nullptr);
    if (!state->type_CastingDeviceSelectedEventArgs)
    {
        return nullptr;
    }

    state->type_CastingSource = py::register_python_type(module.get(), type_name_CastingSource, &type_spec_CastingSource, bases.get(), nullptr);

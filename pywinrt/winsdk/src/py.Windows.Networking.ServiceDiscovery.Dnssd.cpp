...
namespace py::cpp::Windows::Networking::ServiceDiscovery::Dnssd
{
    ...
    static PyType_Slot _type_slots_DnssdServiceInstanceCollection[] = 
    {
        ...
        { Py_mp_subscript, _seq_subscript_DnssdServiceInstanceCollection },
        { },
    };
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Networking_ServiceDiscovery_Dnssd(void) noexcept
{
    ...
    state->type_DnssdRegistrationResult = py::register_python_type(module.get(), type_name_DnssdRegistrationResult, &type_spec_DnssdRegistrationResult, bases.get(), nullptr);
    if (!state->type_DnssdRegistrationResult)
    {
        return nullptr;
    }

    state->type_DnssdServiceInstance = py::register_python_type(module.get(), type_name_DnssdServiceInstance, &type_spec_DnssdServiceInstance, bases.get(), nullptr);
    if (!state->type_DnssdServiceInstance)
    {
        return nullptr;
    }

    state->type_DnssdServiceInstanceCollection = py::register_python_type(module.get(), type_name_DnssdServiceInstanceCollection, &type_spec_DnssdServiceInstanceCollection, sequence_bases.get(), nullptr);
    if (!state->type_DnssdServiceInstanceCollection)
    {
        return nullptr;
    }

    state->type_DnssdServiceWatcher = py::register_python_type(module.get(), type_name_DnssdServiceWatcher, &type_spec_DnssdServiceWatcher, bases.get(), nullptr);

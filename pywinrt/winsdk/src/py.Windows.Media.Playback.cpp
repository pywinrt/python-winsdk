...
namespace py::cpp::Windows::Media::Playback
{
    ...
    static PyType_Slot type_slots_BackgroundMediaPlayer_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_BackgroundMediaPlayer_Meta },
        { }
    };

    static PyType_Spec type_spec_BackgroundMediaPlayer_Meta =
    {
        "_winsdk_Windows_Media_Playback.BackgroundMediaPlayer_Meta",
    ...
    static PyType_Slot _type_slots_MediaPlaybackAudioTrackList[] = 
    {
        ...
        { Py_mp_subscript, _seq_subscript_MediaPlaybackAudioTrackList },
        { },
    };
    ...
    static PyType_Slot _type_slots_MediaPlaybackTimedMetadataTrackList[] = 
    {
        ...
        { Py_mp_subscript, _seq_subscript_MediaPlaybackTimedMetadataTrackList },
        { },
    };
    ...
    static PyType_Slot _type_slots_MediaPlaybackVideoTrackList[] = 
    {
        ...
        { Py_mp_subscript, _seq_subscript_MediaPlaybackVideoTrackList },
        { },
    };
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Media_Playback(void) noexcept
{
    ...
    state->type_BackgroundMediaPlayer = py::register_python_type(module.get(), type_name_BackgroundMediaPlayer, &type_spec_BackgroundMediaPlayer, nullptr, reinterpret_cast<PyTypeObject*>(type_BackgroundMediaPlayer_Meta.get()));
    if (!state->type_BackgroundMediaPlayer)
    {
        return nullptr;
    }

    state->type_CurrentMediaPlaybackItemChangedEventArgs = py::register_python_type(module.get(), type_name_CurrentMediaPlaybackItemChangedEventArgs, &type_spec_CurrentMediaPlaybackItemChangedEventArgs, bases.get(), nullptr);
    if (!state->type_CurrentMediaPlaybackItemChangedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaBreak = py::register_python_type(module.get(), type_name_MediaBreak, &type_spec_MediaBreak, bases.get(), nullptr);
    if (!state->type_MediaBreak)
    {
        return nullptr;
    }

    state->type_MediaBreakEndedEventArgs = py::register_python_type(module.get(), type_name_MediaBreakEndedEventArgs, &type_spec_MediaBreakEndedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaBreakEndedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaBreakManager = py::register_python_type(module.get(), type_name_MediaBreakManager, &type_spec_MediaBreakManager, bases.get(), nullptr);
    if (!state->type_MediaBreakManager)
    {
        return nullptr;
    }

    state->type_MediaBreakSchedule = py::register_python_type(module.get(), type_name_MediaBreakSchedule, &type_spec_MediaBreakSchedule, bases.get(), nullptr);
    if (!state->type_MediaBreakSchedule)
    {
        return nullptr;
    }

    state->type_MediaBreakSeekedOverEventArgs = py::register_python_type(module.get(), type_name_MediaBreakSeekedOverEventArgs, &type_spec_MediaBreakSeekedOverEventArgs, bases.get(), nullptr);
    if (!state->type_MediaBreakSeekedOverEventArgs)
    {
        return nullptr;
    }

    state->type_MediaBreakSkippedEventArgs = py::register_python_type(module.get(), type_name_MediaBreakSkippedEventArgs, &type_spec_MediaBreakSkippedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaBreakSkippedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaBreakStartedEventArgs = py::register_python_type(module.get(), type_name_MediaBreakStartedEventArgs, &type_spec_MediaBreakStartedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaBreakStartedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaItemDisplayProperties = py::register_python_type(module.get(), type_name_MediaItemDisplayProperties, &type_spec_MediaItemDisplayProperties, bases.get(), nullptr);
    if (!state->type_MediaItemDisplayProperties)
    {
        return nullptr;
    }

    state->type_MediaPlaybackAudioTrackList = py::register_python_type(module.get(), type_name_MediaPlaybackAudioTrackList, &type_spec_MediaPlaybackAudioTrackList, sequence_bases.get(), nullptr);
    if (!state->type_MediaPlaybackAudioTrackList)
    {
        return nullptr;
    }

    state->type_MediaPlaybackCommandManager = py::register_python_type(module.get(), type_name_MediaPlaybackCommandManager, &type_spec_MediaPlaybackCommandManager, bases.get(), nullptr);
    if (!state->type_MediaPlaybackCommandManager)
    {
        return nullptr;
    }

    state->type_MediaPlaybackCommandManagerAutoRepeatModeReceivedEventArgs = py::register_python_type(module.get(), type_name_MediaPlaybackCommandManagerAutoRepeatModeReceivedEventArgs, &type_spec_MediaPlaybackCommandManagerAutoRepeatModeReceivedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaPlaybackCommandManagerAutoRepeatModeReceivedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaPlaybackCommandManagerCommandBehavior = py::register_python_type(module.get(), type_name_MediaPlaybackCommandManagerCommandBehavior, &type_spec_MediaPlaybackCommandManagerCommandBehavior, bases.get(), nullptr);
    if (!state->type_MediaPlaybackCommandManagerCommandBehavior)
    {
        return nullptr;
    }

    state->type_MediaPlaybackCommandManagerFastForwardReceivedEventArgs = py::register_python_type(module.get(), type_name_MediaPlaybackCommandManagerFastForwardReceivedEventArgs, &type_spec_MediaPlaybackCommandManagerFastForwardReceivedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaPlaybackCommandManagerFastForwardReceivedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaPlaybackCommandManagerNextReceivedEventArgs = py::register_python_type(module.get(), type_name_MediaPlaybackCommandManagerNextReceivedEventArgs, &type_spec_MediaPlaybackCommandManagerNextReceivedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaPlaybackCommandManagerNextReceivedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaPlaybackCommandManagerPauseReceivedEventArgs = py::register_python_type(module.get(), type_name_MediaPlaybackCommandManagerPauseReceivedEventArgs, &type_spec_MediaPlaybackCommandManagerPauseReceivedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaPlaybackCommandManagerPauseReceivedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaPlaybackCommandManagerPlayReceivedEventArgs = py::register_python_type(module.get(), type_name_MediaPlaybackCommandManagerPlayReceivedEventArgs, &type_spec_MediaPlaybackCommandManagerPlayReceivedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaPlaybackCommandManagerPlayReceivedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaPlaybackCommandManagerPositionReceivedEventArgs = py::register_python_type(module.get(), type_name_MediaPlaybackCommandManagerPositionReceivedEventArgs, &type_spec_MediaPlaybackCommandManagerPositionReceivedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaPlaybackCommandManagerPositionReceivedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaPlaybackCommandManagerPreviousReceivedEventArgs = py::register_python_type(module.get(), type_name_MediaPlaybackCommandManagerPreviousReceivedEventArgs, &type_spec_MediaPlaybackCommandManagerPreviousReceivedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaPlaybackCommandManagerPreviousReceivedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaPlaybackCommandManagerRateReceivedEventArgs = py::register_python_type(module.get(), type_name_MediaPlaybackCommandManagerRateReceivedEventArgs, &type_spec_MediaPlaybackCommandManagerRateReceivedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaPlaybackCommandManagerRateReceivedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaPlaybackCommandManagerRewindReceivedEventArgs = py::register_python_type(module.get(), type_name_MediaPlaybackCommandManagerRewindReceivedEventArgs, &type_spec_MediaPlaybackCommandManagerRewindReceivedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaPlaybackCommandManagerRewindReceivedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaPlaybackCommandManagerShuffleReceivedEventArgs = py::register_python_type(module.get(), type_name_MediaPlaybackCommandManagerShuffleReceivedEventArgs, &type_spec_MediaPlaybackCommandManagerShuffleReceivedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaPlaybackCommandManagerShuffleReceivedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaPlaybackItem = py::register_python_type(module.get(), type_name_MediaPlaybackItem, &type_spec_MediaPlaybackItem, bases.get(), nullptr);
    if (!state->type_MediaPlaybackItem)
    {
        return nullptr;
    }

    state->type_MediaPlaybackItemError = py::register_python_type(module.get(), type_name_MediaPlaybackItemError, &type_spec_MediaPlaybackItemError, bases.get(), nullptr);
    if (!state->type_MediaPlaybackItemError)
    {
        return nullptr;
    }

    state->type_MediaPlaybackItemFailedEventArgs = py::register_python_type(module.get(), type_name_MediaPlaybackItemFailedEventArgs, &type_spec_MediaPlaybackItemFailedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaPlaybackItemFailedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaPlaybackItemOpenedEventArgs = py::register_python_type(module.get(), type_name_MediaPlaybackItemOpenedEventArgs, &type_spec_MediaPlaybackItemOpenedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaPlaybackItemOpenedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaPlaybackList = py::register_python_type(module.get(), type_name_MediaPlaybackList, &type_spec_MediaPlaybackList, bases.get(), nullptr);
    if (!state->type_MediaPlaybackList)
    {
        return nullptr;
    }

    state->type_MediaPlaybackSession = py::register_python_type(module.get(), type_name_MediaPlaybackSession, &type_spec_MediaPlaybackSession, bases.get(), nullptr);
    if (!state->type_MediaPlaybackSession)
    {
        return nullptr;
    }

    state->type_MediaPlaybackSessionBufferingStartedEventArgs = py::register_python_type(module.get(), type_name_MediaPlaybackSessionBufferingStartedEventArgs, &type_spec_MediaPlaybackSessionBufferingStartedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaPlaybackSessionBufferingStartedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaPlaybackSessionOutputDegradationPolicyState = py::register_python_type(module.get(), type_name_MediaPlaybackSessionOutputDegradationPolicyState, &type_spec_MediaPlaybackSessionOutputDegradationPolicyState, bases.get(), nullptr);
    if (!state->type_MediaPlaybackSessionOutputDegradationPolicyState)
    {
        return nullptr;
    }

    state->type_MediaPlaybackSphericalVideoProjection = py::register_python_type(module.get(), type_name_MediaPlaybackSphericalVideoProjection, &type_spec_MediaPlaybackSphericalVideoProjection, bases.get(), nullptr);
    if (!state->type_MediaPlaybackSphericalVideoProjection)
    {
        return nullptr;
    }

    state->type_MediaPlaybackTimedMetadataTrackList = py::register_python_type(module.get(), type_name_MediaPlaybackTimedMetadataTrackList, &type_spec_MediaPlaybackTimedMetadataTrackList, sequence_bases.get(), nullptr);
    if (!state->type_MediaPlaybackTimedMetadataTrackList)
    {
        return nullptr;
    }

    state->type_MediaPlaybackVideoTrackList = py::register_python_type(module.get(), type_name_MediaPlaybackVideoTrackList, &type_spec_MediaPlaybackVideoTrackList, sequence_bases.get(), nullptr);
    if (!state->type_MediaPlaybackVideoTrackList)
    {
        return nullptr;
    }

    state->type_MediaPlayer = py::register_python_type(module.get(), type_name_MediaPlayer, &type_spec_MediaPlayer, bases.get(), nullptr);
    if (!state->type_MediaPlayer)
    {
        return nullptr;
    }

    state->type_MediaPlayerDataReceivedEventArgs = py::register_python_type(module.get(), type_name_MediaPlayerDataReceivedEventArgs, &type_spec_MediaPlayerDataReceivedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaPlayerDataReceivedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaPlayerFailedEventArgs = py::register_python_type(module.get(), type_name_MediaPlayerFailedEventArgs, &type_spec_MediaPlayerFailedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaPlayerFailedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaPlayerRateChangedEventArgs = py::register_python_type(module.get(), type_name_MediaPlayerRateChangedEventArgs, &type_spec_MediaPlayerRateChangedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaPlayerRateChangedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaPlayerSurface = py::register_python_type(module.get(), type_name_MediaPlayerSurface, &type_spec_MediaPlayerSurface, bases.get(), nullptr);
    if (!state->type_MediaPlayerSurface)
    {
        return nullptr;
    }

    state->type_PlaybackMediaMarker = py::register_python_type(module.get(), type_name_PlaybackMediaMarker, &type_spec_PlaybackMediaMarker, bases.get(), nullptr);
    if (!state->type_PlaybackMediaMarker)
    {
        return nullptr;
    }

    state->type_PlaybackMediaMarkerReachedEventArgs = py::register_python_type(module.get(), type_name_PlaybackMediaMarkerReachedEventArgs, &type_spec_PlaybackMediaMarkerReachedEventArgs, bases.get(), nullptr);
    if (!state->type_PlaybackMediaMarkerReachedEventArgs)
    {
        return nullptr;
    }

    state->type_PlaybackMediaMarkerSequence = py::register_python_type(module.get(), type_name_PlaybackMediaMarkerSequence, &type_spec_PlaybackMediaMarkerSequence, bases.get(), nullptr);
    if (!state->type_PlaybackMediaMarkerSequence)
    {
        return nullptr;
    }

    state->type_TimedMetadataPresentationModeChangedEventArgs = py::register_python_type(module.get(), type_name_TimedMetadataPresentationModeChangedEventArgs, &type_spec_TimedMetadataPresentationModeChangedEventArgs, bases.get(), nullptr);
    if (!state->type_TimedMetadataPresentationModeChangedEventArgs)
    {
        return nullptr;
    }

    state->type_IMediaEnginePlaybackSource = py::register_python_type(module.get(), type_name_IMediaEnginePlaybackSource, &type_spec_IMediaEnginePlaybackSource, bases.get(), nullptr);
    if (!state->type_IMediaEnginePlaybackSource)
    {
        return nullptr;
    }

    state->type_IMediaPlaybackSource = py::register_python_type(module.get(), type_name_IMediaPlaybackSource, &type_spec_IMediaPlaybackSource, bases.get(), nullptr);

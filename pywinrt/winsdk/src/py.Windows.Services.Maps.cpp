...
namespace py::cpp::Windows::Services::Maps
{
    ...
    static PyType_Slot type_slots_MapService_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_MapService_Meta },
        { }
    };

    static PyType_Spec type_spec_MapService_Meta =
    {
        "_winsdk_Windows_Services_Maps.MapService_Meta",
    ...
    {
        { Py_tp_base, &PyType_Type },
        { Py_tp_getset, getset_PlaceInfo_Meta },
        { }
    };

    static PyType_Spec type_spec_PlaceInfo_Meta =
    {
        "_winsdk_Windows_Services_Maps.PlaceInfo_Meta",
        static_cast<int>(PyType_Type.tp_basicsize),
        static_cast<int>(PyType_Type.tp_itemsize),
        Py_TPFLAGS_DEFAULT,
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Services_Maps(void) noexcept
{
    ...
    state->type_EnhancedWaypoint = py::register_python_type(module.get(), type_name_EnhancedWaypoint, &type_spec_EnhancedWaypoint, bases.get(), nullptr);
    if (!state->type_EnhancedWaypoint)
    {
        return nullptr;
    }

    state->type_ManeuverWarning = py::register_python_type(module.get(), type_name_ManeuverWarning, &type_spec_ManeuverWarning, bases.get(), nullptr);
    if (!state->type_ManeuverWarning)
    {
        return nullptr;
    }

    state->type_MapAddress = py::register_python_type(module.get(), type_name_MapAddress, &type_spec_MapAddress, bases.get(), nullptr);
    if (!state->type_MapAddress)
    {
        return nullptr;
    }

    state->type_MapLocation = py::register_python_type(module.get(), type_name_MapLocation, &type_spec_MapLocation, bases.get(), nullptr);
    if (!state->type_MapLocation)
    {
        return nullptr;
    }

    state->type_MapLocationFinder = py::register_python_type(module.get(), type_name_MapLocationFinder, &type_spec_MapLocationFinder, nullptr, nullptr);
    if (!state->type_MapLocationFinder)
    {
        return nullptr;
    }

    state->type_MapLocationFinderResult = py::register_python_type(module.get(), type_name_MapLocationFinderResult, &type_spec_MapLocationFinderResult, bases.get(), nullptr);
    if (!state->type_MapLocationFinderResult)
    {
        return nullptr;
    }

    state->type_MapManager = py::register_python_type(module.get(), type_name_MapManager, &type_spec_MapManager, nullptr, nullptr);
    if (!state->type_MapManager)
    {
        return nullptr;
    }

    state->type_MapRoute = py::register_python_type(module.get(), type_name_MapRoute, &type_spec_MapRoute, bases.get(), nullptr);
    if (!state->type_MapRoute)
    {
        return nullptr;
    }

    state->type_MapRouteDrivingOptions = py::register_python_type(module.get(), type_name_MapRouteDrivingOptions, &type_spec_MapRouteDrivingOptions, bases.get(), nullptr);
    if (!state->type_MapRouteDrivingOptions)
    {
        return nullptr;
    }

    state->type_MapRouteFinder = py::register_python_type(module.get(), type_name_MapRouteFinder, &type_spec_MapRouteFinder, nullptr, nullptr);
    if (!state->type_MapRouteFinder)
    {
        return nullptr;
    }

    state->type_MapRouteFinderResult = py::register_python_type(module.get(), type_name_MapRouteFinderResult, &type_spec_MapRouteFinderResult, bases.get(), nullptr);
    if (!state->type_MapRouteFinderResult)
    {
        return nullptr;
    }

    state->type_MapRouteLeg = py::register_python_type(module.get(), type_name_MapRouteLeg, &type_spec_MapRouteLeg, bases.get(), nullptr);
    if (!state->type_MapRouteLeg)
    {
        return nullptr;
    }

    state->type_MapRouteManeuver = py::register_python_type(module.get(), type_name_MapRouteManeuver, &type_spec_MapRouteManeuver, bases.get(), nullptr);
    ...
    state->type_MapService = py::register_python_type(module.get(), type_name_MapService, &type_spec_MapService, nullptr, reinterpret_cast<PyTypeObject*>(type_MapService_Meta.get()));
    ...
    state->type_PlaceInfo = py::register_python_type(module.get(), type_name_PlaceInfo, &type_spec_PlaceInfo, bases.get(), reinterpret_cast<PyTypeObject*>(type_PlaceInfo_Meta.get()));
    if (!state->type_PlaceInfo)
    {
        return nullptr;
    }

    state->type_PlaceInfoCreateOptions = py::register_python_type(module.get(), type_name_PlaceInfoCreateOptions, &type_spec_PlaceInfoCreateOptions, bases.get(), nullptr);

...
namespace py::cpp::Windows::System::Diagnostics
{
    ...
    static PyObject* register_DiagnosticActionState(PyObject* module, PyObject* type)
    {
    ...
    constexpr const char* const type_name_DiagnosticActionResult = "DiagnosticActionResult";
    ...
    constexpr const char* const type_name_DiagnosticInvoker = "DiagnosticInvoker";
    ...
    constexpr const char* const type_name_ProcessCpuUsage = "ProcessCpuUsage";
    ...
    constexpr const char* const type_name_ProcessCpuUsageReport = "ProcessCpuUsageReport";
    ...
    constexpr const char* const type_name_ProcessDiagnosticInfo = "ProcessDiagnosticInfo";
    ...
    constexpr const char* const type_name_ProcessDiskUsage = "ProcessDiskUsage";
    ...
    constexpr const char* const type_name_ProcessDiskUsageReport = "ProcessDiskUsageReport";
    ...
    constexpr const char* const type_name_ProcessMemoryUsage = "ProcessMemoryUsage";
    ...
    constexpr const char* const type_name_ProcessMemoryUsageReport = "ProcessMemoryUsageReport";
    ...
    constexpr const char* const type_name_SystemCpuUsage = "SystemCpuUsage";
    ...
    constexpr const char* const type_name_SystemCpuUsageReport = "SystemCpuUsageReport";
    ...
    constexpr const char* const type_name_SystemDiagnosticInfo = "SystemDiagnosticInfo";
    ...
    constexpr const char* const type_name_SystemMemoryUsage = "SystemMemoryUsage";
    ...
    constexpr const char* const type_name_SystemMemoryUsageReport = "SystemMemoryUsageReport";
...
PyMODINIT_FUNC PyInit__winsdk_Windows_System_Diagnostics(void) noexcept
{
    ...
    state->type_DiagnosticActionResult = py::register_python_type(module.get(), type_name_DiagnosticActionResult, &type_spec_DiagnosticActionResult, bases.get());
    if (!state->type_DiagnosticActionResult)
    {
        return nullptr;
    }

    Py_INCREF(state->type_DiagnosticActionResult);

    state->type_DiagnosticInvoker = py::register_python_type(module.get(), type_name_DiagnosticInvoker, &type_spec_DiagnosticInvoker, bases.get());
    if (!state->type_DiagnosticInvoker)
    {
        return nullptr;
    }

    Py_INCREF(state->type_DiagnosticInvoker);

    state->type_ProcessCpuUsage = py::register_python_type(module.get(), type_name_ProcessCpuUsage, &type_spec_ProcessCpuUsage, bases.get());
    if (!state->type_ProcessCpuUsage)
    {
        return nullptr;
    }

    Py_INCREF(state->type_ProcessCpuUsage);

    state->type_ProcessCpuUsageReport = py::register_python_type(module.get(), type_name_ProcessCpuUsageReport, &type_spec_ProcessCpuUsageReport, bases.get());
    if (!state->type_ProcessCpuUsageReport)
    {
        return nullptr;
    }

    Py_INCREF(state->type_ProcessCpuUsageReport);

    state->type_ProcessDiagnosticInfo = py::register_python_type(module.get(), type_name_ProcessDiagnosticInfo, &type_spec_ProcessDiagnosticInfo, bases.get());
    if (!state->type_ProcessDiagnosticInfo)
    {
        return nullptr;
    }

    Py_INCREF(state->type_ProcessDiagnosticInfo);

    state->type_ProcessDiskUsage = py::register_python_type(module.get(), type_name_ProcessDiskUsage, &type_spec_ProcessDiskUsage, bases.get());
    if (!state->type_ProcessDiskUsage)
    {
        return nullptr;
    }

    Py_INCREF(state->type_ProcessDiskUsage);

    state->type_ProcessDiskUsageReport = py::register_python_type(module.get(), type_name_ProcessDiskUsageReport, &type_spec_ProcessDiskUsageReport, bases.get());
    if (!state->type_ProcessDiskUsageReport)
    {
        return nullptr;
    }

    Py_INCREF(state->type_ProcessDiskUsageReport);

    state->type_ProcessMemoryUsage = py::register_python_type(module.get(), type_name_ProcessMemoryUsage, &type_spec_ProcessMemoryUsage, bases.get());
    if (!state->type_ProcessMemoryUsage)
    {
        return nullptr;
    }

    Py_INCREF(state->type_ProcessMemoryUsage);

    state->type_ProcessMemoryUsageReport = py::register_python_type(module.get(), type_name_ProcessMemoryUsageReport, &type_spec_ProcessMemoryUsageReport, bases.get());
    if (!state->type_ProcessMemoryUsageReport)
    {
        return nullptr;
    }

    Py_INCREF(state->type_ProcessMemoryUsageReport);

    state->type_SystemCpuUsage = py::register_python_type(module.get(), type_name_SystemCpuUsage, &type_spec_SystemCpuUsage, bases.get());
    if (!state->type_SystemCpuUsage)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SystemCpuUsage);

    state->type_SystemCpuUsageReport = py::register_python_type(module.get(), type_name_SystemCpuUsageReport, &type_spec_SystemCpuUsageReport, bases.get());
    if (!state->type_SystemCpuUsageReport)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SystemCpuUsageReport);

    state->type_SystemDiagnosticInfo = py::register_python_type(module.get(), type_name_SystemDiagnosticInfo, &type_spec_SystemDiagnosticInfo, bases.get());
    if (!state->type_SystemDiagnosticInfo)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SystemDiagnosticInfo);

    state->type_SystemMemoryUsage = py::register_python_type(module.get(), type_name_SystemMemoryUsage, &type_spec_SystemMemoryUsage, bases.get());
    if (!state->type_SystemMemoryUsage)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SystemMemoryUsage);

    state->type_SystemMemoryUsageReport = py::register_python_type(module.get(), type_name_SystemMemoryUsageReport, &type_spec_SystemMemoryUsageReport, bases.get());
    if (!state->type_SystemMemoryUsageReport)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SystemMemoryUsageReport);

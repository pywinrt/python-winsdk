...
PyMODINIT_FUNC PyInit__winsdk_Windows_Media_Protection(void) noexcept
{
    ...
    state->type_ComponentLoadFailedEventArgs = py::register_python_type(module.get(), type_name_ComponentLoadFailedEventArgs, &type_spec_ComponentLoadFailedEventArgs, bases.get(), nullptr);
    if (!state->type_ComponentLoadFailedEventArgs)
    {
        return nullptr;
    }

    state->type_ComponentRenewal = py::register_python_type(module.get(), type_name_ComponentRenewal, &type_spec_ComponentRenewal, nullptr, nullptr);
    if (!state->type_ComponentRenewal)
    {
        return nullptr;
    }

    state->type_HdcpSession = py::register_python_type(module.get(), type_name_HdcpSession, &type_spec_HdcpSession, bases.get(), nullptr);
    if (!state->type_HdcpSession)
    {
        return nullptr;
    }

    state->type_MediaProtectionManager = py::register_python_type(module.get(), type_name_MediaProtectionManager, &type_spec_MediaProtectionManager, bases.get(), nullptr);
    if (!state->type_MediaProtectionManager)
    {
        return nullptr;
    }

    state->type_MediaProtectionPMPServer = py::register_python_type(module.get(), type_name_MediaProtectionPMPServer, &type_spec_MediaProtectionPMPServer, bases.get(), nullptr);
    if (!state->type_MediaProtectionPMPServer)
    {
        return nullptr;
    }

    state->type_MediaProtectionServiceCompletion = py::register_python_type(module.get(), type_name_MediaProtectionServiceCompletion, &type_spec_MediaProtectionServiceCompletion, bases.get(), nullptr);
    if (!state->type_MediaProtectionServiceCompletion)
    {
        return nullptr;
    }

    state->type_ProtectionCapabilities = py::register_python_type(module.get(), type_name_ProtectionCapabilities, &type_spec_ProtectionCapabilities, bases.get(), nullptr);
    if (!state->type_ProtectionCapabilities)
    {
        return nullptr;
    }

    state->type_RevocationAndRenewalInformation = py::register_python_type(module.get(), type_name_RevocationAndRenewalInformation, &type_spec_RevocationAndRenewalInformation, bases.get(), nullptr);
    if (!state->type_RevocationAndRenewalInformation)
    {
        return nullptr;
    }

    state->type_RevocationAndRenewalItem = py::register_python_type(module.get(), type_name_RevocationAndRenewalItem, &type_spec_RevocationAndRenewalItem, bases.get(), nullptr);
    if (!state->type_RevocationAndRenewalItem)
    {
        return nullptr;
    }

    state->type_ServiceRequestedEventArgs = py::register_python_type(module.get(), type_name_ServiceRequestedEventArgs, &type_spec_ServiceRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_ServiceRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_IMediaProtectionServiceRequest = py::register_python_type(module.get(), type_name_IMediaProtectionServiceRequest, &type_spec_IMediaProtectionServiceRequest, bases.get(), nullptr);

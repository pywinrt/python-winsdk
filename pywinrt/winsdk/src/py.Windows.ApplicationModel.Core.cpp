...
namespace py::cpp::Windows::ApplicationModel::Core
{
    ...
    static PyGetSetDef getset_CoreApplication_Meta[] = {
    ...
    static PyType_Slot type_slots_CoreApplication_Meta[] = 
    {
        { Py_tp_base, &PyType_Type },
        { Py_tp_getset, getset_CoreApplication_Meta },
        { }
    };

    static PyType_Spec type_spec_CoreApplication_Meta =
    {
        "_winsdk_Windows_ApplicationModel_Core.CoreApplication_Meta",
        static_cast<int>(PyType_Type.tp_basicsize),
        static_cast<int>(PyType_Type.tp_itemsize),
        Py_TPFLAGS_DEFAULT,
        type_slots_CoreApplication_Meta
    };
...
PyMODINIT_FUNC PyInit__winsdk_Windows_ApplicationModel_Core(void) noexcept
{
    ...
    state->type_AppListEntry = py::register_python_type(module.get(), type_name_AppListEntry, &type_spec_AppListEntry, bases.get(), nullptr);
    if (!state->type_AppListEntry)
    {
        return nullptr;
    }

    py::pyobj_handle type_CoreApplication_Meta{PyType_FromSpec(&type_spec_CoreApplication_Meta)};
    if (!type_CoreApplication_Meta)
    {
        return nullptr;
    }

    state->type_CoreApplication = py::register_python_type(module.get(), type_name_CoreApplication, &type_spec_CoreApplication, nullptr, reinterpret_cast<PyTypeObject*>(type_CoreApplication_Meta.get()));
    if (!state->type_CoreApplication)
    {
        return nullptr;
    }

    state->type_CoreApplicationView = py::register_python_type(module.get(), type_name_CoreApplicationView, &type_spec_CoreApplicationView, bases.get(), nullptr);
    if (!state->type_CoreApplicationView)
    {
        return nullptr;
    }

    state->type_CoreApplicationViewTitleBar = py::register_python_type(module.get(), type_name_CoreApplicationViewTitleBar, &type_spec_CoreApplicationViewTitleBar, bases.get(), nullptr);
    if (!state->type_CoreApplicationViewTitleBar)
    {
        return nullptr;
    }

    state->type_HostedViewClosingEventArgs = py::register_python_type(module.get(), type_name_HostedViewClosingEventArgs, &type_spec_HostedViewClosingEventArgs, bases.get(), nullptr);
    if (!state->type_HostedViewClosingEventArgs)
    {
        return nullptr;
    }

    state->type_UnhandledError = py::register_python_type(module.get(), type_name_UnhandledError, &type_spec_UnhandledError, bases.get(), nullptr);
    if (!state->type_UnhandledError)
    {
        return nullptr;
    }

    state->type_UnhandledErrorDetectedEventArgs = py::register_python_type(module.get(), type_name_UnhandledErrorDetectedEventArgs, &type_spec_UnhandledErrorDetectedEventArgs, bases.get(), nullptr);
    if (!state->type_UnhandledErrorDetectedEventArgs)
    {
        return nullptr;
    }

    state->type_ICoreApplicationUnhandledError = py::register_python_type(module.get(), type_name_ICoreApplicationUnhandledError, &type_spec_ICoreApplicationUnhandledError, bases.get(), nullptr);
    if (!state->type_ICoreApplicationUnhandledError)
    {
        return nullptr;
    }

    state->type_IFrameworkView = py::register_python_type(module.get(), type_name_IFrameworkView, &type_spec_IFrameworkView, bases.get(), nullptr);
    if (!state->type_IFrameworkView)
    {
        return nullptr;
    }

    state->type_IFrameworkViewSource = py::register_python_type(module.get(), type_name_IFrameworkViewSource, &type_spec_IFrameworkViewSource, bases.get(), nullptr);

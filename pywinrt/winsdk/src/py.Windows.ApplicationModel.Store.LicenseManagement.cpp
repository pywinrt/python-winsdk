...
namespace py::cpp::Windows::ApplicationModel::Store::LicenseManagement
{
    ...
    static PyObject* register_LicenseRefreshOption(PyObject* module, PyObject* type)
    {
    ...
    constexpr const char* const type_name_LicenseManager = "LicenseManager";
    ...
    constexpr const char* const type_name_LicenseSatisfactionInfo = "LicenseSatisfactionInfo";
    ...
    constexpr const char* const type_name_LicenseSatisfactionResult = "LicenseSatisfactionResult";
...
PyMODINIT_FUNC PyInit__winsdk_Windows_ApplicationModel_Store_LicenseManagement(void) noexcept
{
    ...
    state->type_LicenseManager = py::register_python_type(module.get(), type_name_LicenseManager, &type_spec_LicenseManager, nullptr);
    if (!state->type_LicenseManager)
    {
        return nullptr;
    }

    Py_INCREF(state->type_LicenseManager);

    state->type_LicenseSatisfactionInfo = py::register_python_type(module.get(), type_name_LicenseSatisfactionInfo, &type_spec_LicenseSatisfactionInfo, bases.get());
    if (!state->type_LicenseSatisfactionInfo)
    {
        return nullptr;
    }

    Py_INCREF(state->type_LicenseSatisfactionInfo);

    state->type_LicenseSatisfactionResult = py::register_python_type(module.get(), type_name_LicenseSatisfactionResult, &type_spec_LicenseSatisfactionResult, bases.get());
    if (!state->type_LicenseSatisfactionResult)
    {
        return nullptr;
    }

    Py_INCREF(state->type_LicenseSatisfactionResult);

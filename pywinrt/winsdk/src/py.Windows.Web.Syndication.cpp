...
namespace py::cpp::Windows::Web::Syndication
{
    ...
    static PyObject* register_SyndicationErrorStatus(PyObject* module, PyObject* type)
    {
    ...
    static PyObject* register_SyndicationFormat(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_SyndicationTextType(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    constexpr const char* const type_name_SyndicationAttribute = "SyndicationAttribute";
    ...
    constexpr const char* const type_name_SyndicationCategory = "SyndicationCategory";
    ...
    constexpr const char* const type_name_SyndicationClient = "SyndicationClient";
    ...
    constexpr const char* const type_name_SyndicationContent = "SyndicationContent";
    ...
    constexpr const char* const type_name_SyndicationError = "SyndicationError";
    ...
    constexpr const char* const type_name_SyndicationFeed = "SyndicationFeed";
    ...
    constexpr const char* const type_name_SyndicationGenerator = "SyndicationGenerator";
    ...
    constexpr const char* const type_name_SyndicationItem = "SyndicationItem";
    ...
    constexpr const char* const type_name_SyndicationLink = "SyndicationLink";
    ...
    constexpr const char* const type_name_SyndicationNode = "SyndicationNode";
    ...
    constexpr const char* const type_name_SyndicationPerson = "SyndicationPerson";
    ...
    constexpr const char* const type_name_SyndicationText = "SyndicationText";
    ...
    constexpr const char* const type_name_ISyndicationClient = "ISyndicationClient";
    ...
    constexpr const char* const type_name_ISyndicationNode = "ISyndicationNode";
    ...
    constexpr const char* const type_name_ISyndicationText = "ISyndicationText";
    ...
    constexpr const char* const type_name_RetrievalProgress = "RetrievalProgress";
    ...
    constexpr const char* const type_name_TransferProgress = "TransferProgress";
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Web_Syndication(void) noexcept
{
    ...
    state->type_SyndicationAttribute = py::register_python_type(module.get(), type_name_SyndicationAttribute, &type_spec_SyndicationAttribute, bases.get());
    if (!state->type_SyndicationAttribute)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SyndicationAttribute);

    state->type_SyndicationCategory = py::register_python_type(module.get(), type_name_SyndicationCategory, &type_spec_SyndicationCategory, bases.get());
    if (!state->type_SyndicationCategory)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SyndicationCategory);

    state->type_SyndicationClient = py::register_python_type(module.get(), type_name_SyndicationClient, &type_spec_SyndicationClient, bases.get());
    if (!state->type_SyndicationClient)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SyndicationClient);

    state->type_SyndicationContent = py::register_python_type(module.get(), type_name_SyndicationContent, &type_spec_SyndicationContent, bases.get());
    if (!state->type_SyndicationContent)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SyndicationContent);

    state->type_SyndicationError = py::register_python_type(module.get(), type_name_SyndicationError, &type_spec_SyndicationError, nullptr);
    if (!state->type_SyndicationError)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SyndicationError);

    state->type_SyndicationFeed = py::register_python_type(module.get(), type_name_SyndicationFeed, &type_spec_SyndicationFeed, bases.get());
    if (!state->type_SyndicationFeed)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SyndicationFeed);

    state->type_SyndicationGenerator = py::register_python_type(module.get(), type_name_SyndicationGenerator, &type_spec_SyndicationGenerator, bases.get());
    if (!state->type_SyndicationGenerator)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SyndicationGenerator);

    state->type_SyndicationItem = py::register_python_type(module.get(), type_name_SyndicationItem, &type_spec_SyndicationItem, bases.get());
    if (!state->type_SyndicationItem)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SyndicationItem);

    state->type_SyndicationLink = py::register_python_type(module.get(), type_name_SyndicationLink, &type_spec_SyndicationLink, bases.get());
    if (!state->type_SyndicationLink)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SyndicationLink);

    state->type_SyndicationNode = py::register_python_type(module.get(), type_name_SyndicationNode, &type_spec_SyndicationNode, bases.get());
    if (!state->type_SyndicationNode)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SyndicationNode);

    state->type_SyndicationPerson = py::register_python_type(module.get(), type_name_SyndicationPerson, &type_spec_SyndicationPerson, bases.get());
    if (!state->type_SyndicationPerson)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SyndicationPerson);

    state->type_SyndicationText = py::register_python_type(module.get(), type_name_SyndicationText, &type_spec_SyndicationText, bases.get());
    if (!state->type_SyndicationText)
    {
        return nullptr;
    }

    Py_INCREF(state->type_SyndicationText);

    state->type_ISyndicationClient = py::register_python_type(module.get(), type_name_ISyndicationClient, &type_spec_ISyndicationClient, bases.get());
    if (!state->type_ISyndicationClient)
    {
        return nullptr;
    }

    Py_INCREF(state->type_ISyndicationClient);

    state->type_ISyndicationNode = py::register_python_type(module.get(), type_name_ISyndicationNode, &type_spec_ISyndicationNode, bases.get());
    if (!state->type_ISyndicationNode)
    {
        return nullptr;
    }

    Py_INCREF(state->type_ISyndicationNode);

    state->type_ISyndicationText = py::register_python_type(module.get(), type_name_ISyndicationText, &type_spec_ISyndicationText, bases.get());
    if (!state->type_ISyndicationText)
    {
        return nullptr;
    }

    Py_INCREF(state->type_ISyndicationText);

    state->type_RetrievalProgress = py::register_python_type(module.get(), type_name_RetrievalProgress, &type_spec_RetrievalProgress, bases.get());
    if (!state->type_RetrievalProgress)
    {
        return nullptr;
    }

    Py_INCREF(state->type_RetrievalProgress);

    state->type_TransferProgress = py::register_python_type(module.get(), type_name_TransferProgress, &type_spec_TransferProgress, bases.get());
    if (!state->type_TransferProgress)
    {
        return nullptr;
    }

    Py_INCREF(state->type_TransferProgress);

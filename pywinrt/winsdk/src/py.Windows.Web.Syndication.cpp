...
PyMODINIT_FUNC PyInit__winsdk_Windows_Web_Syndication(void) noexcept
{
    ...
    state->type_SyndicationAttribute = py::register_python_type(module.get(), type_name_SyndicationAttribute, &type_spec_SyndicationAttribute, bases.get(), nullptr);
    if (!state->type_SyndicationAttribute)
    {
        return nullptr;
    }

    state->type_SyndicationCategory = py::register_python_type(module.get(), type_name_SyndicationCategory, &type_spec_SyndicationCategory, bases.get(), nullptr);
    if (!state->type_SyndicationCategory)
    {
        return nullptr;
    }

    state->type_SyndicationClient = py::register_python_type(module.get(), type_name_SyndicationClient, &type_spec_SyndicationClient, bases.get(), nullptr);
    if (!state->type_SyndicationClient)
    {
        return nullptr;
    }

    state->type_SyndicationContent = py::register_python_type(module.get(), type_name_SyndicationContent, &type_spec_SyndicationContent, bases.get(), nullptr);
    if (!state->type_SyndicationContent)
    {
        return nullptr;
    }

    state->type_SyndicationError = py::register_python_type(module.get(), type_name_SyndicationError, &type_spec_SyndicationError, nullptr, nullptr);
    if (!state->type_SyndicationError)
    {
        return nullptr;
    }

    state->type_SyndicationFeed = py::register_python_type(module.get(), type_name_SyndicationFeed, &type_spec_SyndicationFeed, bases.get(), nullptr);
    if (!state->type_SyndicationFeed)
    {
        return nullptr;
    }

    state->type_SyndicationGenerator = py::register_python_type(module.get(), type_name_SyndicationGenerator, &type_spec_SyndicationGenerator, bases.get(), nullptr);
    if (!state->type_SyndicationGenerator)
    {
        return nullptr;
    }

    state->type_SyndicationItem = py::register_python_type(module.get(), type_name_SyndicationItem, &type_spec_SyndicationItem, bases.get(), nullptr);
    if (!state->type_SyndicationItem)
    {
        return nullptr;
    }

    state->type_SyndicationLink = py::register_python_type(module.get(), type_name_SyndicationLink, &type_spec_SyndicationLink, bases.get(), nullptr);
    if (!state->type_SyndicationLink)
    {
        return nullptr;
    }

    state->type_SyndicationNode = py::register_python_type(module.get(), type_name_SyndicationNode, &type_spec_SyndicationNode, bases.get(), nullptr);
    if (!state->type_SyndicationNode)
    {
        return nullptr;
    }

    state->type_SyndicationPerson = py::register_python_type(module.get(), type_name_SyndicationPerson, &type_spec_SyndicationPerson, bases.get(), nullptr);
    if (!state->type_SyndicationPerson)
    {
        return nullptr;
    }

    state->type_SyndicationText = py::register_python_type(module.get(), type_name_SyndicationText, &type_spec_SyndicationText, bases.get(), nullptr);
    if (!state->type_SyndicationText)
    {
        return nullptr;
    }

    state->type_ISyndicationClient = py::register_python_type(module.get(), type_name_ISyndicationClient, &type_spec_ISyndicationClient, bases.get(), nullptr);
    if (!state->type_ISyndicationClient)
    {
        return nullptr;
    }

    state->type_ISyndicationNode = py::register_python_type(module.get(), type_name_ISyndicationNode, &type_spec_ISyndicationNode, bases.get(), nullptr);
    if (!state->type_ISyndicationNode)
    {
        return nullptr;
    }

    state->type_ISyndicationText = py::register_python_type(module.get(), type_name_ISyndicationText, &type_spec_ISyndicationText, bases.get(), nullptr);
    if (!state->type_ISyndicationText)
    {
        return nullptr;
    }

    state->type_RetrievalProgress = py::register_python_type(module.get(), type_name_RetrievalProgress, &type_spec_RetrievalProgress, bases.get(), nullptr);
    if (!state->type_RetrievalProgress)
    {
        return nullptr;
    }

    state->type_TransferProgress = py::register_python_type(module.get(), type_name_TransferProgress, &type_spec_TransferProgress, bases.get(), nullptr);

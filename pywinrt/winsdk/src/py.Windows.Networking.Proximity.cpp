...
namespace py::cpp::Windows::Networking::Proximity
{
    ...
    static PyType_Slot type_slots_PeerFinder_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_PeerFinder_Meta },
        { }
    };

    static PyType_Spec type_spec_PeerFinder_Meta =
    {
        "_winsdk_Windows_Networking_Proximity.PeerFinder_Meta",
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Networking_Proximity(void) noexcept
{
    ...
    state->type_ConnectionRequestedEventArgs = py::register_python_type(module.get(), type_name_ConnectionRequestedEventArgs, &type_spec_ConnectionRequestedEventArgs, bases.get(), nullptr);
    ...
    state->type_PeerFinder = py::register_python_type(module.get(), type_name_PeerFinder, &type_spec_PeerFinder, nullptr, reinterpret_cast<PyTypeObject*>(type_PeerFinder_Meta.get()));
    if (!state->type_PeerFinder)
    {
        return nullptr;
    }

    state->type_PeerInformation = py::register_python_type(module.get(), type_name_PeerInformation, &type_spec_PeerInformation, bases.get(), nullptr);
    if (!state->type_PeerInformation)
    {
        return nullptr;
    }

    state->type_PeerWatcher = py::register_python_type(module.get(), type_name_PeerWatcher, &type_spec_PeerWatcher, bases.get(), nullptr);
    if (!state->type_PeerWatcher)
    {
        return nullptr;
    }

    state->type_ProximityDevice = py::register_python_type(module.get(), type_name_ProximityDevice, &type_spec_ProximityDevice, bases.get(), nullptr);
    if (!state->type_ProximityDevice)
    {
        return nullptr;
    }

    state->type_ProximityMessage = py::register_python_type(module.get(), type_name_ProximityMessage, &type_spec_ProximityMessage, bases.get(), nullptr);
    if (!state->type_ProximityMessage)
    {
        return nullptr;
    }

    state->type_TriggeredConnectionStateChangedEventArgs = py::register_python_type(module.get(), type_name_TriggeredConnectionStateChangedEventArgs, &type_spec_TriggeredConnectionStateChangedEventArgs, bases.get(), nullptr);

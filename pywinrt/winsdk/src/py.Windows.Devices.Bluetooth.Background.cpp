...
namespace py::cpp::Windows::Devices::Bluetooth::Background
{
    ...
    static PyType_Slot type_slots_GattServiceProviderConnection_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_GattServiceProviderConnection_Meta },
        { }
    };

    static PyType_Spec type_spec_GattServiceProviderConnection_Meta =
    {
        "_winsdk_Windows_Devices_Bluetooth_Background.GattServiceProviderConnection_Meta",
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Devices_Bluetooth_Background(void) noexcept
{
    ...
    state->type_BluetoothLEAdvertisementPublisherTriggerDetails = py::register_python_type(module.get(), type_name_BluetoothLEAdvertisementPublisherTriggerDetails, &type_spec_BluetoothLEAdvertisementPublisherTriggerDetails, bases.get(), nullptr);
    if (!state->type_BluetoothLEAdvertisementPublisherTriggerDetails)
    {
        return nullptr;
    }

    state->type_BluetoothLEAdvertisementWatcherTriggerDetails = py::register_python_type(module.get(), type_name_BluetoothLEAdvertisementWatcherTriggerDetails, &type_spec_BluetoothLEAdvertisementWatcherTriggerDetails, bases.get(), nullptr);
    if (!state->type_BluetoothLEAdvertisementWatcherTriggerDetails)
    {
        return nullptr;
    }

    state->type_GattCharacteristicNotificationTriggerDetails = py::register_python_type(module.get(), type_name_GattCharacteristicNotificationTriggerDetails, &type_spec_GattCharacteristicNotificationTriggerDetails, bases.get(), nullptr);
    ...
    state->type_GattServiceProviderConnection = py::register_python_type(module.get(), type_name_GattServiceProviderConnection, &type_spec_GattServiceProviderConnection, bases.get(), reinterpret_cast<PyTypeObject*>(type_GattServiceProviderConnection_Meta.get()));
    if (!state->type_GattServiceProviderConnection)
    {
        return nullptr;
    }

    state->type_GattServiceProviderTriggerDetails = py::register_python_type(module.get(), type_name_GattServiceProviderTriggerDetails, &type_spec_GattServiceProviderTriggerDetails, bases.get(), nullptr);
    if (!state->type_GattServiceProviderTriggerDetails)
    {
        return nullptr;
    }

    state->type_RfcommConnectionTriggerDetails = py::register_python_type(module.get(), type_name_RfcommConnectionTriggerDetails, &type_spec_RfcommConnectionTriggerDetails, bases.get(), nullptr);
    if (!state->type_RfcommConnectionTriggerDetails)
    {
        return nullptr;
    }

    state->type_RfcommInboundConnectionInformation = py::register_python_type(module.get(), type_name_RfcommInboundConnectionInformation, &type_spec_RfcommInboundConnectionInformation, bases.get(), nullptr);
    if (!state->type_RfcommInboundConnectionInformation)
    {
        return nullptr;
    }

    state->type_RfcommOutboundConnectionInformation = py::register_python_type(module.get(), type_name_RfcommOutboundConnectionInformation, &type_spec_RfcommOutboundConnectionInformation, bases.get(), nullptr);

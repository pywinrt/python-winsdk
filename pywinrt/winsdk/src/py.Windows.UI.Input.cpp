...
namespace py::cpp::Windows::UI::Input
{
    ...
    static PyObject* register_CrossSlidingState(PyObject* module, PyObject* type) noexcept
    {
    ...
    static PyObject* register_DraggingState(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_EdgeGestureKind(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_GazeInputAccessStatus(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_GestureSettings(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_HoldingState(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_InputActivationState(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_PointerUpdateKind(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_RadialControllerMenuKnownIcon(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_RadialControllerSystemMenuItemKind(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyType_Slot type_slots_RadialControllerConfiguration_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_RadialControllerConfiguration_Meta },
        { }
    };

    static PyType_Spec type_spec_RadialControllerConfiguration_Meta =
    {
        "_winsdk_Windows_UI_Input.RadialControllerConfiguration_Meta",
...
PyMODINIT_FUNC PyInit__winsdk_Windows_UI_Input(void) noexcept
{
    ...
    state->type_AttachableInputObject = py::register_python_type(module.get(), type_name_AttachableInputObject, &type_spec_AttachableInputObject, bases.get(), nullptr);
    if (!state->type_AttachableInputObject)
    {
        return nullptr;
    }

    state->type_CrossSlidingEventArgs = py::register_python_type(module.get(), type_name_CrossSlidingEventArgs, &type_spec_CrossSlidingEventArgs, bases.get(), nullptr);
    if (!state->type_CrossSlidingEventArgs)
    {
        return nullptr;
    }

    state->type_DraggingEventArgs = py::register_python_type(module.get(), type_name_DraggingEventArgs, &type_spec_DraggingEventArgs, bases.get(), nullptr);
    if (!state->type_DraggingEventArgs)
    {
        return nullptr;
    }

    state->type_EdgeGesture = py::register_python_type(module.get(), type_name_EdgeGesture, &type_spec_EdgeGesture, bases.get(), nullptr);
    if (!state->type_EdgeGesture)
    {
        return nullptr;
    }

    state->type_EdgeGestureEventArgs = py::register_python_type(module.get(), type_name_EdgeGestureEventArgs, &type_spec_EdgeGestureEventArgs, bases.get(), nullptr);
    if (!state->type_EdgeGestureEventArgs)
    {
        return nullptr;
    }

    state->type_GestureRecognizer = py::register_python_type(module.get(), type_name_GestureRecognizer, &type_spec_GestureRecognizer, bases.get(), nullptr);
    if (!state->type_GestureRecognizer)
    {
        return nullptr;
    }

    state->type_HoldingEventArgs = py::register_python_type(module.get(), type_name_HoldingEventArgs, &type_spec_HoldingEventArgs, bases.get(), nullptr);
    if (!state->type_HoldingEventArgs)
    {
        return nullptr;
    }

    state->type_InputActivationListener = py::register_python_type(module.get(), type_name_InputActivationListener, &type_spec_InputActivationListener, bases.get(), nullptr);
    if (!state->type_InputActivationListener)
    {
        return nullptr;
    }

    state->type_InputActivationListenerActivationChangedEventArgs = py::register_python_type(module.get(), type_name_InputActivationListenerActivationChangedEventArgs, &type_spec_InputActivationListenerActivationChangedEventArgs, bases.get(), nullptr);
    if (!state->type_InputActivationListenerActivationChangedEventArgs)
    {
        return nullptr;
    }

    state->type_KeyboardDeliveryInterceptor = py::register_python_type(module.get(), type_name_KeyboardDeliveryInterceptor, &type_spec_KeyboardDeliveryInterceptor, bases.get(), nullptr);
    if (!state->type_KeyboardDeliveryInterceptor)
    {
        return nullptr;
    }

    state->type_ManipulationCompletedEventArgs = py::register_python_type(module.get(), type_name_ManipulationCompletedEventArgs, &type_spec_ManipulationCompletedEventArgs, bases.get(), nullptr);
    if (!state->type_ManipulationCompletedEventArgs)
    {
        return nullptr;
    }

    state->type_ManipulationInertiaStartingEventArgs = py::register_python_type(module.get(), type_name_ManipulationInertiaStartingEventArgs, &type_spec_ManipulationInertiaStartingEventArgs, bases.get(), nullptr);
    if (!state->type_ManipulationInertiaStartingEventArgs)
    {
        return nullptr;
    }

    state->type_ManipulationStartedEventArgs = py::register_python_type(module.get(), type_name_ManipulationStartedEventArgs, &type_spec_ManipulationStartedEventArgs, bases.get(), nullptr);
    if (!state->type_ManipulationStartedEventArgs)
    {
        return nullptr;
    }

    state->type_ManipulationUpdatedEventArgs = py::register_python_type(module.get(), type_name_ManipulationUpdatedEventArgs, &type_spec_ManipulationUpdatedEventArgs, bases.get(), nullptr);
    if (!state->type_ManipulationUpdatedEventArgs)
    {
        return nullptr;
    }

    state->type_MouseWheelParameters = py::register_python_type(module.get(), type_name_MouseWheelParameters, &type_spec_MouseWheelParameters, bases.get(), nullptr);
    if (!state->type_MouseWheelParameters)
    {
        return nullptr;
    }

    state->type_PointerPoint = py::register_python_type(module.get(), type_name_PointerPoint, &type_spec_PointerPoint, bases.get(), nullptr);
    if (!state->type_PointerPoint)
    {
        return nullptr;
    }

    state->type_PointerPointProperties = py::register_python_type(module.get(), type_name_PointerPointProperties, &type_spec_PointerPointProperties, bases.get(), nullptr);
    if (!state->type_PointerPointProperties)
    {
        return nullptr;
    }

    state->type_PointerVisualizationSettings = py::register_python_type(module.get(), type_name_PointerVisualizationSettings, &type_spec_PointerVisualizationSettings, bases.get(), nullptr);
    if (!state->type_PointerVisualizationSettings)
    {
        return nullptr;
    }

    state->type_RadialController = py::register_python_type(module.get(), type_name_RadialController, &type_spec_RadialController, bases.get(), nullptr);
    if (!state->type_RadialController)
    {
        return nullptr;
    }

    state->type_RadialControllerButtonClickedEventArgs = py::register_python_type(module.get(), type_name_RadialControllerButtonClickedEventArgs, &type_spec_RadialControllerButtonClickedEventArgs, bases.get(), nullptr);
    if (!state->type_RadialControllerButtonClickedEventArgs)
    {
        return nullptr;
    }

    state->type_RadialControllerButtonHoldingEventArgs = py::register_python_type(module.get(), type_name_RadialControllerButtonHoldingEventArgs, &type_spec_RadialControllerButtonHoldingEventArgs, bases.get(), nullptr);
    if (!state->type_RadialControllerButtonHoldingEventArgs)
    {
        return nullptr;
    }

    state->type_RadialControllerButtonPressedEventArgs = py::register_python_type(module.get(), type_name_RadialControllerButtonPressedEventArgs, &type_spec_RadialControllerButtonPressedEventArgs, bases.get(), nullptr);
    if (!state->type_RadialControllerButtonPressedEventArgs)
    {
        return nullptr;
    }

    state->type_RadialControllerButtonReleasedEventArgs = py::register_python_type(module.get(), type_name_RadialControllerButtonReleasedEventArgs, &type_spec_RadialControllerButtonReleasedEventArgs, bases.get(), nullptr);
    ...
    state->type_RadialControllerConfiguration = py::register_python_type(module.get(), type_name_RadialControllerConfiguration, &type_spec_RadialControllerConfiguration, bases.get(), reinterpret_cast<PyTypeObject*>(type_RadialControllerConfiguration_Meta.get()));
    if (!state->type_RadialControllerConfiguration)
    {
        return nullptr;
    }

    state->type_RadialControllerControlAcquiredEventArgs = py::register_python_type(module.get(), type_name_RadialControllerControlAcquiredEventArgs, &type_spec_RadialControllerControlAcquiredEventArgs, bases.get(), nullptr);
    if (!state->type_RadialControllerControlAcquiredEventArgs)
    {
        return nullptr;
    }

    state->type_RadialControllerMenu = py::register_python_type(module.get(), type_name_RadialControllerMenu, &type_spec_RadialControllerMenu, bases.get(), nullptr);
    if (!state->type_RadialControllerMenu)
    {
        return nullptr;
    }

    state->type_RadialControllerMenuItem = py::register_python_type(module.get(), type_name_RadialControllerMenuItem, &type_spec_RadialControllerMenuItem, bases.get(), nullptr);
    if (!state->type_RadialControllerMenuItem)
    {
        return nullptr;
    }

    state->type_RadialControllerRotationChangedEventArgs = py::register_python_type(module.get(), type_name_RadialControllerRotationChangedEventArgs, &type_spec_RadialControllerRotationChangedEventArgs, bases.get(), nullptr);
    if (!state->type_RadialControllerRotationChangedEventArgs)
    {
        return nullptr;
    }

    state->type_RadialControllerScreenContact = py::register_python_type(module.get(), type_name_RadialControllerScreenContact, &type_spec_RadialControllerScreenContact, bases.get(), nullptr);
    if (!state->type_RadialControllerScreenContact)
    {
        return nullptr;
    }

    state->type_RadialControllerScreenContactContinuedEventArgs = py::register_python_type(module.get(), type_name_RadialControllerScreenContactContinuedEventArgs, &type_spec_RadialControllerScreenContactContinuedEventArgs, bases.get(), nullptr);
    if (!state->type_RadialControllerScreenContactContinuedEventArgs)
    {
        return nullptr;
    }

    state->type_RadialControllerScreenContactEndedEventArgs = py::register_python_type(module.get(), type_name_RadialControllerScreenContactEndedEventArgs, &type_spec_RadialControllerScreenContactEndedEventArgs, bases.get(), nullptr);
    if (!state->type_RadialControllerScreenContactEndedEventArgs)
    {
        return nullptr;
    }

    state->type_RadialControllerScreenContactStartedEventArgs = py::register_python_type(module.get(), type_name_RadialControllerScreenContactStartedEventArgs, &type_spec_RadialControllerScreenContactStartedEventArgs, bases.get(), nullptr);
    if (!state->type_RadialControllerScreenContactStartedEventArgs)
    {
        return nullptr;
    }

    state->type_RightTappedEventArgs = py::register_python_type(module.get(), type_name_RightTappedEventArgs, &type_spec_RightTappedEventArgs, bases.get(), nullptr);
    if (!state->type_RightTappedEventArgs)
    {
        return nullptr;
    }

    state->type_SystemButtonEventController = py::register_python_type(module.get(), type_name_SystemButtonEventController, &type_spec_SystemButtonEventController, bases.get(), nullptr);
    if (!state->type_SystemButtonEventController)
    {
        return nullptr;
    }

    state->type_SystemFunctionButtonEventArgs = py::register_python_type(module.get(), type_name_SystemFunctionButtonEventArgs, &type_spec_SystemFunctionButtonEventArgs, bases.get(), nullptr);
    if (!state->type_SystemFunctionButtonEventArgs)
    {
        return nullptr;
    }

    state->type_SystemFunctionLockChangedEventArgs = py::register_python_type(module.get(), type_name_SystemFunctionLockChangedEventArgs, &type_spec_SystemFunctionLockChangedEventArgs, bases.get(), nullptr);
    if (!state->type_SystemFunctionLockChangedEventArgs)
    {
        return nullptr;
    }

    state->type_SystemFunctionLockIndicatorChangedEventArgs = py::register_python_type(module.get(), type_name_SystemFunctionLockIndicatorChangedEventArgs, &type_spec_SystemFunctionLockIndicatorChangedEventArgs, bases.get(), nullptr);
    if (!state->type_SystemFunctionLockIndicatorChangedEventArgs)
    {
        return nullptr;
    }

    state->type_TappedEventArgs = py::register_python_type(module.get(), type_name_TappedEventArgs, &type_spec_TappedEventArgs, bases.get(), nullptr);
    if (!state->type_TappedEventArgs)
    {
        return nullptr;
    }

    state->type_IPointerPointTransform = py::register_python_type(module.get(), type_name_IPointerPointTransform, &type_spec_IPointerPointTransform, bases.get(), nullptr);
    if (!state->type_IPointerPointTransform)
    {
        return nullptr;
    }

    state->type_CrossSlideThresholds = py::register_python_type(module.get(), type_name_CrossSlideThresholds, &type_spec_CrossSlideThresholds, bases.get(), nullptr);
    if (!state->type_CrossSlideThresholds)
    {
        return nullptr;
    }

    state->type_ManipulationDelta = py::register_python_type(module.get(), type_name_ManipulationDelta, &type_spec_ManipulationDelta, bases.get(), nullptr);
    if (!state->type_ManipulationDelta)
    {
        return nullptr;
    }

    state->type_ManipulationVelocities = py::register_python_type(module.get(), type_name_ManipulationVelocities, &type_spec_ManipulationVelocities, bases.get(), nullptr);

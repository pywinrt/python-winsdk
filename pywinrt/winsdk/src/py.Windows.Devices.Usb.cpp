...
namespace py::cpp::Windows::Devices::Usb
{
    ...
    static PyGetSetDef getset_UsbDeviceClasses_Meta[] = {
    ...
    static PyType_Slot type_slots_UsbDeviceClasses_Meta[] = 
    {
        { Py_tp_base, reinterpret_cast<void*>(&PyType_Type) },
        { Py_tp_getset, reinterpret_cast<void*>(getset_UsbDeviceClasses_Meta) },
        { }
    };

    static PyType_Spec type_spec_UsbDeviceClasses_Meta =
    {
        "_winsdk_Windows_Devices_Usb.UsbDeviceClasses_Meta",
        static_cast<int>(PyType_Type.tp_basicsize),
        static_cast<int>(PyType_Type.tp_itemsize),
        Py_TPFLAGS_DEFAULT,
        type_slots_UsbDeviceClasses_Meta
    };
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Devices_Usb(void) noexcept
{
    ...
    state->type_UsbBulkInEndpointDescriptor = py::register_python_type(module.get(), type_name_UsbBulkInEndpointDescriptor, &type_spec_UsbBulkInEndpointDescriptor, bases.get(), nullptr);
    if (!state->type_UsbBulkInEndpointDescriptor)
    {
        return nullptr;
    }

    state->type_UsbBulkInPipe = py::register_python_type(module.get(), type_name_UsbBulkInPipe, &type_spec_UsbBulkInPipe, bases.get(), nullptr);
    if (!state->type_UsbBulkInPipe)
    {
        return nullptr;
    }

    state->type_UsbBulkOutEndpointDescriptor = py::register_python_type(module.get(), type_name_UsbBulkOutEndpointDescriptor, &type_spec_UsbBulkOutEndpointDescriptor, bases.get(), nullptr);
    if (!state->type_UsbBulkOutEndpointDescriptor)
    {
        return nullptr;
    }

    state->type_UsbBulkOutPipe = py::register_python_type(module.get(), type_name_UsbBulkOutPipe, &type_spec_UsbBulkOutPipe, bases.get(), nullptr);
    if (!state->type_UsbBulkOutPipe)
    {
        return nullptr;
    }

    state->type_UsbConfiguration = py::register_python_type(module.get(), type_name_UsbConfiguration, &type_spec_UsbConfiguration, bases.get(), nullptr);
    if (!state->type_UsbConfiguration)
    {
        return nullptr;
    }

    state->type_UsbConfigurationDescriptor = py::register_python_type(module.get(), type_name_UsbConfigurationDescriptor, &type_spec_UsbConfigurationDescriptor, bases.get(), nullptr);
    if (!state->type_UsbConfigurationDescriptor)
    {
        return nullptr;
    }

    state->type_UsbControlRequestType = py::register_python_type(module.get(), type_name_UsbControlRequestType, &type_spec_UsbControlRequestType, bases.get(), nullptr);
    if (!state->type_UsbControlRequestType)
    {
        return nullptr;
    }

    state->type_UsbDescriptor = py::register_python_type(module.get(), type_name_UsbDescriptor, &type_spec_UsbDescriptor, bases.get(), nullptr);
    if (!state->type_UsbDescriptor)
    {
        return nullptr;
    }

    state->type_UsbDevice = py::register_python_type(module.get(), type_name_UsbDevice, &type_spec_UsbDevice, bases.get(), nullptr);
    if (!state->type_UsbDevice)
    {
        return nullptr;
    }

    state->type_UsbDeviceClass = py::register_python_type(module.get(), type_name_UsbDeviceClass, &type_spec_UsbDeviceClass, bases.get(), nullptr);
    if (!state->type_UsbDeviceClass)
    {
        return nullptr;
    }

    py::pyobj_handle type_UsbDeviceClasses_Meta{PyType_FromSpec(&type_spec_UsbDeviceClasses_Meta)};
    if (!type_UsbDeviceClasses_Meta)
    {
        return nullptr;
    }

    state->type_UsbDeviceClasses = py::register_python_type(module.get(), type_name_UsbDeviceClasses, &type_spec_UsbDeviceClasses, bases.get(), reinterpret_cast<PyTypeObject*>(type_UsbDeviceClasses_Meta.get()));
    if (!state->type_UsbDeviceClasses)
    {
        return nullptr;
    }

    state->type_UsbDeviceDescriptor = py::register_python_type(module.get(), type_name_UsbDeviceDescriptor, &type_spec_UsbDeviceDescriptor, bases.get(), nullptr);
    if (!state->type_UsbDeviceDescriptor)
    {
        return nullptr;
    }

    state->type_UsbEndpointDescriptor = py::register_python_type(module.get(), type_name_UsbEndpointDescriptor, &type_spec_UsbEndpointDescriptor, bases.get(), nullptr);
    if (!state->type_UsbEndpointDescriptor)
    {
        return nullptr;
    }

    state->type_UsbInterface = py::register_python_type(module.get(), type_name_UsbInterface, &type_spec_UsbInterface, bases.get(), nullptr);
    if (!state->type_UsbInterface)
    {
        return nullptr;
    }

    state->type_UsbInterfaceDescriptor = py::register_python_type(module.get(), type_name_UsbInterfaceDescriptor, &type_spec_UsbInterfaceDescriptor, bases.get(), nullptr);
    if (!state->type_UsbInterfaceDescriptor)
    {
        return nullptr;
    }

    state->type_UsbInterfaceSetting = py::register_python_type(module.get(), type_name_UsbInterfaceSetting, &type_spec_UsbInterfaceSetting, bases.get(), nullptr);
    if (!state->type_UsbInterfaceSetting)
    {
        return nullptr;
    }

    state->type_UsbInterruptInEndpointDescriptor = py::register_python_type(module.get(), type_name_UsbInterruptInEndpointDescriptor, &type_spec_UsbInterruptInEndpointDescriptor, bases.get(), nullptr);
    if (!state->type_UsbInterruptInEndpointDescriptor)
    {
        return nullptr;
    }

    state->type_UsbInterruptInEventArgs = py::register_python_type(module.get(), type_name_UsbInterruptInEventArgs, &type_spec_UsbInterruptInEventArgs, bases.get(), nullptr);
    if (!state->type_UsbInterruptInEventArgs)
    {
        return nullptr;
    }

    state->type_UsbInterruptInPipe = py::register_python_type(module.get(), type_name_UsbInterruptInPipe, &type_spec_UsbInterruptInPipe, bases.get(), nullptr);
    if (!state->type_UsbInterruptInPipe)
    {
        return nullptr;
    }

    state->type_UsbInterruptOutEndpointDescriptor = py::register_python_type(module.get(), type_name_UsbInterruptOutEndpointDescriptor, &type_spec_UsbInterruptOutEndpointDescriptor, bases.get(), nullptr);
    if (!state->type_UsbInterruptOutEndpointDescriptor)
    {
        return nullptr;
    }

    state->type_UsbInterruptOutPipe = py::register_python_type(module.get(), type_name_UsbInterruptOutPipe, &type_spec_UsbInterruptOutPipe, bases.get(), nullptr);
    if (!state->type_UsbInterruptOutPipe)
    {
        return nullptr;
    }

    state->type_UsbSetupPacket = py::register_python_type(module.get(), type_name_UsbSetupPacket, &type_spec_UsbSetupPacket, bases.get(), nullptr);

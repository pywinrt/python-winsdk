...
namespace py::cpp::Windows::Graphics::Holographic
{
    ...
    static PyObject* register_HolographicDepthReprojectionMethod(PyObject* module, PyObject* type) noexcept
    {
    ...
    static PyObject* register_HolographicFramePresentResult(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_HolographicFramePresentWaitBehavior(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_HolographicReprojectionMode(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_HolographicSpaceUserPresence(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_HolographicViewConfigurationKind(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyType_Slot type_slots_HolographicSpace_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_HolographicSpace_Meta },
        { }
    };

    static PyType_Spec type_spec_HolographicSpace_Meta =
    {
        "_winsdk_Windows_Graphics_Holographic.HolographicSpace_Meta",
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Graphics_Holographic(void) noexcept
{
    ...
    state->type_HolographicCamera = py::register_python_type(module.get(), type_name_HolographicCamera, &type_spec_HolographicCamera, bases.get(), nullptr);
    if (!state->type_HolographicCamera)
    {
        return nullptr;
    }

    state->type_HolographicCameraPose = py::register_python_type(module.get(), type_name_HolographicCameraPose, &type_spec_HolographicCameraPose, bases.get(), nullptr);
    if (!state->type_HolographicCameraPose)
    {
        return nullptr;
    }

    state->type_HolographicCameraRenderingParameters = py::register_python_type(module.get(), type_name_HolographicCameraRenderingParameters, &type_spec_HolographicCameraRenderingParameters, bases.get(), nullptr);
    if (!state->type_HolographicCameraRenderingParameters)
    {
        return nullptr;
    }

    state->type_HolographicCameraViewportParameters = py::register_python_type(module.get(), type_name_HolographicCameraViewportParameters, &type_spec_HolographicCameraViewportParameters, bases.get(), nullptr);
    if (!state->type_HolographicCameraViewportParameters)
    {
        return nullptr;
    }

    state->type_HolographicDisplay = py::register_python_type(module.get(), type_name_HolographicDisplay, &type_spec_HolographicDisplay, bases.get(), nullptr);
    if (!state->type_HolographicDisplay)
    {
        return nullptr;
    }

    state->type_HolographicFrame = py::register_python_type(module.get(), type_name_HolographicFrame, &type_spec_HolographicFrame, bases.get(), nullptr);
    if (!state->type_HolographicFrame)
    {
        return nullptr;
    }

    state->type_HolographicFramePrediction = py::register_python_type(module.get(), type_name_HolographicFramePrediction, &type_spec_HolographicFramePrediction, bases.get(), nullptr);
    if (!state->type_HolographicFramePrediction)
    {
        return nullptr;
    }

    state->type_HolographicFramePresentationMonitor = py::register_python_type(module.get(), type_name_HolographicFramePresentationMonitor, &type_spec_HolographicFramePresentationMonitor, bases.get(), nullptr);
    if (!state->type_HolographicFramePresentationMonitor)
    {
        return nullptr;
    }

    state->type_HolographicFramePresentationReport = py::register_python_type(module.get(), type_name_HolographicFramePresentationReport, &type_spec_HolographicFramePresentationReport, bases.get(), nullptr);
    if (!state->type_HolographicFramePresentationReport)
    {
        return nullptr;
    }

    state->type_HolographicFrameRenderingReport = py::register_python_type(module.get(), type_name_HolographicFrameRenderingReport, &type_spec_HolographicFrameRenderingReport, bases.get(), nullptr);
    if (!state->type_HolographicFrameRenderingReport)
    {
        return nullptr;
    }

    state->type_HolographicFrameScanoutMonitor = py::register_python_type(module.get(), type_name_HolographicFrameScanoutMonitor, &type_spec_HolographicFrameScanoutMonitor, bases.get(), nullptr);
    if (!state->type_HolographicFrameScanoutMonitor)
    {
        return nullptr;
    }

    state->type_HolographicFrameScanoutReport = py::register_python_type(module.get(), type_name_HolographicFrameScanoutReport, &type_spec_HolographicFrameScanoutReport, bases.get(), nullptr);
    if (!state->type_HolographicFrameScanoutReport)
    {
        return nullptr;
    }

    state->type_HolographicQuadLayer = py::register_python_type(module.get(), type_name_HolographicQuadLayer, &type_spec_HolographicQuadLayer, bases.get(), nullptr);
    if (!state->type_HolographicQuadLayer)
    {
        return nullptr;
    }

    state->type_HolographicQuadLayerUpdateParameters = py::register_python_type(module.get(), type_name_HolographicQuadLayerUpdateParameters, &type_spec_HolographicQuadLayerUpdateParameters, bases.get(), nullptr);
    ...
    state->type_HolographicSpace = py::register_python_type(module.get(), type_name_HolographicSpace, &type_spec_HolographicSpace, bases.get(), reinterpret_cast<PyTypeObject*>(type_HolographicSpace_Meta.get()));
    if (!state->type_HolographicSpace)
    {
        return nullptr;
    }

    state->type_HolographicSpaceCameraAddedEventArgs = py::register_python_type(module.get(), type_name_HolographicSpaceCameraAddedEventArgs, &type_spec_HolographicSpaceCameraAddedEventArgs, bases.get(), nullptr);
    if (!state->type_HolographicSpaceCameraAddedEventArgs)
    {
        return nullptr;
    }

    state->type_HolographicSpaceCameraRemovedEventArgs = py::register_python_type(module.get(), type_name_HolographicSpaceCameraRemovedEventArgs, &type_spec_HolographicSpaceCameraRemovedEventArgs, bases.get(), nullptr);
    if (!state->type_HolographicSpaceCameraRemovedEventArgs)
    {
        return nullptr;
    }

    state->type_HolographicViewConfiguration = py::register_python_type(module.get(), type_name_HolographicViewConfiguration, &type_spec_HolographicViewConfiguration, bases.get(), nullptr);
    if (!state->type_HolographicViewConfiguration)
    {
        return nullptr;
    }

    state->type_HolographicAdapterId = py::register_python_type(module.get(), type_name_HolographicAdapterId, &type_spec_HolographicAdapterId, bases.get(), nullptr);
    if (!state->type_HolographicAdapterId)
    {
        return nullptr;
    }

    state->type_HolographicFrameId = py::register_python_type(module.get(), type_name_HolographicFrameId, &type_spec_HolographicFrameId, bases.get(), nullptr);
    if (!state->type_HolographicFrameId)
    {
        return nullptr;
    }

    state->type_HolographicStereoTransform = py::register_python_type(module.get(), type_name_HolographicStereoTransform, &type_spec_HolographicStereoTransform, bases.get(), nullptr);

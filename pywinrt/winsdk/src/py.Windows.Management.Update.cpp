...
PyMODINIT_FUNC PyInit__winsdk_Windows_Management_Update(void) noexcept
{
    ...
    state->type_PreviewBuildsManager = py::register_python_type(module.get(), type_name_PreviewBuildsManager, &type_spec_PreviewBuildsManager, bases.get(), nullptr);
    if (!state->type_PreviewBuildsManager)
    {
        return nullptr;
    }

    state->type_PreviewBuildsState = py::register_python_type(module.get(), type_name_PreviewBuildsState, &type_spec_PreviewBuildsState, bases.get(), nullptr);
    if (!state->type_PreviewBuildsState)
    {
        return nullptr;
    }

    state->type_WindowsUpdate = py::register_python_type(module.get(), type_name_WindowsUpdate, &type_spec_WindowsUpdate, bases.get(), nullptr);
    if (!state->type_WindowsUpdate)
    {
        return nullptr;
    }

    state->type_WindowsUpdateActionCompletedEventArgs = py::register_python_type(module.get(), type_name_WindowsUpdateActionCompletedEventArgs, &type_spec_WindowsUpdateActionCompletedEventArgs, bases.get(), nullptr);
    if (!state->type_WindowsUpdateActionCompletedEventArgs)
    {
        return nullptr;
    }

    state->type_WindowsUpdateActionProgress = py::register_python_type(module.get(), type_name_WindowsUpdateActionProgress, &type_spec_WindowsUpdateActionProgress, bases.get(), nullptr);
    if (!state->type_WindowsUpdateActionProgress)
    {
        return nullptr;
    }

    state->type_WindowsUpdateActionResult = py::register_python_type(module.get(), type_name_WindowsUpdateActionResult, &type_spec_WindowsUpdateActionResult, bases.get(), nullptr);
    if (!state->type_WindowsUpdateActionResult)
    {
        return nullptr;
    }

    state->type_WindowsUpdateAdministrator = py::register_python_type(module.get(), type_name_WindowsUpdateAdministrator, &type_spec_WindowsUpdateAdministrator, bases.get(), nullptr);
    if (!state->type_WindowsUpdateAdministrator)
    {
        return nullptr;
    }

    state->type_WindowsUpdateApprovalData = py::register_python_type(module.get(), type_name_WindowsUpdateApprovalData, &type_spec_WindowsUpdateApprovalData, bases.get(), nullptr);
    if (!state->type_WindowsUpdateApprovalData)
    {
        return nullptr;
    }

    state->type_WindowsUpdateAttentionRequiredInfo = py::register_python_type(module.get(), type_name_WindowsUpdateAttentionRequiredInfo, &type_spec_WindowsUpdateAttentionRequiredInfo, bases.get(), nullptr);
    if (!state->type_WindowsUpdateAttentionRequiredInfo)
    {
        return nullptr;
    }

    state->type_WindowsUpdateAttentionRequiredReasonChangedEventArgs = py::register_python_type(module.get(), type_name_WindowsUpdateAttentionRequiredReasonChangedEventArgs, &type_spec_WindowsUpdateAttentionRequiredReasonChangedEventArgs, bases.get(), nullptr);
    if (!state->type_WindowsUpdateAttentionRequiredReasonChangedEventArgs)
    {
        return nullptr;
    }

    state->type_WindowsUpdateGetAdministratorResult = py::register_python_type(module.get(), type_name_WindowsUpdateGetAdministratorResult, &type_spec_WindowsUpdateGetAdministratorResult, bases.get(), nullptr);
    if (!state->type_WindowsUpdateGetAdministratorResult)
    {
        return nullptr;
    }

    state->type_WindowsUpdateItem = py::register_python_type(module.get(), type_name_WindowsUpdateItem, &type_spec_WindowsUpdateItem, bases.get(), nullptr);
    if (!state->type_WindowsUpdateItem)
    {
        return nullptr;
    }

    state->type_WindowsUpdateManager = py::register_python_type(module.get(), type_name_WindowsUpdateManager, &type_spec_WindowsUpdateManager, bases.get(), nullptr);
    if (!state->type_WindowsUpdateManager)
    {
        return nullptr;
    }

    state->type_WindowsUpdateProgressChangedEventArgs = py::register_python_type(module.get(), type_name_WindowsUpdateProgressChangedEventArgs, &type_spec_WindowsUpdateProgressChangedEventArgs, bases.get(), nullptr);
    if (!state->type_WindowsUpdateProgressChangedEventArgs)
    {
        return nullptr;
    }

    state->type_WindowsUpdateRestartRequestOptions = py::register_python_type(module.get(), type_name_WindowsUpdateRestartRequestOptions, &type_spec_WindowsUpdateRestartRequestOptions, bases.get(), nullptr);
    if (!state->type_WindowsUpdateRestartRequestOptions)
    {
        return nullptr;
    }

    state->type_WindowsUpdateScanCompletedEventArgs = py::register_python_type(module.get(), type_name_WindowsUpdateScanCompletedEventArgs, &type_spec_WindowsUpdateScanCompletedEventArgs, bases.get(), nullptr);

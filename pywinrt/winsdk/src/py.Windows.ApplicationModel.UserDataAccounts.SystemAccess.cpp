...
PyMODINIT_FUNC PyInit__winsdk_Windows_ApplicationModel_UserDataAccounts_SystemAccess(void) noexcept
{
    ...
    state->type_DeviceAccountConfiguration = py::register_python_type(module.get(), type_name_DeviceAccountConfiguration, &type_spec_DeviceAccountConfiguration, bases.get(), nullptr);
    if (!state->type_DeviceAccountConfiguration)
    {
        return nullptr;
    }

    state->type_UserDataAccountSystemAccessManager = py::register_python_type(module.get(), type_name_UserDataAccountSystemAccessManager, &type_spec_UserDataAccountSystemAccessManager, nullptr, nullptr);

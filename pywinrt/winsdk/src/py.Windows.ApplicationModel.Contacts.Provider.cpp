...
namespace py::cpp::Windows::ApplicationModel::Contacts::Provider
{
    ...
    static PyObject* register_AddContactResult(PyObject* module, PyObject* type)
    {
...
PyMODINIT_FUNC PyInit__winsdk_Windows_ApplicationModel_Contacts_Provider(void) noexcept
{
    ...
    state->type_ContactPickerUI = py::register_python_type(module.get(), type_name_ContactPickerUI, &type_spec_ContactPickerUI, bases.get());
    if (!state->type_ContactPickerUI)
    {
        return nullptr;
    }

    Py_INCREF(state->type_ContactPickerUI);

    state->type_ContactRemovedEventArgs = py::register_python_type(module.get(), type_name_ContactRemovedEventArgs, &type_spec_ContactRemovedEventArgs, bases.get());
    if (!state->type_ContactRemovedEventArgs)
    {
        return nullptr;
    }

    Py_INCREF(state->type_ContactRemovedEventArgs);

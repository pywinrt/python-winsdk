...
namespace py::cpp::Windows::UI::WindowManagement::Preview
{
    ...
    constexpr const char* const type_name_WindowManagementPreview = "WindowManagementPreview";
...
PyMODINIT_FUNC PyInit__winsdk_Windows_UI_WindowManagement_Preview(void) noexcept
{
    ...
    state->type_WindowManagementPreview = py::register_python_type(module.get(), type_name_WindowManagementPreview, &type_spec_WindowManagementPreview, bases.get());
    if (!state->type_WindowManagementPreview)
    {
        return nullptr;
    }

    Py_INCREF(state->type_WindowManagementPreview);

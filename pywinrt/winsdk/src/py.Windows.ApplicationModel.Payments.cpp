...
namespace py::cpp::Windows::ApplicationModel::Payments
{
    ...
    static PyObject* register_PaymentCanMakePaymentResultStatus(PyObject* module, PyObject* type)
    {
    ...
    static PyObject* register_PaymentOptionPresence(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_PaymentRequestChangeKind(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_PaymentRequestCompletionStatus(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_PaymentRequestStatus(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_PaymentShippingType(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    constexpr const char* const type_name_PaymentAddress = "PaymentAddress";
    ...
    constexpr const char* const type_name_PaymentCanMakePaymentResult = "PaymentCanMakePaymentResult";
    ...
    constexpr const char* const type_name_PaymentCurrencyAmount = "PaymentCurrencyAmount";
    ...
    constexpr const char* const type_name_PaymentDetails = "PaymentDetails";
    ...
    constexpr const char* const type_name_PaymentDetailsModifier = "PaymentDetailsModifier";
    ...
    // ----- PaymentItem class --------------------
    constexpr const char* const type_name_PaymentItem = "PaymentItem";
    ...
    constexpr const char* const type_name_PaymentMediator = "PaymentMediator";
    ...
    constexpr const char* const type_name_PaymentMerchantInfo = "PaymentMerchantInfo";
    ...
    constexpr const char* const type_name_PaymentMethodData = "PaymentMethodData";
    ...
    constexpr const char* const type_name_PaymentOptions = "PaymentOptions";
    ...
    constexpr const char* const type_name_PaymentRequest = "PaymentRequest";
    ...
    constexpr const char* const type_name_PaymentRequestChangedArgs = "PaymentRequestChangedArgs";
    ...
    constexpr const char* const type_name_PaymentRequestChangedResult = "PaymentRequestChangedResult";
    ...
    constexpr const char* const type_name_PaymentRequestSubmitResult = "PaymentRequestSubmitResult";
    ...
    constexpr const char* const type_name_PaymentResponse = "PaymentResponse";
    ...
    constexpr const char* const type_name_PaymentShippingOption = "PaymentShippingOption";
    ...
    // ----- PaymentToken class --------------------
    constexpr const char* const type_name_PaymentToken = "PaymentToken";
...
PyMODINIT_FUNC PyInit__winsdk_Windows_ApplicationModel_Payments(void) noexcept
{
    ...
    state->type_PaymentAddress = py::register_python_type(module.get(), type_name_PaymentAddress, &type_spec_PaymentAddress, bases.get());
    if (!state->type_PaymentAddress)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PaymentAddress);

    state->type_PaymentCanMakePaymentResult = py::register_python_type(module.get(), type_name_PaymentCanMakePaymentResult, &type_spec_PaymentCanMakePaymentResult, bases.get());
    if (!state->type_PaymentCanMakePaymentResult)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PaymentCanMakePaymentResult);

    state->type_PaymentCurrencyAmount = py::register_python_type(module.get(), type_name_PaymentCurrencyAmount, &type_spec_PaymentCurrencyAmount, bases.get());
    if (!state->type_PaymentCurrencyAmount)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PaymentCurrencyAmount);

    state->type_PaymentDetails = py::register_python_type(module.get(), type_name_PaymentDetails, &type_spec_PaymentDetails, bases.get());
    if (!state->type_PaymentDetails)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PaymentDetails);

    state->type_PaymentDetailsModifier = py::register_python_type(module.get(), type_name_PaymentDetailsModifier, &type_spec_PaymentDetailsModifier, bases.get());
    if (!state->type_PaymentDetailsModifier)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PaymentDetailsModifier);

    state->type_PaymentItem = py::register_python_type(module.get(), type_name_PaymentItem, &type_spec_PaymentItem, bases.get());
    if (!state->type_PaymentItem)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PaymentItem);

    state->type_PaymentMediator = py::register_python_type(module.get(), type_name_PaymentMediator, &type_spec_PaymentMediator, bases.get());
    if (!state->type_PaymentMediator)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PaymentMediator);

    state->type_PaymentMerchantInfo = py::register_python_type(module.get(), type_name_PaymentMerchantInfo, &type_spec_PaymentMerchantInfo, bases.get());
    if (!state->type_PaymentMerchantInfo)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PaymentMerchantInfo);

    state->type_PaymentMethodData = py::register_python_type(module.get(), type_name_PaymentMethodData, &type_spec_PaymentMethodData, bases.get());
    if (!state->type_PaymentMethodData)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PaymentMethodData);

    state->type_PaymentOptions = py::register_python_type(module.get(), type_name_PaymentOptions, &type_spec_PaymentOptions, bases.get());
    if (!state->type_PaymentOptions)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PaymentOptions);

    state->type_PaymentRequest = py::register_python_type(module.get(), type_name_PaymentRequest, &type_spec_PaymentRequest, bases.get());
    if (!state->type_PaymentRequest)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PaymentRequest);

    state->type_PaymentRequestChangedArgs = py::register_python_type(module.get(), type_name_PaymentRequestChangedArgs, &type_spec_PaymentRequestChangedArgs, bases.get());
    if (!state->type_PaymentRequestChangedArgs)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PaymentRequestChangedArgs);

    state->type_PaymentRequestChangedResult = py::register_python_type(module.get(), type_name_PaymentRequestChangedResult, &type_spec_PaymentRequestChangedResult, bases.get());
    if (!state->type_PaymentRequestChangedResult)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PaymentRequestChangedResult);

    state->type_PaymentRequestSubmitResult = py::register_python_type(module.get(), type_name_PaymentRequestSubmitResult, &type_spec_PaymentRequestSubmitResult, bases.get());
    if (!state->type_PaymentRequestSubmitResult)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PaymentRequestSubmitResult);

    state->type_PaymentResponse = py::register_python_type(module.get(), type_name_PaymentResponse, &type_spec_PaymentResponse, bases.get());
    if (!state->type_PaymentResponse)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PaymentResponse);

    state->type_PaymentShippingOption = py::register_python_type(module.get(), type_name_PaymentShippingOption, &type_spec_PaymentShippingOption, bases.get());
    if (!state->type_PaymentShippingOption)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PaymentShippingOption);

    state->type_PaymentToken = py::register_python_type(module.get(), type_name_PaymentToken, &type_spec_PaymentToken, bases.get());
    if (!state->type_PaymentToken)
    {
        return nullptr;
    }

    Py_INCREF(state->type_PaymentToken);

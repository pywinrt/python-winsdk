...
namespace py::cpp::Windows::Graphics::Imaging
{
    ...
    static PyObject* register_BitmapAlphaMode(PyObject* module, PyObject* type) noexcept
    {
    ...
    static PyObject* register_BitmapBufferAccessMode(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_BitmapFlip(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_BitmapInterpolationMode(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_BitmapPixelFormat(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_BitmapRotation(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_ColorManagementMode(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_ExifOrientationMode(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_JpegSubsamplingMode(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_PngFilterMode(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyObject* register_TiffCompressionMode(PyObject* module, PyObject* type) noexcept
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
    ...
    static PyType_Slot type_slots_BitmapDecoder_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_BitmapDecoder_Meta },
        { }
    };

    static PyType_Spec type_spec_BitmapDecoder_Meta =
    {
        "_winsdk_Windows_Graphics_Imaging.BitmapDecoder_Meta",
    ...
    {
        { Py_tp_base, &PyType_Type },
        { Py_tp_getset, getset_BitmapEncoder_Meta },
        { }
    };

    static PyType_Spec type_spec_BitmapEncoder_Meta =
    {
        "_winsdk_Windows_Graphics_Imaging.BitmapEncoder_Meta",
        static_cast<int>(PyType_Type.tp_basicsize),
        static_cast<int>(PyType_Type.tp_itemsize),
        Py_TPFLAGS_DEFAULT,
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Graphics_Imaging(void) noexcept
{
    ...
    state->type_BitmapBuffer = py::register_python_type(module.get(), type_name_BitmapBuffer, &type_spec_BitmapBuffer, bases.get(), nullptr);
    if (!state->type_BitmapBuffer)
    {
        return nullptr;
    }

    state->type_BitmapCodecInformation = py::register_python_type(module.get(), type_name_BitmapCodecInformation, &type_spec_BitmapCodecInformation, bases.get(), nullptr);
    ...
    state->type_BitmapDecoder = py::register_python_type(module.get(), type_name_BitmapDecoder, &type_spec_BitmapDecoder, bases.get(), reinterpret_cast<PyTypeObject*>(type_BitmapDecoder_Meta.get()));
    ...
    state->type_BitmapEncoder = py::register_python_type(module.get(), type_name_BitmapEncoder, &type_spec_BitmapEncoder, bases.get(), reinterpret_cast<PyTypeObject*>(type_BitmapEncoder_Meta.get()));
    if (!state->type_BitmapEncoder)
    {
        return nullptr;
    }

    state->type_BitmapFrame = py::register_python_type(module.get(), type_name_BitmapFrame, &type_spec_BitmapFrame, bases.get(), nullptr);
    if (!state->type_BitmapFrame)
    {
        return nullptr;
    }

    state->type_BitmapProperties = py::register_python_type(module.get(), type_name_BitmapProperties, &type_spec_BitmapProperties, bases.get(), nullptr);
    if (!state->type_BitmapProperties)
    {
        return nullptr;
    }

    state->type_BitmapPropertiesView = py::register_python_type(module.get(), type_name_BitmapPropertiesView, &type_spec_BitmapPropertiesView, bases.get(), nullptr);
    if (!state->type_BitmapPropertiesView)
    {
        return nullptr;
    }

    state->type_BitmapPropertySet = py::register_python_type(module.get(), type_name_BitmapPropertySet, &type_spec_BitmapPropertySet, mutable_mapping_bases.get(), nullptr);
    if (!state->type_BitmapPropertySet)
    {
        return nullptr;
    }

    state->type_BitmapTransform = py::register_python_type(module.get(), type_name_BitmapTransform, &type_spec_BitmapTransform, bases.get(), nullptr);
    if (!state->type_BitmapTransform)
    {
        return nullptr;
    }

    state->type_BitmapTypedValue = py::register_python_type(module.get(), type_name_BitmapTypedValue, &type_spec_BitmapTypedValue, bases.get(), nullptr);
    if (!state->type_BitmapTypedValue)
    {
        return nullptr;
    }

    state->type_ImageStream = py::register_python_type(module.get(), type_name_ImageStream, &type_spec_ImageStream, bases.get(), nullptr);
    if (!state->type_ImageStream)
    {
        return nullptr;
    }

    state->type_PixelDataProvider = py::register_python_type(module.get(), type_name_PixelDataProvider, &type_spec_PixelDataProvider, bases.get(), nullptr);
    if (!state->type_PixelDataProvider)
    {
        return nullptr;
    }

    state->type_SoftwareBitmap = py::register_python_type(module.get(), type_name_SoftwareBitmap, &type_spec_SoftwareBitmap, bases.get(), nullptr);
    if (!state->type_SoftwareBitmap)
    {
        return nullptr;
    }

    state->type_IBitmapFrame = py::register_python_type(module.get(), type_name_IBitmapFrame, &type_spec_IBitmapFrame, bases.get(), nullptr);
    if (!state->type_IBitmapFrame)
    {
        return nullptr;
    }

    state->type_IBitmapFrameWithSoftwareBitmap = py::register_python_type(module.get(), type_name_IBitmapFrameWithSoftwareBitmap, &type_spec_IBitmapFrameWithSoftwareBitmap, bases.get(), nullptr);
    if (!state->type_IBitmapFrameWithSoftwareBitmap)
    {
        return nullptr;
    }

    state->type_IBitmapPropertiesView = py::register_python_type(module.get(), type_name_IBitmapPropertiesView, &type_spec_IBitmapPropertiesView, bases.get(), nullptr);
    if (!state->type_IBitmapPropertiesView)
    {
        return nullptr;
    }

    state->type_BitmapBounds = py::register_python_type(module.get(), type_name_BitmapBounds, &type_spec_BitmapBounds, bases.get(), nullptr);
    if (!state->type_BitmapBounds)
    {
        return nullptr;
    }

    state->type_BitmapPlaneDescription = py::register_python_type(module.get(), type_name_BitmapPlaneDescription, &type_spec_BitmapPlaneDescription, bases.get(), nullptr);
    if (!state->type_BitmapPlaneDescription)
    {
        return nullptr;
    }

    state->type_BitmapSize = py::register_python_type(module.get(), type_name_BitmapSize, &type_spec_BitmapSize, bases.get(), nullptr);

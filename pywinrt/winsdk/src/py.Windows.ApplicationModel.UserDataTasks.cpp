...
PyMODINIT_FUNC PyInit__winsdk_Windows_ApplicationModel_UserDataTasks(void) noexcept
{
    ...
    state->type_UserDataTask = py::register_python_type(module.get(), type_name_UserDataTask, &type_spec_UserDataTask, bases.get(), nullptr);
    if (!state->type_UserDataTask)
    {
        return nullptr;
    }

    state->type_UserDataTaskBatch = py::register_python_type(module.get(), type_name_UserDataTaskBatch, &type_spec_UserDataTaskBatch, bases.get(), nullptr);
    if (!state->type_UserDataTaskBatch)
    {
        return nullptr;
    }

    state->type_UserDataTaskList = py::register_python_type(module.get(), type_name_UserDataTaskList, &type_spec_UserDataTaskList, bases.get(), nullptr);
    if (!state->type_UserDataTaskList)
    {
        return nullptr;
    }

    state->type_UserDataTaskListLimitedWriteOperations = py::register_python_type(module.get(), type_name_UserDataTaskListLimitedWriteOperations, &type_spec_UserDataTaskListLimitedWriteOperations, bases.get(), nullptr);
    if (!state->type_UserDataTaskListLimitedWriteOperations)
    {
        return nullptr;
    }

    state->type_UserDataTaskListSyncManager = py::register_python_type(module.get(), type_name_UserDataTaskListSyncManager, &type_spec_UserDataTaskListSyncManager, bases.get(), nullptr);
    if (!state->type_UserDataTaskListSyncManager)
    {
        return nullptr;
    }

    state->type_UserDataTaskManager = py::register_python_type(module.get(), type_name_UserDataTaskManager, &type_spec_UserDataTaskManager, bases.get(), nullptr);
    if (!state->type_UserDataTaskManager)
    {
        return nullptr;
    }

    state->type_UserDataTaskQueryOptions = py::register_python_type(module.get(), type_name_UserDataTaskQueryOptions, &type_spec_UserDataTaskQueryOptions, bases.get(), nullptr);
    if (!state->type_UserDataTaskQueryOptions)
    {
        return nullptr;
    }

    state->type_UserDataTaskReader = py::register_python_type(module.get(), type_name_UserDataTaskReader, &type_spec_UserDataTaskReader, bases.get(), nullptr);
    if (!state->type_UserDataTaskReader)
    {
        return nullptr;
    }

    state->type_UserDataTaskRecurrenceProperties = py::register_python_type(module.get(), type_name_UserDataTaskRecurrenceProperties, &type_spec_UserDataTaskRecurrenceProperties, bases.get(), nullptr);
    if (!state->type_UserDataTaskRecurrenceProperties)
    {
        return nullptr;
    }

    state->type_UserDataTaskRegenerationProperties = py::register_python_type(module.get(), type_name_UserDataTaskRegenerationProperties, &type_spec_UserDataTaskRegenerationProperties, bases.get(), nullptr);
    if (!state->type_UserDataTaskRegenerationProperties)
    {
        return nullptr;
    }

    state->type_UserDataTaskStore = py::register_python_type(module.get(), type_name_UserDataTaskStore, &type_spec_UserDataTaskStore, bases.get(), nullptr);

...
PyMODINIT_FUNC PyInit__winsdk_Windows_Gaming_Preview_GamesEnumeration(void) noexcept
{
    ...
    state->type_GameList = py::register_python_type(module.get(), type_name_GameList, &type_spec_GameList, nullptr, nullptr);
    if (!state->type_GameList)
    {
        return nullptr;
    }

    state->type_GameListEntry = py::register_python_type(module.get(), type_name_GameListEntry, &type_spec_GameListEntry, bases.get(), nullptr);
    if (!state->type_GameListEntry)
    {
        return nullptr;
    }

    state->type_GameModeConfiguration = py::register_python_type(module.get(), type_name_GameModeConfiguration, &type_spec_GameModeConfiguration, bases.get(), nullptr);
    if (!state->type_GameModeConfiguration)
    {
        return nullptr;
    }

    state->type_GameModeUserConfiguration = py::register_python_type(module.get(), type_name_GameModeUserConfiguration, &type_spec_GameModeUserConfiguration, bases.get(), nullptr);
    if (!state->type_GameModeUserConfiguration)
    {
        return nullptr;
    }

    state->type_IGameListEntry = py::register_python_type(module.get(), type_name_IGameListEntry, &type_spec_IGameListEntry, bases.get(), nullptr);

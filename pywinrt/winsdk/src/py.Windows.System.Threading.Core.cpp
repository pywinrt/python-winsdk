...
PyMODINIT_FUNC PyInit__winsdk_Windows_System_Threading_Core(void) noexcept
{
    ...
    state->type_PreallocatedWorkItem = py::register_python_type(module.get(), type_name_PreallocatedWorkItem, &type_spec_PreallocatedWorkItem, bases.get(), nullptr);
    if (!state->type_PreallocatedWorkItem)
    {
        return nullptr;
    }

    state->type_SignalNotifier = py::register_python_type(module.get(), type_name_SignalNotifier, &type_spec_SignalNotifier, bases.get(), nullptr);

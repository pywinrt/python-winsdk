...
PyMODINIT_FUNC PyInit__winsdk_Windows_Devices_Input_Preview(void) noexcept
{
    ...
    state->type_GazeDevicePreview = py::register_python_type(module.get(), type_name_GazeDevicePreview, &type_spec_GazeDevicePreview, bases.get(), nullptr);
    if (!state->type_GazeDevicePreview)
    {
        return nullptr;
    }

    state->type_GazeDeviceWatcherAddedPreviewEventArgs = py::register_python_type(module.get(), type_name_GazeDeviceWatcherAddedPreviewEventArgs, &type_spec_GazeDeviceWatcherAddedPreviewEventArgs, bases.get(), nullptr);
    if (!state->type_GazeDeviceWatcherAddedPreviewEventArgs)
    {
        return nullptr;
    }

    state->type_GazeDeviceWatcherPreview = py::register_python_type(module.get(), type_name_GazeDeviceWatcherPreview, &type_spec_GazeDeviceWatcherPreview, bases.get(), nullptr);
    if (!state->type_GazeDeviceWatcherPreview)
    {
        return nullptr;
    }

    state->type_GazeDeviceWatcherRemovedPreviewEventArgs = py::register_python_type(module.get(), type_name_GazeDeviceWatcherRemovedPreviewEventArgs, &type_spec_GazeDeviceWatcherRemovedPreviewEventArgs, bases.get(), nullptr);
    if (!state->type_GazeDeviceWatcherRemovedPreviewEventArgs)
    {
        return nullptr;
    }

    state->type_GazeDeviceWatcherUpdatedPreviewEventArgs = py::register_python_type(module.get(), type_name_GazeDeviceWatcherUpdatedPreviewEventArgs, &type_spec_GazeDeviceWatcherUpdatedPreviewEventArgs, bases.get(), nullptr);
    if (!state->type_GazeDeviceWatcherUpdatedPreviewEventArgs)
    {
        return nullptr;
    }

    state->type_GazeEnteredPreviewEventArgs = py::register_python_type(module.get(), type_name_GazeEnteredPreviewEventArgs, &type_spec_GazeEnteredPreviewEventArgs, bases.get(), nullptr);
    if (!state->type_GazeEnteredPreviewEventArgs)
    {
        return nullptr;
    }

    state->type_GazeExitedPreviewEventArgs = py::register_python_type(module.get(), type_name_GazeExitedPreviewEventArgs, &type_spec_GazeExitedPreviewEventArgs, bases.get(), nullptr);
    if (!state->type_GazeExitedPreviewEventArgs)
    {
        return nullptr;
    }

    state->type_GazeInputSourcePreview = py::register_python_type(module.get(), type_name_GazeInputSourcePreview, &type_spec_GazeInputSourcePreview, bases.get(), nullptr);
    if (!state->type_GazeInputSourcePreview)
    {
        return nullptr;
    }

    state->type_GazeMovedPreviewEventArgs = py::register_python_type(module.get(), type_name_GazeMovedPreviewEventArgs, &type_spec_GazeMovedPreviewEventArgs, bases.get(), nullptr);
    if (!state->type_GazeMovedPreviewEventArgs)
    {
        return nullptr;
    }

    state->type_GazePointPreview = py::register_python_type(module.get(), type_name_GazePointPreview, &type_spec_GazePointPreview, bases.get(), nullptr);

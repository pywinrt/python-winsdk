...
namespace py::cpp::Windows::Devices::Geolocation
{
    ...
    static PyType_Slot type_slots_Geolocator_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_Geolocator_Meta },
        { }
    };

    static PyType_Spec type_spec_Geolocator_Meta =
    {
        "_winsdk_Windows_Devices_Geolocation.Geolocator_Meta",
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Devices_Geolocation(void) noexcept
{
    ...
    state->type_CivicAddress = py::register_python_type(module.get(), type_name_CivicAddress, &type_spec_CivicAddress, bases.get(), nullptr);
    if (!state->type_CivicAddress)
    {
        return nullptr;
    }

    state->type_GeoboundingBox = py::register_python_type(module.get(), type_name_GeoboundingBox, &type_spec_GeoboundingBox, bases.get(), nullptr);
    if (!state->type_GeoboundingBox)
    {
        return nullptr;
    }

    state->type_Geocircle = py::register_python_type(module.get(), type_name_Geocircle, &type_spec_Geocircle, bases.get(), nullptr);
    if (!state->type_Geocircle)
    {
        return nullptr;
    }

    state->type_Geocoordinate = py::register_python_type(module.get(), type_name_Geocoordinate, &type_spec_Geocoordinate, bases.get(), nullptr);
    if (!state->type_Geocoordinate)
    {
        return nullptr;
    }

    state->type_GeocoordinateSatelliteData = py::register_python_type(module.get(), type_name_GeocoordinateSatelliteData, &type_spec_GeocoordinateSatelliteData, bases.get(), nullptr);
    ...
    state->type_Geolocator = py::register_python_type(module.get(), type_name_Geolocator, &type_spec_Geolocator, bases.get(), reinterpret_cast<PyTypeObject*>(type_Geolocator_Meta.get()));
    if (!state->type_Geolocator)
    {
        return nullptr;
    }

    state->type_Geopath = py::register_python_type(module.get(), type_name_Geopath, &type_spec_Geopath, bases.get(), nullptr);
    if (!state->type_Geopath)
    {
        return nullptr;
    }

    state->type_Geopoint = py::register_python_type(module.get(), type_name_Geopoint, &type_spec_Geopoint, bases.get(), nullptr);
    if (!state->type_Geopoint)
    {
        return nullptr;
    }

    state->type_Geoposition = py::register_python_type(module.get(), type_name_Geoposition, &type_spec_Geoposition, bases.get(), nullptr);
    if (!state->type_Geoposition)
    {
        return nullptr;
    }

    state->type_Geovisit = py::register_python_type(module.get(), type_name_Geovisit, &type_spec_Geovisit, bases.get(), nullptr);
    if (!state->type_Geovisit)
    {
        return nullptr;
    }

    state->type_GeovisitMonitor = py::register_python_type(module.get(), type_name_GeovisitMonitor, &type_spec_GeovisitMonitor, bases.get(), nullptr);
    if (!state->type_GeovisitMonitor)
    {
        return nullptr;
    }

    state->type_GeovisitStateChangedEventArgs = py::register_python_type(module.get(), type_name_GeovisitStateChangedEventArgs, &type_spec_GeovisitStateChangedEventArgs, bases.get(), nullptr);
    if (!state->type_GeovisitStateChangedEventArgs)
    {
        return nullptr;
    }

    state->type_GeovisitTriggerDetails = py::register_python_type(module.get(), type_name_GeovisitTriggerDetails, &type_spec_GeovisitTriggerDetails, bases.get(), nullptr);
    if (!state->type_GeovisitTriggerDetails)
    {
        return nullptr;
    }

    state->type_PositionChangedEventArgs = py::register_python_type(module.get(), type_name_PositionChangedEventArgs, &type_spec_PositionChangedEventArgs, bases.get(), nullptr);
    if (!state->type_PositionChangedEventArgs)
    {
        return nullptr;
    }

    state->type_StatusChangedEventArgs = py::register_python_type(module.get(), type_name_StatusChangedEventArgs, &type_spec_StatusChangedEventArgs, bases.get(), nullptr);
    if (!state->type_StatusChangedEventArgs)
    {
        return nullptr;
    }

    state->type_VenueData = py::register_python_type(module.get(), type_name_VenueData, &type_spec_VenueData, bases.get(), nullptr);
    if (!state->type_VenueData)
    {
        return nullptr;
    }

    state->type_IGeoshape = py::register_python_type(module.get(), type_name_IGeoshape, &type_spec_IGeoshape, bases.get(), nullptr);
    if (!state->type_IGeoshape)
    {
        return nullptr;
    }

    state->type_BasicGeoposition = py::register_python_type(module.get(), type_name_BasicGeoposition, &type_spec_BasicGeoposition, bases.get(), nullptr);

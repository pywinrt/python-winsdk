...
PyMODINIT_FUNC PyInit__winsdk_Windows_Devices_WiFi(void) noexcept
{
    ...
    state->type_WiFiAdapter = py::register_python_type(module.get(), type_name_WiFiAdapter, &type_spec_WiFiAdapter, bases.get(), nullptr);
    if (!state->type_WiFiAdapter)
    {
        return nullptr;
    }

    state->type_WiFiAvailableNetwork = py::register_python_type(module.get(), type_name_WiFiAvailableNetwork, &type_spec_WiFiAvailableNetwork, bases.get(), nullptr);
    if (!state->type_WiFiAvailableNetwork)
    {
        return nullptr;
    }

    state->type_WiFiConnectionResult = py::register_python_type(module.get(), type_name_WiFiConnectionResult, &type_spec_WiFiConnectionResult, bases.get(), nullptr);
    if (!state->type_WiFiConnectionResult)
    {
        return nullptr;
    }

    state->type_WiFiNetworkReport = py::register_python_type(module.get(), type_name_WiFiNetworkReport, &type_spec_WiFiNetworkReport, bases.get(), nullptr);
    if (!state->type_WiFiNetworkReport)
    {
        return nullptr;
    }

    state->type_WiFiOnDemandHotspotConnectTriggerDetails = py::register_python_type(module.get(), type_name_WiFiOnDemandHotspotConnectTriggerDetails, &type_spec_WiFiOnDemandHotspotConnectTriggerDetails, bases.get(), nullptr);
    if (!state->type_WiFiOnDemandHotspotConnectTriggerDetails)
    {
        return nullptr;
    }

    state->type_WiFiOnDemandHotspotConnectionResult = py::register_python_type(module.get(), type_name_WiFiOnDemandHotspotConnectionResult, &type_spec_WiFiOnDemandHotspotConnectionResult, bases.get(), nullptr);
    if (!state->type_WiFiOnDemandHotspotConnectionResult)
    {
        return nullptr;
    }

    state->type_WiFiOnDemandHotspotNetwork = py::register_python_type(module.get(), type_name_WiFiOnDemandHotspotNetwork, &type_spec_WiFiOnDemandHotspotNetwork, bases.get(), nullptr);
    if (!state->type_WiFiOnDemandHotspotNetwork)
    {
        return nullptr;
    }

    state->type_WiFiOnDemandHotspotNetworkProperties = py::register_python_type(module.get(), type_name_WiFiOnDemandHotspotNetworkProperties, &type_spec_WiFiOnDemandHotspotNetworkProperties, bases.get(), nullptr);
    if (!state->type_WiFiOnDemandHotspotNetworkProperties)
    {
        return nullptr;
    }

    state->type_WiFiWpsConfigurationResult = py::register_python_type(module.get(), type_name_WiFiWpsConfigurationResult, &type_spec_WiFiWpsConfigurationResult, bases.get(), nullptr);

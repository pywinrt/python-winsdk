...
PyMODINIT_FUNC PyInit__winsdk_Windows_Foundation_Diagnostics(void) noexcept
{
    ...
    state->type_AsyncCausalityTracer = py::register_python_type(module.get(), type_name_AsyncCausalityTracer, &type_spec_AsyncCausalityTracer, nullptr, nullptr);
    if (!state->type_AsyncCausalityTracer)
    {
        return nullptr;
    }

    state->type_ErrorDetails = py::register_python_type(module.get(), type_name_ErrorDetails, &type_spec_ErrorDetails, bases.get(), nullptr);
    if (!state->type_ErrorDetails)
    {
        return nullptr;
    }

    state->type_FileLoggingSession = py::register_python_type(module.get(), type_name_FileLoggingSession, &type_spec_FileLoggingSession, bases.get(), nullptr);
    if (!state->type_FileLoggingSession)
    {
        return nullptr;
    }

    state->type_LogFileGeneratedEventArgs = py::register_python_type(module.get(), type_name_LogFileGeneratedEventArgs, &type_spec_LogFileGeneratedEventArgs, bases.get(), nullptr);
    if (!state->type_LogFileGeneratedEventArgs)
    {
        return nullptr;
    }

    state->type_LoggingActivity = py::register_python_type(module.get(), type_name_LoggingActivity, &type_spec_LoggingActivity, bases.get(), nullptr);
    if (!state->type_LoggingActivity)
    {
        return nullptr;
    }

    state->type_LoggingChannel = py::register_python_type(module.get(), type_name_LoggingChannel, &type_spec_LoggingChannel, bases.get(), nullptr);
    if (!state->type_LoggingChannel)
    {
        return nullptr;
    }

    state->type_LoggingChannelOptions = py::register_python_type(module.get(), type_name_LoggingChannelOptions, &type_spec_LoggingChannelOptions, bases.get(), nullptr);
    if (!state->type_LoggingChannelOptions)
    {
        return nullptr;
    }

    state->type_LoggingFields = py::register_python_type(module.get(), type_name_LoggingFields, &type_spec_LoggingFields, bases.get(), nullptr);
    if (!state->type_LoggingFields)
    {
        return nullptr;
    }

    state->type_LoggingOptions = py::register_python_type(module.get(), type_name_LoggingOptions, &type_spec_LoggingOptions, bases.get(), nullptr);
    if (!state->type_LoggingOptions)
    {
        return nullptr;
    }

    state->type_LoggingSession = py::register_python_type(module.get(), type_name_LoggingSession, &type_spec_LoggingSession, bases.get(), nullptr);
    if (!state->type_LoggingSession)
    {
        return nullptr;
    }

    state->type_RuntimeBrokerErrorSettings = py::register_python_type(module.get(), type_name_RuntimeBrokerErrorSettings, &type_spec_RuntimeBrokerErrorSettings, bases.get(), nullptr);
    if (!state->type_RuntimeBrokerErrorSettings)
    {
        return nullptr;
    }

    state->type_TracingStatusChangedEventArgs = py::register_python_type(module.get(), type_name_TracingStatusChangedEventArgs, &type_spec_TracingStatusChangedEventArgs, bases.get(), nullptr);
    if (!state->type_TracingStatusChangedEventArgs)
    {
        return nullptr;
    }

    state->type_IErrorReportingSettings = py::register_python_type(module.get(), type_name_IErrorReportingSettings, &type_spec_IErrorReportingSettings, bases.get(), nullptr);
    if (!state->type_IErrorReportingSettings)
    {
        return nullptr;
    }

    state->type_IFileLoggingSession = py::register_python_type(module.get(), type_name_IFileLoggingSession, &type_spec_IFileLoggingSession, bases.get(), nullptr);
    if (!state->type_IFileLoggingSession)
    {
        return nullptr;
    }

    state->type_ILoggingChannel = py::register_python_type(module.get(), type_name_ILoggingChannel, &type_spec_ILoggingChannel, bases.get(), nullptr);
    if (!state->type_ILoggingChannel)
    {
        return nullptr;
    }

    state->type_ILoggingSession = py::register_python_type(module.get(), type_name_ILoggingSession, &type_spec_ILoggingSession, bases.get(), nullptr);
    if (!state->type_ILoggingSession)
    {
        return nullptr;
    }

    state->type_ILoggingTarget = py::register_python_type(module.get(), type_name_ILoggingTarget, &type_spec_ILoggingTarget, bases.get(), nullptr);

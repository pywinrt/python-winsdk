...
namespace py::cpp::Windows::Graphics::Printing3D
{
    ...
    static PyType_Slot type_slots_Printing3DBaseMaterial_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_Printing3DBaseMaterial_Meta },
        { }
    };

    static PyType_Spec type_spec_Printing3DBaseMaterial_Meta =
    {
        "_winsdk_Windows_Graphics_Printing3D.Printing3DBaseMaterial_Meta",
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Graphics_Printing3D(void) noexcept
{
    ...
    state->type_Print3DManager = py::register_python_type(module.get(), type_name_Print3DManager, &type_spec_Print3DManager, bases.get(), nullptr);
    if (!state->type_Print3DManager)
    {
        return nullptr;
    }

    state->type_Print3DTask = py::register_python_type(module.get(), type_name_Print3DTask, &type_spec_Print3DTask, bases.get(), nullptr);
    if (!state->type_Print3DTask)
    {
        return nullptr;
    }

    state->type_Print3DTaskCompletedEventArgs = py::register_python_type(module.get(), type_name_Print3DTaskCompletedEventArgs, &type_spec_Print3DTaskCompletedEventArgs, bases.get(), nullptr);
    if (!state->type_Print3DTaskCompletedEventArgs)
    {
        return nullptr;
    }

    state->type_Print3DTaskRequest = py::register_python_type(module.get(), type_name_Print3DTaskRequest, &type_spec_Print3DTaskRequest, bases.get(), nullptr);
    if (!state->type_Print3DTaskRequest)
    {
        return nullptr;
    }

    state->type_Print3DTaskRequestedEventArgs = py::register_python_type(module.get(), type_name_Print3DTaskRequestedEventArgs, &type_spec_Print3DTaskRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_Print3DTaskRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_Print3DTaskSourceChangedEventArgs = py::register_python_type(module.get(), type_name_Print3DTaskSourceChangedEventArgs, &type_spec_Print3DTaskSourceChangedEventArgs, bases.get(), nullptr);
    if (!state->type_Print3DTaskSourceChangedEventArgs)
    {
        return nullptr;
    }

    state->type_Print3DTaskSourceRequestedArgs = py::register_python_type(module.get(), type_name_Print3DTaskSourceRequestedArgs, &type_spec_Print3DTaskSourceRequestedArgs, bases.get(), nullptr);
    if (!state->type_Print3DTaskSourceRequestedArgs)
    {
        return nullptr;
    }

    state->type_Printing3D3MFPackage = py::register_python_type(module.get(), type_name_Printing3D3MFPackage, &type_spec_Printing3D3MFPackage, bases.get(), nullptr);
    ...
    state->type_Printing3DBaseMaterial = py::register_python_type(module.get(), type_name_Printing3DBaseMaterial, &type_spec_Printing3DBaseMaterial, bases.get(), reinterpret_cast<PyTypeObject*>(type_Printing3DBaseMaterial_Meta.get()));
    if (!state->type_Printing3DBaseMaterial)
    {
        return nullptr;
    }

    state->type_Printing3DBaseMaterialGroup = py::register_python_type(module.get(), type_name_Printing3DBaseMaterialGroup, &type_spec_Printing3DBaseMaterialGroup, bases.get(), nullptr);
    if (!state->type_Printing3DBaseMaterialGroup)
    {
        return nullptr;
    }

    state->type_Printing3DColorMaterial = py::register_python_type(module.get(), type_name_Printing3DColorMaterial, &type_spec_Printing3DColorMaterial, bases.get(), nullptr);
    if (!state->type_Printing3DColorMaterial)
    {
        return nullptr;
    }

    state->type_Printing3DColorMaterialGroup = py::register_python_type(module.get(), type_name_Printing3DColorMaterialGroup, &type_spec_Printing3DColorMaterialGroup, bases.get(), nullptr);
    if (!state->type_Printing3DColorMaterialGroup)
    {
        return nullptr;
    }

    state->type_Printing3DComponent = py::register_python_type(module.get(), type_name_Printing3DComponent, &type_spec_Printing3DComponent, bases.get(), nullptr);
    if (!state->type_Printing3DComponent)
    {
        return nullptr;
    }

    state->type_Printing3DComponentWithMatrix = py::register_python_type(module.get(), type_name_Printing3DComponentWithMatrix, &type_spec_Printing3DComponentWithMatrix, bases.get(), nullptr);
    if (!state->type_Printing3DComponentWithMatrix)
    {
        return nullptr;
    }

    state->type_Printing3DCompositeMaterial = py::register_python_type(module.get(), type_name_Printing3DCompositeMaterial, &type_spec_Printing3DCompositeMaterial, bases.get(), nullptr);
    if (!state->type_Printing3DCompositeMaterial)
    {
        return nullptr;
    }

    state->type_Printing3DCompositeMaterialGroup = py::register_python_type(module.get(), type_name_Printing3DCompositeMaterialGroup, &type_spec_Printing3DCompositeMaterialGroup, bases.get(), nullptr);
    if (!state->type_Printing3DCompositeMaterialGroup)
    {
        return nullptr;
    }

    state->type_Printing3DFaceReductionOptions = py::register_python_type(module.get(), type_name_Printing3DFaceReductionOptions, &type_spec_Printing3DFaceReductionOptions, bases.get(), nullptr);
    if (!state->type_Printing3DFaceReductionOptions)
    {
        return nullptr;
    }

    state->type_Printing3DMaterial = py::register_python_type(module.get(), type_name_Printing3DMaterial, &type_spec_Printing3DMaterial, bases.get(), nullptr);
    if (!state->type_Printing3DMaterial)
    {
        return nullptr;
    }

    state->type_Printing3DMesh = py::register_python_type(module.get(), type_name_Printing3DMesh, &type_spec_Printing3DMesh, bases.get(), nullptr);
    if (!state->type_Printing3DMesh)
    {
        return nullptr;
    }

    state->type_Printing3DMeshVerificationResult = py::register_python_type(module.get(), type_name_Printing3DMeshVerificationResult, &type_spec_Printing3DMeshVerificationResult, bases.get(), nullptr);
    if (!state->type_Printing3DMeshVerificationResult)
    {
        return nullptr;
    }

    state->type_Printing3DModel = py::register_python_type(module.get(), type_name_Printing3DModel, &type_spec_Printing3DModel, bases.get(), nullptr);
    if (!state->type_Printing3DModel)
    {
        return nullptr;
    }

    state->type_Printing3DModelTexture = py::register_python_type(module.get(), type_name_Printing3DModelTexture, &type_spec_Printing3DModelTexture, bases.get(), nullptr);
    if (!state->type_Printing3DModelTexture)
    {
        return nullptr;
    }

    state->type_Printing3DMultiplePropertyMaterial = py::register_python_type(module.get(), type_name_Printing3DMultiplePropertyMaterial, &type_spec_Printing3DMultiplePropertyMaterial, bases.get(), nullptr);
    if (!state->type_Printing3DMultiplePropertyMaterial)
    {
        return nullptr;
    }

    state->type_Printing3DMultiplePropertyMaterialGroup = py::register_python_type(module.get(), type_name_Printing3DMultiplePropertyMaterialGroup, &type_spec_Printing3DMultiplePropertyMaterialGroup, bases.get(), nullptr);
    if (!state->type_Printing3DMultiplePropertyMaterialGroup)
    {
        return nullptr;
    }

    state->type_Printing3DTexture2CoordMaterial = py::register_python_type(module.get(), type_name_Printing3DTexture2CoordMaterial, &type_spec_Printing3DTexture2CoordMaterial, bases.get(), nullptr);
    if (!state->type_Printing3DTexture2CoordMaterial)
    {
        return nullptr;
    }

    state->type_Printing3DTexture2CoordMaterialGroup = py::register_python_type(module.get(), type_name_Printing3DTexture2CoordMaterialGroup, &type_spec_Printing3DTexture2CoordMaterialGroup, bases.get(), nullptr);
    if (!state->type_Printing3DTexture2CoordMaterialGroup)
    {
        return nullptr;
    }

    state->type_Printing3DTextureResource = py::register_python_type(module.get(), type_name_Printing3DTextureResource, &type_spec_Printing3DTextureResource, bases.get(), nullptr);
    if (!state->type_Printing3DTextureResource)
    {
        return nullptr;
    }

    state->type_Printing3DBufferDescription = py::register_python_type(module.get(), type_name_Printing3DBufferDescription, &type_spec_Printing3DBufferDescription, bases.get(), nullptr);

...
namespace py::cpp::Windows::Graphics::Printing
{
    ...
    static PyType_Slot type_slots_StandardPrintTaskOptions_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_StandardPrintTaskOptions_Meta },
        { }
    };

    static PyType_Spec type_spec_StandardPrintTaskOptions_Meta =
    {
        "_winsdk_Windows_Graphics_Printing.StandardPrintTaskOptions_Meta",
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Graphics_Printing(void) noexcept
{
    ...
    state->type_PrintManager = py::register_python_type(module.get(), type_name_PrintManager, &type_spec_PrintManager, bases.get(), nullptr);
    if (!state->type_PrintManager)
    {
        return nullptr;
    }

    state->type_PrintPageInfo = py::register_python_type(module.get(), type_name_PrintPageInfo, &type_spec_PrintPageInfo, bases.get(), nullptr);
    if (!state->type_PrintPageInfo)
    {
        return nullptr;
    }

    state->type_PrintPageRange = py::register_python_type(module.get(), type_name_PrintPageRange, &type_spec_PrintPageRange, bases.get(), nullptr);
    if (!state->type_PrintPageRange)
    {
        return nullptr;
    }

    state->type_PrintPageRangeOptions = py::register_python_type(module.get(), type_name_PrintPageRangeOptions, &type_spec_PrintPageRangeOptions, bases.get(), nullptr);
    if (!state->type_PrintPageRangeOptions)
    {
        return nullptr;
    }

    state->type_PrintTask = py::register_python_type(module.get(), type_name_PrintTask, &type_spec_PrintTask, bases.get(), nullptr);
    if (!state->type_PrintTask)
    {
        return nullptr;
    }

    state->type_PrintTaskCompletedEventArgs = py::register_python_type(module.get(), type_name_PrintTaskCompletedEventArgs, &type_spec_PrintTaskCompletedEventArgs, bases.get(), nullptr);
    if (!state->type_PrintTaskCompletedEventArgs)
    {
        return nullptr;
    }

    state->type_PrintTaskOptions = py::register_python_type(module.get(), type_name_PrintTaskOptions, &type_spec_PrintTaskOptions, bases.get(), nullptr);
    if (!state->type_PrintTaskOptions)
    {
        return nullptr;
    }

    state->type_PrintTaskProgressingEventArgs = py::register_python_type(module.get(), type_name_PrintTaskProgressingEventArgs, &type_spec_PrintTaskProgressingEventArgs, bases.get(), nullptr);
    if (!state->type_PrintTaskProgressingEventArgs)
    {
        return nullptr;
    }

    state->type_PrintTaskRequest = py::register_python_type(module.get(), type_name_PrintTaskRequest, &type_spec_PrintTaskRequest, bases.get(), nullptr);
    if (!state->type_PrintTaskRequest)
    {
        return nullptr;
    }

    state->type_PrintTaskRequestedDeferral = py::register_python_type(module.get(), type_name_PrintTaskRequestedDeferral, &type_spec_PrintTaskRequestedDeferral, bases.get(), nullptr);
    if (!state->type_PrintTaskRequestedDeferral)
    {
        return nullptr;
    }

    state->type_PrintTaskRequestedEventArgs = py::register_python_type(module.get(), type_name_PrintTaskRequestedEventArgs, &type_spec_PrintTaskRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_PrintTaskRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_PrintTaskSourceRequestedArgs = py::register_python_type(module.get(), type_name_PrintTaskSourceRequestedArgs, &type_spec_PrintTaskSourceRequestedArgs, bases.get(), nullptr);
    if (!state->type_PrintTaskSourceRequestedArgs)
    {
        return nullptr;
    }

    state->type_PrintTaskSourceRequestedDeferral = py::register_python_type(module.get(), type_name_PrintTaskSourceRequestedDeferral, &type_spec_PrintTaskSourceRequestedDeferral, bases.get(), nullptr);
    ...
    state->type_StandardPrintTaskOptions = py::register_python_type(module.get(), type_name_StandardPrintTaskOptions, &type_spec_StandardPrintTaskOptions, nullptr, reinterpret_cast<PyTypeObject*>(type_StandardPrintTaskOptions_Meta.get()));
    if (!state->type_StandardPrintTaskOptions)
    {
        return nullptr;
    }

    state->type_IPrintDocumentSource = py::register_python_type(module.get(), type_name_IPrintDocumentSource, &type_spec_IPrintDocumentSource, bases.get(), nullptr);
    if (!state->type_IPrintDocumentSource)
    {
        return nullptr;
    }

    state->type_IPrintTaskOptionsCore = py::register_python_type(module.get(), type_name_IPrintTaskOptionsCore, &type_spec_IPrintTaskOptionsCore, bases.get(), nullptr);
    if (!state->type_IPrintTaskOptionsCore)
    {
        return nullptr;
    }

    state->type_IPrintTaskOptionsCoreProperties = py::register_python_type(module.get(), type_name_IPrintTaskOptionsCoreProperties, &type_spec_IPrintTaskOptionsCoreProperties, bases.get(), nullptr);
    if (!state->type_IPrintTaskOptionsCoreProperties)
    {
        return nullptr;
    }

    state->type_IPrintTaskOptionsCoreUIConfiguration = py::register_python_type(module.get(), type_name_IPrintTaskOptionsCoreUIConfiguration, &type_spec_IPrintTaskOptionsCoreUIConfiguration, bases.get(), nullptr);
    if (!state->type_IPrintTaskOptionsCoreUIConfiguration)
    {
        return nullptr;
    }

    state->type_PrintPageDescription = py::register_python_type(module.get(), type_name_PrintPageDescription, &type_spec_PrintPageDescription, bases.get(), nullptr);

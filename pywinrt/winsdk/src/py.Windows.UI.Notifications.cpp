...
namespace py::cpp::Windows::UI::Notifications
{
    ...
    static PyType_Slot type_slots_KnownAdaptiveNotificationHints_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_KnownAdaptiveNotificationHints_Meta },
        { }
    };

    static PyType_Spec type_spec_KnownAdaptiveNotificationHints_Meta =
    {
        "_winsdk_Windows_UI_Notifications.KnownAdaptiveNotificationHints_Meta",
    ...
    {
        { Py_tp_base, &PyType_Type },
        { Py_tp_getset, getset_KnownAdaptiveNotificationTextStyles_Meta },
        { }
    };

    static PyType_Spec type_spec_KnownAdaptiveNotificationTextStyles_Meta =
    {
        "_winsdk_Windows_UI_Notifications.KnownAdaptiveNotificationTextStyles_Meta",
        static_cast<int>(PyType_Type.tp_basicsize),
        static_cast<int>(PyType_Type.tp_itemsize),
        Py_TPFLAGS_DEFAULT,
    ...
    {
        { Py_tp_base, &PyType_Type },
        { Py_tp_getset, getset_KnownNotificationBindings_Meta },
        { }
    };

    static PyType_Spec type_spec_KnownNotificationBindings_Meta =
    {
        "_winsdk_Windows_UI_Notifications.KnownNotificationBindings_Meta",
        static_cast<int>(PyType_Type.tp_basicsize),
        static_cast<int>(PyType_Type.tp_itemsize),
        Py_TPFLAGS_DEFAULT,
    ...
    {
        { Py_tp_base, &PyType_Type },
        { Py_tp_getset, getset_ToastNotificationManager_Meta },
        { }
    };

    static PyType_Spec type_spec_ToastNotificationManager_Meta =
    {
        "_winsdk_Windows_UI_Notifications.ToastNotificationManager_Meta",
        static_cast<int>(PyType_Type.tp_basicsize),
        static_cast<int>(PyType_Type.tp_itemsize),
        Py_TPFLAGS_DEFAULT,
...
PyMODINIT_FUNC PyInit__winsdk_Windows_UI_Notifications(void) noexcept
{
    ...
    state->type_AdaptiveNotificationText = py::register_python_type(module.get(), type_name_AdaptiveNotificationText, &type_spec_AdaptiveNotificationText, bases.get(), nullptr);
    if (!state->type_AdaptiveNotificationText)
    {
        return nullptr;
    }

    state->type_BadgeNotification = py::register_python_type(module.get(), type_name_BadgeNotification, &type_spec_BadgeNotification, bases.get(), nullptr);
    if (!state->type_BadgeNotification)
    {
        return nullptr;
    }

    state->type_BadgeUpdateManager = py::register_python_type(module.get(), type_name_BadgeUpdateManager, &type_spec_BadgeUpdateManager, nullptr, nullptr);
    if (!state->type_BadgeUpdateManager)
    {
        return nullptr;
    }

    state->type_BadgeUpdateManagerForUser = py::register_python_type(module.get(), type_name_BadgeUpdateManagerForUser, &type_spec_BadgeUpdateManagerForUser, bases.get(), nullptr);
    if (!state->type_BadgeUpdateManagerForUser)
    {
        return nullptr;
    }

    state->type_BadgeUpdater = py::register_python_type(module.get(), type_name_BadgeUpdater, &type_spec_BadgeUpdater, bases.get(), nullptr);
    ...
    state->type_KnownAdaptiveNotificationHints = py::register_python_type(module.get(), type_name_KnownAdaptiveNotificationHints, &type_spec_KnownAdaptiveNotificationHints, nullptr, reinterpret_cast<PyTypeObject*>(type_KnownAdaptiveNotificationHints_Meta.get()));
    ...
    state->type_KnownAdaptiveNotificationTextStyles = py::register_python_type(module.get(), type_name_KnownAdaptiveNotificationTextStyles, &type_spec_KnownAdaptiveNotificationTextStyles, nullptr, reinterpret_cast<PyTypeObject*>(type_KnownAdaptiveNotificationTextStyles_Meta.get()));
    ...
    state->type_KnownNotificationBindings = py::register_python_type(module.get(), type_name_KnownNotificationBindings, &type_spec_KnownNotificationBindings, nullptr, reinterpret_cast<PyTypeObject*>(type_KnownNotificationBindings_Meta.get()));
    if (!state->type_KnownNotificationBindings)
    {
        return nullptr;
    }

    state->type_Notification = py::register_python_type(module.get(), type_name_Notification, &type_spec_Notification, bases.get(), nullptr);
    if (!state->type_Notification)
    {
        return nullptr;
    }

    state->type_NotificationBinding = py::register_python_type(module.get(), type_name_NotificationBinding, &type_spec_NotificationBinding, bases.get(), nullptr);
    if (!state->type_NotificationBinding)
    {
        return nullptr;
    }

    state->type_NotificationData = py::register_python_type(module.get(), type_name_NotificationData, &type_spec_NotificationData, bases.get(), nullptr);
    if (!state->type_NotificationData)
    {
        return nullptr;
    }

    state->type_NotificationVisual = py::register_python_type(module.get(), type_name_NotificationVisual, &type_spec_NotificationVisual, bases.get(), nullptr);
    if (!state->type_NotificationVisual)
    {
        return nullptr;
    }

    state->type_ScheduledTileNotification = py::register_python_type(module.get(), type_name_ScheduledTileNotification, &type_spec_ScheduledTileNotification, bases.get(), nullptr);
    if (!state->type_ScheduledTileNotification)
    {
        return nullptr;
    }

    state->type_ScheduledToastNotification = py::register_python_type(module.get(), type_name_ScheduledToastNotification, &type_spec_ScheduledToastNotification, bases.get(), nullptr);
    if (!state->type_ScheduledToastNotification)
    {
        return nullptr;
    }

    state->type_ScheduledToastNotificationShowingEventArgs = py::register_python_type(module.get(), type_name_ScheduledToastNotificationShowingEventArgs, &type_spec_ScheduledToastNotificationShowingEventArgs, bases.get(), nullptr);
    if (!state->type_ScheduledToastNotificationShowingEventArgs)
    {
        return nullptr;
    }

    state->type_ShownTileNotification = py::register_python_type(module.get(), type_name_ShownTileNotification, &type_spec_ShownTileNotification, bases.get(), nullptr);
    if (!state->type_ShownTileNotification)
    {
        return nullptr;
    }

    state->type_TileFlyoutNotification = py::register_python_type(module.get(), type_name_TileFlyoutNotification, &type_spec_TileFlyoutNotification, bases.get(), nullptr);
    if (!state->type_TileFlyoutNotification)
    {
        return nullptr;
    }

    state->type_TileFlyoutUpdateManager = py::register_python_type(module.get(), type_name_TileFlyoutUpdateManager, &type_spec_TileFlyoutUpdateManager, nullptr, nullptr);
    if (!state->type_TileFlyoutUpdateManager)
    {
        return nullptr;
    }

    state->type_TileFlyoutUpdater = py::register_python_type(module.get(), type_name_TileFlyoutUpdater, &type_spec_TileFlyoutUpdater, bases.get(), nullptr);
    if (!state->type_TileFlyoutUpdater)
    {
        return nullptr;
    }

    state->type_TileNotification = py::register_python_type(module.get(), type_name_TileNotification, &type_spec_TileNotification, bases.get(), nullptr);
    if (!state->type_TileNotification)
    {
        return nullptr;
    }

    state->type_TileUpdateManager = py::register_python_type(module.get(), type_name_TileUpdateManager, &type_spec_TileUpdateManager, nullptr, nullptr);
    if (!state->type_TileUpdateManager)
    {
        return nullptr;
    }

    state->type_TileUpdateManagerForUser = py::register_python_type(module.get(), type_name_TileUpdateManagerForUser, &type_spec_TileUpdateManagerForUser, bases.get(), nullptr);
    if (!state->type_TileUpdateManagerForUser)
    {
        return nullptr;
    }

    state->type_TileUpdater = py::register_python_type(module.get(), type_name_TileUpdater, &type_spec_TileUpdater, bases.get(), nullptr);
    if (!state->type_TileUpdater)
    {
        return nullptr;
    }

    state->type_ToastActivatedEventArgs = py::register_python_type(module.get(), type_name_ToastActivatedEventArgs, &type_spec_ToastActivatedEventArgs, bases.get(), nullptr);
    if (!state->type_ToastActivatedEventArgs)
    {
        return nullptr;
    }

    state->type_ToastCollection = py::register_python_type(module.get(), type_name_ToastCollection, &type_spec_ToastCollection, bases.get(), nullptr);
    if (!state->type_ToastCollection)
    {
        return nullptr;
    }

    state->type_ToastCollectionManager = py::register_python_type(module.get(), type_name_ToastCollectionManager, &type_spec_ToastCollectionManager, bases.get(), nullptr);
    if (!state->type_ToastCollectionManager)
    {
        return nullptr;
    }

    state->type_ToastDismissedEventArgs = py::register_python_type(module.get(), type_name_ToastDismissedEventArgs, &type_spec_ToastDismissedEventArgs, bases.get(), nullptr);
    if (!state->type_ToastDismissedEventArgs)
    {
        return nullptr;
    }

    state->type_ToastFailedEventArgs = py::register_python_type(module.get(), type_name_ToastFailedEventArgs, &type_spec_ToastFailedEventArgs, bases.get(), nullptr);
    if (!state->type_ToastFailedEventArgs)
    {
        return nullptr;
    }

    state->type_ToastNotification = py::register_python_type(module.get(), type_name_ToastNotification, &type_spec_ToastNotification, bases.get(), nullptr);
    if (!state->type_ToastNotification)
    {
        return nullptr;
    }

    state->type_ToastNotificationActionTriggerDetail = py::register_python_type(module.get(), type_name_ToastNotificationActionTriggerDetail, &type_spec_ToastNotificationActionTriggerDetail, bases.get(), nullptr);
    if (!state->type_ToastNotificationActionTriggerDetail)
    {
        return nullptr;
    }

    state->type_ToastNotificationHistory = py::register_python_type(module.get(), type_name_ToastNotificationHistory, &type_spec_ToastNotificationHistory, bases.get(), nullptr);
    if (!state->type_ToastNotificationHistory)
    {
        return nullptr;
    }

    state->type_ToastNotificationHistoryChangedTriggerDetail = py::register_python_type(module.get(), type_name_ToastNotificationHistoryChangedTriggerDetail, &type_spec_ToastNotificationHistoryChangedTriggerDetail, bases.get(), nullptr);
    ...
    state->type_ToastNotificationManager = py::register_python_type(module.get(), type_name_ToastNotificationManager, &type_spec_ToastNotificationManager, nullptr, reinterpret_cast<PyTypeObject*>(type_ToastNotificationManager_Meta.get()));
    if (!state->type_ToastNotificationManager)
    {
        return nullptr;
    }

    state->type_ToastNotificationManagerForUser = py::register_python_type(module.get(), type_name_ToastNotificationManagerForUser, &type_spec_ToastNotificationManagerForUser, bases.get(), nullptr);
    if (!state->type_ToastNotificationManagerForUser)
    {
        return nullptr;
    }

    state->type_ToastNotifier = py::register_python_type(module.get(), type_name_ToastNotifier, &type_spec_ToastNotifier, bases.get(), nullptr);
    if (!state->type_ToastNotifier)
    {
        return nullptr;
    }

    state->type_UserNotification = py::register_python_type(module.get(), type_name_UserNotification, &type_spec_UserNotification, bases.get(), nullptr);
    if (!state->type_UserNotification)
    {
        return nullptr;
    }

    state->type_UserNotificationChangedEventArgs = py::register_python_type(module.get(), type_name_UserNotificationChangedEventArgs, &type_spec_UserNotificationChangedEventArgs, bases.get(), nullptr);
    if (!state->type_UserNotificationChangedEventArgs)
    {
        return nullptr;
    }

    state->type_IAdaptiveNotificationContent = py::register_python_type(module.get(), type_name_IAdaptiveNotificationContent, &type_spec_IAdaptiveNotificationContent, bases.get(), nullptr);

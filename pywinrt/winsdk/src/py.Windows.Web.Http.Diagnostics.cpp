...
PyMODINIT_FUNC PyInit__winsdk_Windows_Web_Http_Diagnostics(void) noexcept
{
    ...
    state->type_HttpDiagnosticProvider = py::register_python_type(module.get(), type_name_HttpDiagnosticProvider, &type_spec_HttpDiagnosticProvider, bases.get(), nullptr);
    if (!state->type_HttpDiagnosticProvider)
    {
        return nullptr;
    }

    state->type_HttpDiagnosticProviderRequestResponseCompletedEventArgs = py::register_python_type(module.get(), type_name_HttpDiagnosticProviderRequestResponseCompletedEventArgs, &type_spec_HttpDiagnosticProviderRequestResponseCompletedEventArgs, bases.get(), nullptr);
    if (!state->type_HttpDiagnosticProviderRequestResponseCompletedEventArgs)
    {
        return nullptr;
    }

    state->type_HttpDiagnosticProviderRequestResponseTimestamps = py::register_python_type(module.get(), type_name_HttpDiagnosticProviderRequestResponseTimestamps, &type_spec_HttpDiagnosticProviderRequestResponseTimestamps, bases.get(), nullptr);
    if (!state->type_HttpDiagnosticProviderRequestResponseTimestamps)
    {
        return nullptr;
    }

    state->type_HttpDiagnosticProviderRequestSentEventArgs = py::register_python_type(module.get(), type_name_HttpDiagnosticProviderRequestSentEventArgs, &type_spec_HttpDiagnosticProviderRequestSentEventArgs, bases.get(), nullptr);
    if (!state->type_HttpDiagnosticProviderRequestSentEventArgs)
    {
        return nullptr;
    }

    state->type_HttpDiagnosticProviderResponseReceivedEventArgs = py::register_python_type(module.get(), type_name_HttpDiagnosticProviderResponseReceivedEventArgs, &type_spec_HttpDiagnosticProviderResponseReceivedEventArgs, bases.get(), nullptr);
    if (!state->type_HttpDiagnosticProviderResponseReceivedEventArgs)
    {
        return nullptr;
    }

    state->type_HttpDiagnosticSourceLocation = py::register_python_type(module.get(), type_name_HttpDiagnosticSourceLocation, &type_spec_HttpDiagnosticSourceLocation, bases.get(), nullptr);

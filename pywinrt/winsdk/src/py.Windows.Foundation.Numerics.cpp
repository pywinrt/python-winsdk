...
PyMODINIT_FUNC PyInit__winsdk_Windows_Foundation_Numerics(void) noexcept
{
    ...
    state->type_Matrix3x2 = py::register_python_type(module.get(), type_name_Matrix3x2, &type_spec_Matrix3x2, bases.get(), nullptr);
    if (!state->type_Matrix3x2)
    {
        return nullptr;
    }

    state->type_Matrix4x4 = py::register_python_type(module.get(), type_name_Matrix4x4, &type_spec_Matrix4x4, bases.get(), nullptr);
    if (!state->type_Matrix4x4)
    {
        return nullptr;
    }

    state->type_Plane = py::register_python_type(module.get(), type_name_Plane, &type_spec_Plane, bases.get(), nullptr);
    if (!state->type_Plane)
    {
        return nullptr;
    }

    state->type_Quaternion = py::register_python_type(module.get(), type_name_Quaternion, &type_spec_Quaternion, bases.get(), nullptr);
    if (!state->type_Quaternion)
    {
        return nullptr;
    }

    state->type_Rational = py::register_python_type(module.get(), type_name_Rational, &type_spec_Rational, bases.get(), nullptr);
    if (!state->type_Rational)
    {
        return nullptr;
    }

    state->type_Vector2 = py::register_python_type(module.get(), type_name_Vector2, &type_spec_Vector2, bases.get(), nullptr);
    if (!state->type_Vector2)
    {
        return nullptr;
    }

    state->type_Vector3 = py::register_python_type(module.get(), type_name_Vector3, &type_spec_Vector3, bases.get(), nullptr);
    if (!state->type_Vector3)
    {
        return nullptr;
    }

    state->type_Vector4 = py::register_python_type(module.get(), type_name_Vector4, &type_spec_Vector4, bases.get(), nullptr);

...
namespace py::cpp::Windows::Graphics::DirectX::Direct3D11
{
    ...
    static PyObject* register_Direct3DBindings(PyObject* module, PyObject* type)
    {
    ...
    static PyObject* register_Direct3DUsage(PyObject* module, PyObject* type)
    {
        auto state = reinterpret_cast<module_state*>(PyModule_GetState(module));
        assert(state);
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Graphics_DirectX_Direct3D11(void) noexcept
{
    ...
    state->type_IDirect3DDevice = py::register_python_type(module.get(), type_name_IDirect3DDevice, &type_spec_IDirect3DDevice, bases.get());
    if (!state->type_IDirect3DDevice)
    {
        return nullptr;
    }

    Py_INCREF(state->type_IDirect3DDevice);

    state->type_IDirect3DSurface = py::register_python_type(module.get(), type_name_IDirect3DSurface, &type_spec_IDirect3DSurface, bases.get());
    if (!state->type_IDirect3DSurface)
    {
        return nullptr;
    }

    Py_INCREF(state->type_IDirect3DSurface);

    state->type_Direct3DMultisampleDescription = py::register_python_type(module.get(), type_name_Direct3DMultisampleDescription, &type_spec_Direct3DMultisampleDescription, bases.get());
    if (!state->type_Direct3DMultisampleDescription)
    {
        return nullptr;
    }

    Py_INCREF(state->type_Direct3DMultisampleDescription);

    state->type_Direct3DSurfaceDescription = py::register_python_type(module.get(), type_name_Direct3DSurfaceDescription, &type_spec_Direct3DSurfaceDescription, bases.get());
    if (!state->type_Direct3DSurfaceDescription)
    {
        return nullptr;
    }

    Py_INCREF(state->type_Direct3DSurfaceDescription);

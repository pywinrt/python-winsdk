...
namespace py::cpp::Windows::UI::Text
{
    ...
    static PyType_Slot type_slots_FontWeights_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_FontWeights_Meta },
        { }
    };

    static PyType_Spec type_spec_FontWeights_Meta =
    {
        "_winsdk_Windows_UI_Text.FontWeights_Meta",
    ...
    {
        { Py_tp_base, &PyType_Type },
        { Py_tp_getset, getset_TextConstants_Meta },
        { }
    };

    static PyType_Spec type_spec_TextConstants_Meta =
    {
        "_winsdk_Windows_UI_Text.TextConstants_Meta",
        static_cast<int>(PyType_Type.tp_basicsize),
        static_cast<int>(PyType_Type.tp_itemsize),
        Py_TPFLAGS_DEFAULT,
...
PyMODINIT_FUNC PyInit__winsdk_Windows_UI_Text(void) noexcept
{
    ...
    state->type_ContentLinkInfo = py::register_python_type(module.get(), type_name_ContentLinkInfo, &type_spec_ContentLinkInfo, bases.get(), nullptr);
    ...
    state->type_FontWeights = py::register_python_type(module.get(), type_name_FontWeights, &type_spec_FontWeights, bases.get(), reinterpret_cast<PyTypeObject*>(type_FontWeights_Meta.get()));
    if (!state->type_FontWeights)
    {
        return nullptr;
    }

    state->type_RichEditTextDocument = py::register_python_type(module.get(), type_name_RichEditTextDocument, &type_spec_RichEditTextDocument, bases.get(), nullptr);
    if (!state->type_RichEditTextDocument)
    {
        return nullptr;
    }

    state->type_RichEditTextRange = py::register_python_type(module.get(), type_name_RichEditTextRange, &type_spec_RichEditTextRange, bases.get(), nullptr);
    ...
    state->type_TextConstants = py::register_python_type(module.get(), type_name_TextConstants, &type_spec_TextConstants, nullptr, reinterpret_cast<PyTypeObject*>(type_TextConstants_Meta.get()));
    if (!state->type_TextConstants)
    {
        return nullptr;
    }

    state->type_ITextCharacterFormat = py::register_python_type(module.get(), type_name_ITextCharacterFormat, &type_spec_ITextCharacterFormat, bases.get(), nullptr);
    if (!state->type_ITextCharacterFormat)
    {
        return nullptr;
    }

    state->type_ITextDocument = py::register_python_type(module.get(), type_name_ITextDocument, &type_spec_ITextDocument, bases.get(), nullptr);
    if (!state->type_ITextDocument)
    {
        return nullptr;
    }

    state->type_ITextParagraphFormat = py::register_python_type(module.get(), type_name_ITextParagraphFormat, &type_spec_ITextParagraphFormat, bases.get(), nullptr);
    if (!state->type_ITextParagraphFormat)
    {
        return nullptr;
    }

    state->type_ITextRange = py::register_python_type(module.get(), type_name_ITextRange, &type_spec_ITextRange, bases.get(), nullptr);
    if (!state->type_ITextRange)
    {
        return nullptr;
    }

    state->type_ITextSelection = py::register_python_type(module.get(), type_name_ITextSelection, &type_spec_ITextSelection, bases.get(), nullptr);
    if (!state->type_ITextSelection)
    {
        return nullptr;
    }

    state->type_FontWeight = py::register_python_type(module.get(), type_name_FontWeight, &type_spec_FontWeight, bases.get(), nullptr);

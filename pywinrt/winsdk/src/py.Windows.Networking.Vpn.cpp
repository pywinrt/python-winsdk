...
PyMODINIT_FUNC PyInit__winsdk_Windows_Networking_Vpn(void) noexcept
{
    ...
    state->type_VpnAppId = py::register_python_type(module.get(), type_name_VpnAppId, &type_spec_VpnAppId, bases.get(), nullptr);
    if (!state->type_VpnAppId)
    {
        return nullptr;
    }

    state->type_VpnChannel = py::register_python_type(module.get(), type_name_VpnChannel, &type_spec_VpnChannel, bases.get(), nullptr);
    if (!state->type_VpnChannel)
    {
        return nullptr;
    }

    state->type_VpnChannelActivityEventArgs = py::register_python_type(module.get(), type_name_VpnChannelActivityEventArgs, &type_spec_VpnChannelActivityEventArgs, bases.get(), nullptr);
    if (!state->type_VpnChannelActivityEventArgs)
    {
        return nullptr;
    }

    state->type_VpnChannelActivityStateChangedArgs = py::register_python_type(module.get(), type_name_VpnChannelActivityStateChangedArgs, &type_spec_VpnChannelActivityStateChangedArgs, bases.get(), nullptr);
    if (!state->type_VpnChannelActivityStateChangedArgs)
    {
        return nullptr;
    }

    state->type_VpnChannelConfiguration = py::register_python_type(module.get(), type_name_VpnChannelConfiguration, &type_spec_VpnChannelConfiguration, bases.get(), nullptr);
    if (!state->type_VpnChannelConfiguration)
    {
        return nullptr;
    }

    state->type_VpnCredential = py::register_python_type(module.get(), type_name_VpnCredential, &type_spec_VpnCredential, bases.get(), nullptr);
    if (!state->type_VpnCredential)
    {
        return nullptr;
    }

    state->type_VpnCustomCheckBox = py::register_python_type(module.get(), type_name_VpnCustomCheckBox, &type_spec_VpnCustomCheckBox, bases.get(), nullptr);
    if (!state->type_VpnCustomCheckBox)
    {
        return nullptr;
    }

    state->type_VpnCustomComboBox = py::register_python_type(module.get(), type_name_VpnCustomComboBox, &type_spec_VpnCustomComboBox, bases.get(), nullptr);
    if (!state->type_VpnCustomComboBox)
    {
        return nullptr;
    }

    state->type_VpnCustomEditBox = py::register_python_type(module.get(), type_name_VpnCustomEditBox, &type_spec_VpnCustomEditBox, bases.get(), nullptr);
    if (!state->type_VpnCustomEditBox)
    {
        return nullptr;
    }

    state->type_VpnCustomErrorBox = py::register_python_type(module.get(), type_name_VpnCustomErrorBox, &type_spec_VpnCustomErrorBox, bases.get(), nullptr);
    if (!state->type_VpnCustomErrorBox)
    {
        return nullptr;
    }

    state->type_VpnCustomPromptBooleanInput = py::register_python_type(module.get(), type_name_VpnCustomPromptBooleanInput, &type_spec_VpnCustomPromptBooleanInput, bases.get(), nullptr);
    if (!state->type_VpnCustomPromptBooleanInput)
    {
        return nullptr;
    }

    state->type_VpnCustomPromptOptionSelector = py::register_python_type(module.get(), type_name_VpnCustomPromptOptionSelector, &type_spec_VpnCustomPromptOptionSelector, bases.get(), nullptr);
    if (!state->type_VpnCustomPromptOptionSelector)
    {
        return nullptr;
    }

    state->type_VpnCustomPromptText = py::register_python_type(module.get(), type_name_VpnCustomPromptText, &type_spec_VpnCustomPromptText, bases.get(), nullptr);
    if (!state->type_VpnCustomPromptText)
    {
        return nullptr;
    }

    state->type_VpnCustomPromptTextInput = py::register_python_type(module.get(), type_name_VpnCustomPromptTextInput, &type_spec_VpnCustomPromptTextInput, bases.get(), nullptr);
    if (!state->type_VpnCustomPromptTextInput)
    {
        return nullptr;
    }

    state->type_VpnCustomTextBox = py::register_python_type(module.get(), type_name_VpnCustomTextBox, &type_spec_VpnCustomTextBox, bases.get(), nullptr);
    if (!state->type_VpnCustomTextBox)
    {
        return nullptr;
    }

    state->type_VpnDomainNameAssignment = py::register_python_type(module.get(), type_name_VpnDomainNameAssignment, &type_spec_VpnDomainNameAssignment, bases.get(), nullptr);
    if (!state->type_VpnDomainNameAssignment)
    {
        return nullptr;
    }

    state->type_VpnDomainNameInfo = py::register_python_type(module.get(), type_name_VpnDomainNameInfo, &type_spec_VpnDomainNameInfo, bases.get(), nullptr);
    if (!state->type_VpnDomainNameInfo)
    {
        return nullptr;
    }

    state->type_VpnForegroundActivatedEventArgs = py::register_python_type(module.get(), type_name_VpnForegroundActivatedEventArgs, &type_spec_VpnForegroundActivatedEventArgs, bases.get(), nullptr);
    if (!state->type_VpnForegroundActivatedEventArgs)
    {
        return nullptr;
    }

    state->type_VpnForegroundActivationOperation = py::register_python_type(module.get(), type_name_VpnForegroundActivationOperation, &type_spec_VpnForegroundActivationOperation, bases.get(), nullptr);
    if (!state->type_VpnForegroundActivationOperation)
    {
        return nullptr;
    }

    state->type_VpnInterfaceId = py::register_python_type(module.get(), type_name_VpnInterfaceId, &type_spec_VpnInterfaceId, bases.get(), nullptr);
    if (!state->type_VpnInterfaceId)
    {
        return nullptr;
    }

    state->type_VpnManagementAgent = py::register_python_type(module.get(), type_name_VpnManagementAgent, &type_spec_VpnManagementAgent, bases.get(), nullptr);
    if (!state->type_VpnManagementAgent)
    {
        return nullptr;
    }

    state->type_VpnNamespaceAssignment = py::register_python_type(module.get(), type_name_VpnNamespaceAssignment, &type_spec_VpnNamespaceAssignment, bases.get(), nullptr);
    if (!state->type_VpnNamespaceAssignment)
    {
        return nullptr;
    }

    state->type_VpnNamespaceInfo = py::register_python_type(module.get(), type_name_VpnNamespaceInfo, &type_spec_VpnNamespaceInfo, bases.get(), nullptr);
    if (!state->type_VpnNamespaceInfo)
    {
        return nullptr;
    }

    state->type_VpnNativeProfile = py::register_python_type(module.get(), type_name_VpnNativeProfile, &type_spec_VpnNativeProfile, bases.get(), nullptr);
    if (!state->type_VpnNativeProfile)
    {
        return nullptr;
    }

    state->type_VpnPacketBuffer = py::register_python_type(module.get(), type_name_VpnPacketBuffer, &type_spec_VpnPacketBuffer, bases.get(), nullptr);
    if (!state->type_VpnPacketBuffer)
    {
        return nullptr;
    }

    state->type_VpnPacketBufferList = py::register_python_type(module.get(), type_name_VpnPacketBufferList, &type_spec_VpnPacketBufferList, bases.get(), nullptr);
    if (!state->type_VpnPacketBufferList)
    {
        return nullptr;
    }

    state->type_VpnPickedCredential = py::register_python_type(module.get(), type_name_VpnPickedCredential, &type_spec_VpnPickedCredential, bases.get(), nullptr);
    if (!state->type_VpnPickedCredential)
    {
        return nullptr;
    }

    state->type_VpnPlugInProfile = py::register_python_type(module.get(), type_name_VpnPlugInProfile, &type_spec_VpnPlugInProfile, bases.get(), nullptr);
    if (!state->type_VpnPlugInProfile)
    {
        return nullptr;
    }

    state->type_VpnRoute = py::register_python_type(module.get(), type_name_VpnRoute, &type_spec_VpnRoute, bases.get(), nullptr);
    if (!state->type_VpnRoute)
    {
        return nullptr;
    }

    state->type_VpnRouteAssignment = py::register_python_type(module.get(), type_name_VpnRouteAssignment, &type_spec_VpnRouteAssignment, bases.get(), nullptr);
    if (!state->type_VpnRouteAssignment)
    {
        return nullptr;
    }

    state->type_VpnSystemHealth = py::register_python_type(module.get(), type_name_VpnSystemHealth, &type_spec_VpnSystemHealth, bases.get(), nullptr);
    if (!state->type_VpnSystemHealth)
    {
        return nullptr;
    }

    state->type_VpnTrafficFilter = py::register_python_type(module.get(), type_name_VpnTrafficFilter, &type_spec_VpnTrafficFilter, bases.get(), nullptr);
    if (!state->type_VpnTrafficFilter)
    {
        return nullptr;
    }

    state->type_VpnTrafficFilterAssignment = py::register_python_type(module.get(), type_name_VpnTrafficFilterAssignment, &type_spec_VpnTrafficFilterAssignment, bases.get(), nullptr);
    if (!state->type_VpnTrafficFilterAssignment)
    {
        return nullptr;
    }

    state->type_IVpnChannelStatics = py::register_python_type(module.get(), type_name_IVpnChannelStatics, &type_spec_IVpnChannelStatics, bases.get(), nullptr);
    if (!state->type_IVpnChannelStatics)
    {
        return nullptr;
    }

    state->type_IVpnCredential = py::register_python_type(module.get(), type_name_IVpnCredential, &type_spec_IVpnCredential, bases.get(), nullptr);
    if (!state->type_IVpnCredential)
    {
        return nullptr;
    }

    state->type_IVpnCustomPrompt = py::register_python_type(module.get(), type_name_IVpnCustomPrompt, &type_spec_IVpnCustomPrompt, bases.get(), nullptr);
    if (!state->type_IVpnCustomPrompt)
    {
        return nullptr;
    }

    state->type_IVpnCustomPromptElement = py::register_python_type(module.get(), type_name_IVpnCustomPromptElement, &type_spec_IVpnCustomPromptElement, bases.get(), nullptr);
    if (!state->type_IVpnCustomPromptElement)
    {
        return nullptr;
    }

    state->type_IVpnDomainNameInfoFactory = py::register_python_type(module.get(), type_name_IVpnDomainNameInfoFactory, &type_spec_IVpnDomainNameInfoFactory, bases.get(), nullptr);
    if (!state->type_IVpnDomainNameInfoFactory)
    {
        return nullptr;
    }

    state->type_IVpnInterfaceIdFactory = py::register_python_type(module.get(), type_name_IVpnInterfaceIdFactory, &type_spec_IVpnInterfaceIdFactory, bases.get(), nullptr);
    if (!state->type_IVpnInterfaceIdFactory)
    {
        return nullptr;
    }

    state->type_IVpnNamespaceInfoFactory = py::register_python_type(module.get(), type_name_IVpnNamespaceInfoFactory, &type_spec_IVpnNamespaceInfoFactory, bases.get(), nullptr);
    if (!state->type_IVpnNamespaceInfoFactory)
    {
        return nullptr;
    }

    state->type_IVpnPacketBufferFactory = py::register_python_type(module.get(), type_name_IVpnPacketBufferFactory, &type_spec_IVpnPacketBufferFactory, bases.get(), nullptr);
    if (!state->type_IVpnPacketBufferFactory)
    {
        return nullptr;
    }

    state->type_IVpnPlugIn = py::register_python_type(module.get(), type_name_IVpnPlugIn, &type_spec_IVpnPlugIn, bases.get(), nullptr);
    if (!state->type_IVpnPlugIn)
    {
        return nullptr;
    }

    state->type_IVpnProfile = py::register_python_type(module.get(), type_name_IVpnProfile, &type_spec_IVpnProfile, bases.get(), nullptr);
    if (!state->type_IVpnProfile)
    {
        return nullptr;
    }

    state->type_IVpnRouteFactory = py::register_python_type(module.get(), type_name_IVpnRouteFactory, &type_spec_IVpnRouteFactory, bases.get(), nullptr);

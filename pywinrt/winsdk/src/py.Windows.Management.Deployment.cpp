...
namespace py::cpp::Windows::Management::Deployment
{
    ...
    constexpr const char* const type_name_AddPackageOptions = "AddPackageOptions";
    ...
    constexpr const char* const type_name_AppInstallerManager = "AppInstallerManager";
    ...
    constexpr const char* const type_name_AutoUpdateSettingsOptions = "AutoUpdateSettingsOptions";
    ...
    constexpr const char* const type_name_PackageAllUserProvisioningOptions = "PackageAllUserProvisioningOptions";
    ...
    constexpr const char* const type_name_PackageManager = "PackageManager";
    ...
    constexpr const char* const type_name_PackageManagerDebugSettings = "PackageManagerDebugSettings";
    ...
    constexpr const char* const type_name_PackageUserInformation = "PackageUserInformation";
    ...
    // ----- PackageVolume class --------------------
    constexpr const char* const type_name_PackageVolume = "PackageVolume";
    ...
    constexpr const char* const type_name_RegisterPackageOptions = "RegisterPackageOptions";
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Management_Deployment(void) noexcept
{
    ...
    state->type_AddPackageOptions = py::register_python_type(module.get(), type_name_AddPackageOptions, &type_spec_AddPackageOptions, bases.get(), nullptr);
    if (!state->type_AddPackageOptions)
    {
        return nullptr;
    }

    state->type_AppInstallerManager = py::register_python_type(module.get(), type_name_AppInstallerManager, &type_spec_AppInstallerManager, bases.get(), nullptr);
    if (!state->type_AppInstallerManager)
    {
        return nullptr;
    }

    state->type_AutoUpdateSettingsOptions = py::register_python_type(module.get(), type_name_AutoUpdateSettingsOptions, &type_spec_AutoUpdateSettingsOptions, bases.get(), nullptr);
    if (!state->type_AutoUpdateSettingsOptions)
    {
        return nullptr;
    }

    state->type_CreateSharedPackageContainerOptions = py::register_python_type(module.get(), type_name_CreateSharedPackageContainerOptions, &type_spec_CreateSharedPackageContainerOptions, bases.get(), nullptr);
    if (!state->type_CreateSharedPackageContainerOptions)
    {
        return nullptr;
    }

    state->type_CreateSharedPackageContainerResult = py::register_python_type(module.get(), type_name_CreateSharedPackageContainerResult, &type_spec_CreateSharedPackageContainerResult, bases.get(), nullptr);
    if (!state->type_CreateSharedPackageContainerResult)
    {
        return nullptr;
    }

    state->type_DeleteSharedPackageContainerOptions = py::register_python_type(module.get(), type_name_DeleteSharedPackageContainerOptions, &type_spec_DeleteSharedPackageContainerOptions, bases.get(), nullptr);
    if (!state->type_DeleteSharedPackageContainerOptions)
    {
        return nullptr;
    }

    state->type_DeleteSharedPackageContainerResult = py::register_python_type(module.get(), type_name_DeleteSharedPackageContainerResult, &type_spec_DeleteSharedPackageContainerResult, bases.get(), nullptr);
    if (!state->type_DeleteSharedPackageContainerResult)
    {
        return nullptr;
    }

    state->type_DeploymentResult = py::register_python_type(module.get(), type_name_DeploymentResult, &type_spec_DeploymentResult, bases.get(), nullptr);
    if (!state->type_DeploymentResult)
    {
        return nullptr;
    }

    state->type_FindSharedPackageContainerOptions = py::register_python_type(module.get(), type_name_FindSharedPackageContainerOptions, &type_spec_FindSharedPackageContainerOptions, bases.get(), nullptr);
    if (!state->type_FindSharedPackageContainerOptions)
    {
        return nullptr;
    }

    state->type_PackageAllUserProvisioningOptions = py::register_python_type(module.get(), type_name_PackageAllUserProvisioningOptions, &type_spec_PackageAllUserProvisioningOptions, bases.get(), nullptr);
    if (!state->type_PackageAllUserProvisioningOptions)
    {
        return nullptr;
    }

    state->type_PackageManager = py::register_python_type(module.get(), type_name_PackageManager, &type_spec_PackageManager, bases.get(), nullptr);
    if (!state->type_PackageManager)
    {
        return nullptr;
    }

    state->type_PackageManagerDebugSettings = py::register_python_type(module.get(), type_name_PackageManagerDebugSettings, &type_spec_PackageManagerDebugSettings, bases.get(), nullptr);
    if (!state->type_PackageManagerDebugSettings)
    {
        return nullptr;
    }

    state->type_PackageUserInformation = py::register_python_type(module.get(), type_name_PackageUserInformation, &type_spec_PackageUserInformation, bases.get(), nullptr);
    if (!state->type_PackageUserInformation)
    {
        return nullptr;
    }

    state->type_PackageVolume = py::register_python_type(module.get(), type_name_PackageVolume, &type_spec_PackageVolume, bases.get(), nullptr);
    if (!state->type_PackageVolume)
    {
        return nullptr;
    }

    state->type_RegisterPackageOptions = py::register_python_type(module.get(), type_name_RegisterPackageOptions, &type_spec_RegisterPackageOptions, bases.get(), nullptr);
    if (!state->type_RegisterPackageOptions)
    {
        return nullptr;
    }

    state->type_SharedPackageContainer = py::register_python_type(module.get(), type_name_SharedPackageContainer, &type_spec_SharedPackageContainer, bases.get(), nullptr);
    if (!state->type_SharedPackageContainer)
    {
        return nullptr;
    }

    state->type_SharedPackageContainerManager = py::register_python_type(module.get(), type_name_SharedPackageContainerManager, &type_spec_SharedPackageContainerManager, bases.get(), nullptr);
    if (!state->type_SharedPackageContainerManager)
    {
        return nullptr;
    }

    state->type_SharedPackageContainerMember = py::register_python_type(module.get(), type_name_SharedPackageContainerMember, &type_spec_SharedPackageContainerMember, bases.get(), nullptr);
    if (!state->type_SharedPackageContainerMember)
    {
        return nullptr;
    }

    state->type_StagePackageOptions = py::register_python_type(module.get(), type_name_StagePackageOptions, &type_spec_StagePackageOptions, bases.get(), nullptr);
    if (!state->type_StagePackageOptions)
    {
        return nullptr;
    }

    state->type_UpdateSharedPackageContainerOptions = py::register_python_type(module.get(), type_name_UpdateSharedPackageContainerOptions, &type_spec_UpdateSharedPackageContainerOptions, bases.get(), nullptr);
    if (!state->type_UpdateSharedPackageContainerOptions)
    {
        return nullptr;
    }

    state->type_UpdateSharedPackageContainerResult = py::register_python_type(module.get(), type_name_UpdateSharedPackageContainerResult, &type_spec_UpdateSharedPackageContainerResult, bases.get(), nullptr);
    if (!state->type_UpdateSharedPackageContainerResult)
    {
        return nullptr;
    }

    state->type_DeploymentProgress = py::register_python_type(module.get(), type_name_DeploymentProgress, &type_spec_DeploymentProgress, bases.get(), nullptr);

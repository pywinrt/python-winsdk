...
namespace py::cpp::Windows::Storage::Compression
{
    ...
    static PyObject* register_CompressAlgorithm(PyObject* module, PyObject* type)
    {
    ...
    // ----- Compressor class --------------------
    constexpr const char* const type_name_Compressor = "Compressor";
    ...
    // ----- Decompressor class --------------------
    constexpr const char* const type_name_Decompressor = "Decompressor";
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Storage_Compression(void) noexcept
{
    ...
    state->type_Compressor = py::register_python_type(module.get(), type_name_Compressor, &type_spec_Compressor, bases.get());
    if (!state->type_Compressor)
    {
        return nullptr;
    }

    Py_INCREF(state->type_Compressor);

    state->type_Decompressor = py::register_python_type(module.get(), type_name_Decompressor, &type_spec_Decompressor, bases.get());
    if (!state->type_Decompressor)
    {
        return nullptr;
    }

    Py_INCREF(state->type_Decompressor);

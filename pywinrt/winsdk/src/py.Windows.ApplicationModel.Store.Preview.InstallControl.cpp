...
PyMODINIT_FUNC PyInit__winsdk_Windows_ApplicationModel_Store_Preview_InstallControl(void) noexcept
{
    ...
    state->type_AppInstallItem = py::register_python_type(module.get(), type_name_AppInstallItem, &type_spec_AppInstallItem, bases.get(), nullptr);
    if (!state->type_AppInstallItem)
    {
        return nullptr;
    }

    state->type_AppInstallManager = py::register_python_type(module.get(), type_name_AppInstallManager, &type_spec_AppInstallManager, bases.get(), nullptr);
    if (!state->type_AppInstallManager)
    {
        return nullptr;
    }

    state->type_AppInstallManagerItemEventArgs = py::register_python_type(module.get(), type_name_AppInstallManagerItemEventArgs, &type_spec_AppInstallManagerItemEventArgs, bases.get(), nullptr);
    if (!state->type_AppInstallManagerItemEventArgs)
    {
        return nullptr;
    }

    state->type_AppInstallOptions = py::register_python_type(module.get(), type_name_AppInstallOptions, &type_spec_AppInstallOptions, bases.get(), nullptr);
    if (!state->type_AppInstallOptions)
    {
        return nullptr;
    }

    state->type_AppInstallStatus = py::register_python_type(module.get(), type_name_AppInstallStatus, &type_spec_AppInstallStatus, bases.get(), nullptr);
    if (!state->type_AppInstallStatus)
    {
        return nullptr;
    }

    state->type_AppUpdateOptions = py::register_python_type(module.get(), type_name_AppUpdateOptions, &type_spec_AppUpdateOptions, bases.get(), nullptr);
    if (!state->type_AppUpdateOptions)
    {
        return nullptr;
    }

    state->type_GetEntitlementResult = py::register_python_type(module.get(), type_name_GetEntitlementResult, &type_spec_GetEntitlementResult, bases.get(), nullptr);

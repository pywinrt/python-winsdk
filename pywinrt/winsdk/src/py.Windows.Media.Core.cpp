...
namespace py::cpp::Windows::Media::Core
{
    ...
    static PyType_Slot type_slots_CodecSubtypes_Meta[] = 
    {
        ...
        { Py_tp_getset, getset_CodecSubtypes_Meta },
        { }
    };

    static PyType_Spec type_spec_CodecSubtypes_Meta =
    {
        "_winsdk_Windows_Media_Core.CodecSubtypes_Meta",
    ...
    {
        { Py_tp_base, &PyType_Type },
        { Py_tp_getset, getset_LowLightFusion_Meta },
        { }
    };

    static PyType_Spec type_spec_LowLightFusion_Meta =
    {
        "_winsdk_Windows_Media_Core.LowLightFusion_Meta",
        static_cast<int>(PyType_Type.tp_basicsize),
        static_cast<int>(PyType_Type.tp_itemsize),
        Py_TPFLAGS_DEFAULT,
...
PyMODINIT_FUNC PyInit__winsdk_Windows_Media_Core(void) noexcept
{
    ...
    state->type_AudioStreamDescriptor = py::register_python_type(module.get(), type_name_AudioStreamDescriptor, &type_spec_AudioStreamDescriptor, bases.get(), nullptr);
    if (!state->type_AudioStreamDescriptor)
    {
        return nullptr;
    }

    state->type_AudioTrack = py::register_python_type(module.get(), type_name_AudioTrack, &type_spec_AudioTrack, bases.get(), nullptr);
    if (!state->type_AudioTrack)
    {
        return nullptr;
    }

    state->type_AudioTrackOpenFailedEventArgs = py::register_python_type(module.get(), type_name_AudioTrackOpenFailedEventArgs, &type_spec_AudioTrackOpenFailedEventArgs, bases.get(), nullptr);
    if (!state->type_AudioTrackOpenFailedEventArgs)
    {
        return nullptr;
    }

    state->type_AudioTrackSupportInfo = py::register_python_type(module.get(), type_name_AudioTrackSupportInfo, &type_spec_AudioTrackSupportInfo, bases.get(), nullptr);
    if (!state->type_AudioTrackSupportInfo)
    {
        return nullptr;
    }

    state->type_ChapterCue = py::register_python_type(module.get(), type_name_ChapterCue, &type_spec_ChapterCue, bases.get(), nullptr);
    if (!state->type_ChapterCue)
    {
        return nullptr;
    }

    state->type_CodecInfo = py::register_python_type(module.get(), type_name_CodecInfo, &type_spec_CodecInfo, bases.get(), nullptr);
    if (!state->type_CodecInfo)
    {
        return nullptr;
    }

    state->type_CodecQuery = py::register_python_type(module.get(), type_name_CodecQuery, &type_spec_CodecQuery, bases.get(), nullptr);
    ...
    state->type_CodecSubtypes = py::register_python_type(module.get(), type_name_CodecSubtypes, &type_spec_CodecSubtypes, nullptr, reinterpret_cast<PyTypeObject*>(type_CodecSubtypes_Meta.get()));
    if (!state->type_CodecSubtypes)
    {
        return nullptr;
    }

    state->type_DataCue = py::register_python_type(module.get(), type_name_DataCue, &type_spec_DataCue, bases.get(), nullptr);
    if (!state->type_DataCue)
    {
        return nullptr;
    }

    state->type_FaceDetectedEventArgs = py::register_python_type(module.get(), type_name_FaceDetectedEventArgs, &type_spec_FaceDetectedEventArgs, bases.get(), nullptr);
    if (!state->type_FaceDetectedEventArgs)
    {
        return nullptr;
    }

    state->type_FaceDetectionEffect = py::register_python_type(module.get(), type_name_FaceDetectionEffect, &type_spec_FaceDetectionEffect, bases.get(), nullptr);
    if (!state->type_FaceDetectionEffect)
    {
        return nullptr;
    }

    state->type_FaceDetectionEffectDefinition = py::register_python_type(module.get(), type_name_FaceDetectionEffectDefinition, &type_spec_FaceDetectionEffectDefinition, bases.get(), nullptr);
    if (!state->type_FaceDetectionEffectDefinition)
    {
        return nullptr;
    }

    state->type_FaceDetectionEffectFrame = py::register_python_type(module.get(), type_name_FaceDetectionEffectFrame, &type_spec_FaceDetectionEffectFrame, bases.get(), nullptr);
    if (!state->type_FaceDetectionEffectFrame)
    {
        return nullptr;
    }

    state->type_HighDynamicRangeControl = py::register_python_type(module.get(), type_name_HighDynamicRangeControl, &type_spec_HighDynamicRangeControl, bases.get(), nullptr);
    if (!state->type_HighDynamicRangeControl)
    {
        return nullptr;
    }

    state->type_HighDynamicRangeOutput = py::register_python_type(module.get(), type_name_HighDynamicRangeOutput, &type_spec_HighDynamicRangeOutput, bases.get(), nullptr);
    if (!state->type_HighDynamicRangeOutput)
    {
        return nullptr;
    }

    state->type_ImageCue = py::register_python_type(module.get(), type_name_ImageCue, &type_spec_ImageCue, bases.get(), nullptr);
    if (!state->type_ImageCue)
    {
        return nullptr;
    }

    state->type_InitializeMediaStreamSourceRequestedEventArgs = py::register_python_type(module.get(), type_name_InitializeMediaStreamSourceRequestedEventArgs, &type_spec_InitializeMediaStreamSourceRequestedEventArgs, bases.get(), nullptr);
    ...
    state->type_LowLightFusion = py::register_python_type(module.get(), type_name_LowLightFusion, &type_spec_LowLightFusion, nullptr, reinterpret_cast<PyTypeObject*>(type_LowLightFusion_Meta.get()));
    if (!state->type_LowLightFusion)
    {
        return nullptr;
    }

    state->type_LowLightFusionResult = py::register_python_type(module.get(), type_name_LowLightFusionResult, &type_spec_LowLightFusionResult, bases.get(), nullptr);
    if (!state->type_LowLightFusionResult)
    {
        return nullptr;
    }

    state->type_MediaBinder = py::register_python_type(module.get(), type_name_MediaBinder, &type_spec_MediaBinder, bases.get(), nullptr);
    if (!state->type_MediaBinder)
    {
        return nullptr;
    }

    state->type_MediaBindingEventArgs = py::register_python_type(module.get(), type_name_MediaBindingEventArgs, &type_spec_MediaBindingEventArgs, bases.get(), nullptr);
    if (!state->type_MediaBindingEventArgs)
    {
        return nullptr;
    }

    state->type_MediaCueEventArgs = py::register_python_type(module.get(), type_name_MediaCueEventArgs, &type_spec_MediaCueEventArgs, bases.get(), nullptr);
    if (!state->type_MediaCueEventArgs)
    {
        return nullptr;
    }

    state->type_MediaSource = py::register_python_type(module.get(), type_name_MediaSource, &type_spec_MediaSource, bases.get(), nullptr);
    if (!state->type_MediaSource)
    {
        return nullptr;
    }

    state->type_MediaSourceAppServiceConnection = py::register_python_type(module.get(), type_name_MediaSourceAppServiceConnection, &type_spec_MediaSourceAppServiceConnection, bases.get(), nullptr);
    if (!state->type_MediaSourceAppServiceConnection)
    {
        return nullptr;
    }

    state->type_MediaSourceError = py::register_python_type(module.get(), type_name_MediaSourceError, &type_spec_MediaSourceError, bases.get(), nullptr);
    if (!state->type_MediaSourceError)
    {
        return nullptr;
    }

    state->type_MediaSourceOpenOperationCompletedEventArgs = py::register_python_type(module.get(), type_name_MediaSourceOpenOperationCompletedEventArgs, &type_spec_MediaSourceOpenOperationCompletedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaSourceOpenOperationCompletedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaSourceStateChangedEventArgs = py::register_python_type(module.get(), type_name_MediaSourceStateChangedEventArgs, &type_spec_MediaSourceStateChangedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaSourceStateChangedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaStreamSample = py::register_python_type(module.get(), type_name_MediaStreamSample, &type_spec_MediaStreamSample, bases.get(), nullptr);
    if (!state->type_MediaStreamSample)
    {
        return nullptr;
    }

    state->type_MediaStreamSamplePropertySet = py::register_python_type(module.get(), type_name_MediaStreamSamplePropertySet, &type_spec_MediaStreamSamplePropertySet, mutable_mapping_bases.get(), nullptr);
    if (!state->type_MediaStreamSamplePropertySet)
    {
        return nullptr;
    }

    state->type_MediaStreamSampleProtectionProperties = py::register_python_type(module.get(), type_name_MediaStreamSampleProtectionProperties, &type_spec_MediaStreamSampleProtectionProperties, bases.get(), nullptr);
    if (!state->type_MediaStreamSampleProtectionProperties)
    {
        return nullptr;
    }

    state->type_MediaStreamSource = py::register_python_type(module.get(), type_name_MediaStreamSource, &type_spec_MediaStreamSource, bases.get(), nullptr);
    if (!state->type_MediaStreamSource)
    {
        return nullptr;
    }

    state->type_MediaStreamSourceClosedEventArgs = py::register_python_type(module.get(), type_name_MediaStreamSourceClosedEventArgs, &type_spec_MediaStreamSourceClosedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaStreamSourceClosedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaStreamSourceClosedRequest = py::register_python_type(module.get(), type_name_MediaStreamSourceClosedRequest, &type_spec_MediaStreamSourceClosedRequest, bases.get(), nullptr);
    if (!state->type_MediaStreamSourceClosedRequest)
    {
        return nullptr;
    }

    state->type_MediaStreamSourceSampleRenderedEventArgs = py::register_python_type(module.get(), type_name_MediaStreamSourceSampleRenderedEventArgs, &type_spec_MediaStreamSourceSampleRenderedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaStreamSourceSampleRenderedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaStreamSourceSampleRequest = py::register_python_type(module.get(), type_name_MediaStreamSourceSampleRequest, &type_spec_MediaStreamSourceSampleRequest, bases.get(), nullptr);
    if (!state->type_MediaStreamSourceSampleRequest)
    {
        return nullptr;
    }

    state->type_MediaStreamSourceSampleRequestDeferral = py::register_python_type(module.get(), type_name_MediaStreamSourceSampleRequestDeferral, &type_spec_MediaStreamSourceSampleRequestDeferral, bases.get(), nullptr);
    if (!state->type_MediaStreamSourceSampleRequestDeferral)
    {
        return nullptr;
    }

    state->type_MediaStreamSourceSampleRequestedEventArgs = py::register_python_type(module.get(), type_name_MediaStreamSourceSampleRequestedEventArgs, &type_spec_MediaStreamSourceSampleRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaStreamSourceSampleRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_MediaStreamSourceStartingEventArgs = py::register_python_type(module.get(), type_name_MediaStreamSourceStartingEventArgs, &type_spec_MediaStreamSourceStartingEventArgs, bases.get(), nullptr);
    if (!state->type_MediaStreamSourceStartingEventArgs)
    {
        return nullptr;
    }

    state->type_MediaStreamSourceStartingRequest = py::register_python_type(module.get(), type_name_MediaStreamSourceStartingRequest, &type_spec_MediaStreamSourceStartingRequest, bases.get(), nullptr);
    if (!state->type_MediaStreamSourceStartingRequest)
    {
        return nullptr;
    }

    state->type_MediaStreamSourceStartingRequestDeferral = py::register_python_type(module.get(), type_name_MediaStreamSourceStartingRequestDeferral, &type_spec_MediaStreamSourceStartingRequestDeferral, bases.get(), nullptr);
    if (!state->type_MediaStreamSourceStartingRequestDeferral)
    {
        return nullptr;
    }

    state->type_MediaStreamSourceSwitchStreamsRequest = py::register_python_type(module.get(), type_name_MediaStreamSourceSwitchStreamsRequest, &type_spec_MediaStreamSourceSwitchStreamsRequest, bases.get(), nullptr);
    if (!state->type_MediaStreamSourceSwitchStreamsRequest)
    {
        return nullptr;
    }

    state->type_MediaStreamSourceSwitchStreamsRequestDeferral = py::register_python_type(module.get(), type_name_MediaStreamSourceSwitchStreamsRequestDeferral, &type_spec_MediaStreamSourceSwitchStreamsRequestDeferral, bases.get(), nullptr);
    if (!state->type_MediaStreamSourceSwitchStreamsRequestDeferral)
    {
        return nullptr;
    }

    state->type_MediaStreamSourceSwitchStreamsRequestedEventArgs = py::register_python_type(module.get(), type_name_MediaStreamSourceSwitchStreamsRequestedEventArgs, &type_spec_MediaStreamSourceSwitchStreamsRequestedEventArgs, bases.get(), nullptr);
    if (!state->type_MediaStreamSourceSwitchStreamsRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_MseSourceBuffer = py::register_python_type(module.get(), type_name_MseSourceBuffer, &type_spec_MseSourceBuffer, bases.get(), nullptr);
    if (!state->type_MseSourceBuffer)
    {
        return nullptr;
    }

    state->type_MseSourceBufferList = py::register_python_type(module.get(), type_name_MseSourceBufferList, &type_spec_MseSourceBufferList, bases.get(), nullptr);
    if (!state->type_MseSourceBufferList)
    {
        return nullptr;
    }

    state->type_MseStreamSource = py::register_python_type(module.get(), type_name_MseStreamSource, &type_spec_MseStreamSource, bases.get(), nullptr);
    if (!state->type_MseStreamSource)
    {
        return nullptr;
    }

    state->type_SceneAnalysisEffect = py::register_python_type(module.get(), type_name_SceneAnalysisEffect, &type_spec_SceneAnalysisEffect, bases.get(), nullptr);
    if (!state->type_SceneAnalysisEffect)
    {
        return nullptr;
    }

    state->type_SceneAnalysisEffectDefinition = py::register_python_type(module.get(), type_name_SceneAnalysisEffectDefinition, &type_spec_SceneAnalysisEffectDefinition, bases.get(), nullptr);
    if (!state->type_SceneAnalysisEffectDefinition)
    {
        return nullptr;
    }

    state->type_SceneAnalysisEffectFrame = py::register_python_type(module.get(), type_name_SceneAnalysisEffectFrame, &type_spec_SceneAnalysisEffectFrame, bases.get(), nullptr);
    if (!state->type_SceneAnalysisEffectFrame)
    {
        return nullptr;
    }

    state->type_SceneAnalyzedEventArgs = py::register_python_type(module.get(), type_name_SceneAnalyzedEventArgs, &type_spec_SceneAnalyzedEventArgs, bases.get(), nullptr);
    if (!state->type_SceneAnalyzedEventArgs)
    {
        return nullptr;
    }

    state->type_SpeechCue = py::register_python_type(module.get(), type_name_SpeechCue, &type_spec_SpeechCue, bases.get(), nullptr);
    if (!state->type_SpeechCue)
    {
        return nullptr;
    }

    state->type_TimedMetadataStreamDescriptor = py::register_python_type(module.get(), type_name_TimedMetadataStreamDescriptor, &type_spec_TimedMetadataStreamDescriptor, bases.get(), nullptr);
    if (!state->type_TimedMetadataStreamDescriptor)
    {
        return nullptr;
    }

    state->type_TimedMetadataTrack = py::register_python_type(module.get(), type_name_TimedMetadataTrack, &type_spec_TimedMetadataTrack, bases.get(), nullptr);
    if (!state->type_TimedMetadataTrack)
    {
        return nullptr;
    }

    state->type_TimedMetadataTrackError = py::register_python_type(module.get(), type_name_TimedMetadataTrackError, &type_spec_TimedMetadataTrackError, bases.get(), nullptr);
    if (!state->type_TimedMetadataTrackError)
    {
        return nullptr;
    }

    state->type_TimedMetadataTrackFailedEventArgs = py::register_python_type(module.get(), type_name_TimedMetadataTrackFailedEventArgs, &type_spec_TimedMetadataTrackFailedEventArgs, bases.get(), nullptr);
    if (!state->type_TimedMetadataTrackFailedEventArgs)
    {
        return nullptr;
    }

    state->type_TimedTextBouten = py::register_python_type(module.get(), type_name_TimedTextBouten, &type_spec_TimedTextBouten, bases.get(), nullptr);
    if (!state->type_TimedTextBouten)
    {
        return nullptr;
    }

    state->type_TimedTextCue = py::register_python_type(module.get(), type_name_TimedTextCue, &type_spec_TimedTextCue, bases.get(), nullptr);
    if (!state->type_TimedTextCue)
    {
        return nullptr;
    }

    state->type_TimedTextLine = py::register_python_type(module.get(), type_name_TimedTextLine, &type_spec_TimedTextLine, bases.get(), nullptr);
    if (!state->type_TimedTextLine)
    {
        return nullptr;
    }

    state->type_TimedTextRegion = py::register_python_type(module.get(), type_name_TimedTextRegion, &type_spec_TimedTextRegion, bases.get(), nullptr);
    if (!state->type_TimedTextRegion)
    {
        return nullptr;
    }

    state->type_TimedTextRuby = py::register_python_type(module.get(), type_name_TimedTextRuby, &type_spec_TimedTextRuby, bases.get(), nullptr);
    if (!state->type_TimedTextRuby)
    {
        return nullptr;
    }

    state->type_TimedTextSource = py::register_python_type(module.get(), type_name_TimedTextSource, &type_spec_TimedTextSource, bases.get(), nullptr);
    if (!state->type_TimedTextSource)
    {
        return nullptr;
    }

    state->type_TimedTextSourceResolveResultEventArgs = py::register_python_type(module.get(), type_name_TimedTextSourceResolveResultEventArgs, &type_spec_TimedTextSourceResolveResultEventArgs, bases.get(), nullptr);
    if (!state->type_TimedTextSourceResolveResultEventArgs)
    {
        return nullptr;
    }

    state->type_TimedTextStyle = py::register_python_type(module.get(), type_name_TimedTextStyle, &type_spec_TimedTextStyle, bases.get(), nullptr);
    if (!state->type_TimedTextStyle)
    {
        return nullptr;
    }

    state->type_TimedTextSubformat = py::register_python_type(module.get(), type_name_TimedTextSubformat, &type_spec_TimedTextSubformat, bases.get(), nullptr);
    if (!state->type_TimedTextSubformat)
    {
        return nullptr;
    }

    state->type_VideoStabilizationEffect = py::register_python_type(module.get(), type_name_VideoStabilizationEffect, &type_spec_VideoStabilizationEffect, bases.get(), nullptr);
    if (!state->type_VideoStabilizationEffect)
    {
        return nullptr;
    }

    state->type_VideoStabilizationEffectDefinition = py::register_python_type(module.get(), type_name_VideoStabilizationEffectDefinition, &type_spec_VideoStabilizationEffectDefinition, bases.get(), nullptr);
    if (!state->type_VideoStabilizationEffectDefinition)
    {
        return nullptr;
    }

    state->type_VideoStabilizationEffectEnabledChangedEventArgs = py::register_python_type(module.get(), type_name_VideoStabilizationEffectEnabledChangedEventArgs, &type_spec_VideoStabilizationEffectEnabledChangedEventArgs, bases.get(), nullptr);
    if (!state->type_VideoStabilizationEffectEnabledChangedEventArgs)
    {
        return nullptr;
    }

    state->type_VideoStreamDescriptor = py::register_python_type(module.get(), type_name_VideoStreamDescriptor, &type_spec_VideoStreamDescriptor, bases.get(), nullptr);
    if (!state->type_VideoStreamDescriptor)
    {
        return nullptr;
    }

    state->type_VideoTrack = py::register_python_type(module.get(), type_name_VideoTrack, &type_spec_VideoTrack, bases.get(), nullptr);
    if (!state->type_VideoTrack)
    {
        return nullptr;
    }

    state->type_VideoTrackOpenFailedEventArgs = py::register_python_type(module.get(), type_name_VideoTrackOpenFailedEventArgs, &type_spec_VideoTrackOpenFailedEventArgs, bases.get(), nullptr);
    if (!state->type_VideoTrackOpenFailedEventArgs)
    {
        return nullptr;
    }

    state->type_VideoTrackSupportInfo = py::register_python_type(module.get(), type_name_VideoTrackSupportInfo, &type_spec_VideoTrackSupportInfo, bases.get(), nullptr);
    if (!state->type_VideoTrackSupportInfo)
    {
        return nullptr;
    }

    state->type_IMediaCue = py::register_python_type(module.get(), type_name_IMediaCue, &type_spec_IMediaCue, bases.get(), nullptr);
    if (!state->type_IMediaCue)
    {
        return nullptr;
    }

    state->type_IMediaSource = py::register_python_type(module.get(), type_name_IMediaSource, &type_spec_IMediaSource, bases.get(), nullptr);
    if (!state->type_IMediaSource)
    {
        return nullptr;
    }

    state->type_IMediaStreamDescriptor = py::register_python_type(module.get(), type_name_IMediaStreamDescriptor, &type_spec_IMediaStreamDescriptor, bases.get(), nullptr);
    if (!state->type_IMediaStreamDescriptor)
    {
        return nullptr;
    }

    state->type_IMediaStreamDescriptor2 = py::register_python_type(module.get(), type_name_IMediaStreamDescriptor2, &type_spec_IMediaStreamDescriptor2, bases.get(), nullptr);
    if (!state->type_IMediaStreamDescriptor2)
    {
        return nullptr;
    }

    state->type_IMediaTrack = py::register_python_type(module.get(), type_name_IMediaTrack, &type_spec_IMediaTrack, bases.get(), nullptr);
    if (!state->type_IMediaTrack)
    {
        return nullptr;
    }

    state->type_ISingleSelectMediaTrackList = py::register_python_type(module.get(), type_name_ISingleSelectMediaTrackList, &type_spec_ISingleSelectMediaTrackList, bases.get(), nullptr);
    if (!state->type_ISingleSelectMediaTrackList)
    {
        return nullptr;
    }

    state->type_ITimedMetadataTrackProvider = py::register_python_type(module.get(), type_name_ITimedMetadataTrackProvider, &type_spec_ITimedMetadataTrackProvider, bases.get(), nullptr);
    if (!state->type_ITimedMetadataTrackProvider)
    {
        return nullptr;
    }

    state->type_MseTimeRange = py::register_python_type(module.get(), type_name_MseTimeRange, &type_spec_MseTimeRange, bases.get(), nullptr);
    if (!state->type_MseTimeRange)
    {
        return nullptr;
    }

    state->type_TimedTextDouble = py::register_python_type(module.get(), type_name_TimedTextDouble, &type_spec_TimedTextDouble, bases.get(), nullptr);
    if (!state->type_TimedTextDouble)
    {
        return nullptr;
    }

    state->type_TimedTextPadding = py::register_python_type(module.get(), type_name_TimedTextPadding, &type_spec_TimedTextPadding, bases.get(), nullptr);
    if (!state->type_TimedTextPadding)
    {
        return nullptr;
    }

    state->type_TimedTextPoint = py::register_python_type(module.get(), type_name_TimedTextPoint, &type_spec_TimedTextPoint, bases.get(), nullptr);
    if (!state->type_TimedTextPoint)
    {
        return nullptr;
    }

    state->type_TimedTextSize = py::register_python_type(module.get(), type_name_TimedTextSize, &type_spec_TimedTextSize, bases.get(), nullptr);
